...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】船橋市の鍵開け・鍵交換業者おすすめ5選！オートロックのマンションで締め出されたときの動き方も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "船橋市で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、オートロックのマンションで締め出されたときにどこへ連絡するか、共用部と専有部で扱いがどう違うかまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】船橋市の鍵開け・鍵交換業者おすすめ5選！オートロックのマンションで締め出されたときの動き方も分かる鍵屋比較",
    description:
      "船橋市で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。オートロックのマンションで締め出されたときの連絡先と、共用部と専有部の扱いの違いも解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、船橋市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（千葉県対応）", note: "千葉県対応の一次確認値。船橋市での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「なくした」のか「持っているのに開かない」のかを整理し、オートロックのマンションかどうかを最初に伝えられるようにしておきましょう。エントランスと住戸の玄関で作業が分かれる可能性があるためです。賃貸なら、業者より先に管理会社・大家、または契約書に記載された緊急時の連絡先へ。玄関の錠の種類が分かれば、電話での見積もり精度が上がります。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、船橋市からも依頼できます（対応エリアは「千葉県」としての一次確認値のため、詳細条件は各社へ確認を）。オートロック物件であることを伝えたうえで各社の見積もりを比べると、作業範囲の違いも見えてきます。金額に納得できなければ、その場で断る選択肢も残しておきましょう。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "船橋市の鍵開け料金の相場はいくらですか？",
    a: "当サイトが一次確認した各社の公式表示では、鍵開けの基本料金はカギ110番6,600円〜、鍵のレスキュー8,000円〜、鍵の生活救急車・JBR8,800円〜（いずれも税込）、カギの救急車は公式目安11,000〜44,000円です。ただしこれは最低価格や目安で、鍵の種類（ディンプルキー等）・時間帯・出張距離で総額は変わります。オートロックの建物では作業範囲が広がる場合があるため、作業前に総額の見積もりを取りましょう。",
    link: { href: "/ryokin/", label: "料金相場の詳細はこちら" },
  },
  {
    q: "オートロックのエントランスも開けてもらえますか？",
    a: "エントランスの扉は共用部にあたり、住戸の玄関とは扱いが分かれます。共用部の解錠を誰が判断するかは管理規約や管理会社の運用によって変わるため、まず管理会社・大家、または賃貸借契約書に記載された緊急時の連絡先へ連絡してください。業者に依頼する場合も、その建物で対応してよいかを管理側に確認しておくと、あとから問題になりません。",
    link: { href: "/autolock-shimedashi-yobou/", label: "オートロックの締め出し予防はこちら" },
  },
  {
    q: "深夜で管理会社につながりません。どうすればいいですか？",
    a: "賃貸借契約書に緊急時の連絡先が記載されていないかを確認してください。24時間対応の窓口が用意されている物件もあります。契約書が手元にない場合は、エントランスの掲示板や集合ポスト付近に管理会社の連絡先が掲示されていることがあります。それでも連絡がつかないときは鍵業者に相談することになりますが、電話の段階でオートロックの建物であることと、管理会社につながらない状況であることを伝えておきましょう。",
  },
  {
    q: "鍵開けを頼むとき、身分証は必要ですか？",
    a: "鍵開けの依頼では、その部屋の居住者であることの確認を求められるのが一般的です。運転免許証などの身分証が室内にあって手元にない場合は、電話の段階でその旨を伝え、どのような方法で確認するかを聞いておくと現地でのやり取りがスムーズです。契約書の控えや公共料金の通知など、住所と氏名が分かるもので代えられる場合もあります。",
  },
  {
    q: "深夜や早朝でも船橋市に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「千葉県」としての一次確認値のため、船橋市での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "同じ締め出しを繰り返さないための備えはありますか？",
    a: "オートロックの物件では、玄関の施錠と同時に外へ出てしまう動線そのものが締め出しの原因になります。鍵を持ったことを確認する所作を出る前に一つ決めておく、家族や同居人と合鍵の置き場所を共有しておくといった備えが現実的です。管理会社が合鍵を預かっている物件かどうかも、平時のうちに確認しておくと落ち着いて動けます。",
...
export default function AreaPageFunabashi() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "船橋市の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜船橋市で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "船橋市の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜船橋市で鍵業者を呼ぶ前に確認したいこと（オートロックは共用部と専有部で扱いが変わる）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜船橋市の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">船橋市の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["船橋市", "マンション", "オートロック", "締め出し", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              船橋市は千葉県北西部に位置し、JR総武線をはじめ複数の鉄道路線が市内を通る、都心への通勤圏として知られるエリアです。駅の周辺には集合住宅が多く、オートロック付きのマンションで暮らす世帯も少なくありません。オートロックの建物では、締め出されたときにまず越えなければならないのが玄関ドアではなく、エントランスの扉になります。
            </p>
            <p>
              本記事では、船橋市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、オートロックのマンションで締め出されたときにどこへ連絡すればよいか、共用部と専有部で扱いがどう違うかを整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">船橋市で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            船橋市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「千葉県」としての一次確認値のため、船橋市での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">船橋市の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            オートロックの建物で締め出されると、エントランスの外で待つことになる場合があります。到着までの時間がそのまま屋外での待ち時間になるため、駆けつけの目安は確認しておきたいところです。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            終電近くに帰宅して締め出しに気づくと、管理会社の営業時間外にあたることが少なくありません。一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、遅い時間の依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            オートロックの締め出しは、エントランスの扉と住戸の玄関で作業が分かれることがあり、その分だけ総額が読みにくくなります。だからこそ内訳を示す方針かどうかが効いてきます。<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          {/* 6｜エリア特有 */}
          <H2 id="tips" num="6">船橋市で鍵業者を呼ぶ前に確認したいこと（オートロックは「共用部」と「専有部」で扱いが変わる）</H2>
          <p className="leading-loose mb-4">
            オートロックのマンションで締め出されたとき、最初に理解しておきたいのが<Hl>エントランスの扉は共用部、住戸の玄関は専有部</Hl>という区分です。共用部の設備は住民全員のもので、居住者が個別に業者を呼んで手を加えられるとは限りません。管理規約や管理会社の運用によって、共用部の扉の解錠を誰が判断するかは変わります。まず管理会社・大家に連絡し、緊急時の窓口や解錠の手順が決められていないかを確認するのが順番です。
          </p>
          <p className="leading-loose mb-4">
            管理会社に連絡がつかない時間帯なら、<Hl>賃貸借契約書に記載された緊急時の連絡先</Hl>を確認してください。24時間対応の窓口が用意されている物件もあり、自分で業者を探すより早く解決することがあります。契約書がすぐに見つからない場合でも、エントランスの掲示板や集合ポスト付近に管理会社の連絡先が掲示されていることがあります。オートロックの解錠を業者に依頼する場合は、その建物で対応してよいかを管理側に確認しておくと、あとから問題になりません。
          </p>
          <p className="leading-loose mb-4">
            業者を呼ぶ段階になったら、電話で<Hl>オートロックのマンションであること</Hl>を必ず伝えてください。エントランスと玄関の両方で作業が必要になるのか、住戸の玄関だけで済むのかによって、作業内容も金額も変わります。あわせて、住戸の玄関錠の種類（刻みキーかディンプルキーか）と、身分証で居住者であることを示せるかどうかも伝えておきましょう。鍵開けの依頼では、その部屋の居住者であることの確認を求められるのが一般的です。オートロック物件での締め出しを繰り返さないための備えは<Link href="/autolock-shimedashi-yobou/" className="text-primary underline">オートロックの締め出し予防</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            鍵をなくしたことが原因の締め出しなら、解錠のあとに<Hl>交換まで進めるかどうか</Hl>の判断が続きます。目安になるのは、失われた鍵から住所にたどり着けるかどうかです。定期券や社員証、宛名の書かれた書類と同じケースに入れていたなら、交換を前向きに検討する場面になります。賃貸の場合、交換の可否や費用の負担は管理会社・大家との相談事項です。初動は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>、費用の考え方は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用</Link>で扱っています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">船橋市の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。締め出しで気が急く場面ほど言われるままになりがちですが、確認すべき点は多くありません。
          </p>
          {steps.map((s) => (
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">船橋市の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            船橋市で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            オートロックのマンションが多い船橋市では、締め出しの初動が「業者に電話する」ではなく「管理会社・緊急連絡先に確認する」から始まります。エントランスは共用部、住戸の玄関は専有部という区分を押さえ、業者を呼ぶときはオートロックの建物であることを最初に伝える。この流れを知っておけば、屋外で立ち尽くす時間を短くできます。そのうえで、本記事の一次確認値を出発点に、時間帯と予算に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["船橋市", "オートロック", "締め出し", "管理会社", "鍵開け", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
