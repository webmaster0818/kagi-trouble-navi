...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】東大阪市の鍵開け・鍵交換業者おすすめ5選！事業所・店舗の鍵管理と従業員の鍵紛失への備えも分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "東大阪市で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、事業所や店舗で鍵を誰がどう管理するか、従業員が鍵をなくしたときにどこまで交換すべきかまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】東大阪市の鍵開け・鍵交換業者おすすめ5選！事業所・店舗の鍵管理と従業員の鍵紛失への備えも分かる鍵屋比較",
    description:
      "東大阪市で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。事業所・店舗の鍵管理の考え方と、従業員の鍵紛失への備えも解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、東大阪市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（大阪府対応）", note: "大阪府対応の一次確認値。東大阪市での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」といった状況に加えて、対象が事業所か店舗か住まいか、扉は何か所か、必要な鍵は何本かを整理してから電話しましょう。交換まで考えている場合は、扉側面の金属プレートに刻印された錠前のメーカー名・型番も控えておくと話が早く進みます。テナント入居であれば、業者より先に貸主・管理会社への確認が必要です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。事業所の鍵交換は扉の数と本数で金額が動くため、同じ条件を伝えて比べると差が見えやすくなります。今回の5社はいずれも全国対応型で、東大阪市からも依頼できます（対応エリアは「大阪府」としての一次確認値のため、詳細条件は各社へ確認を）。関西エリアでは鍵猿も候補になります。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "東大阪市の鍵開け料金の相場はいくらですか？",
    a: "当サイトが一次確認した各社の公式表示では、鍵開けの基本料金はカギ110番6,600円〜、鍵のレスキュー8,000円〜、鍵の生活救急車・JBR8,800円〜（いずれも税込）、カギの救急車は公式目安11,000〜44,000円です。ただしこれは最低価格や目安で、鍵の種類（ディンプルキー等）・時間帯・出張距離で総額は変わります。事業所で複数の扉を交換する場合は本数分の部材代がかかるため、作業前に総額の見積もりを取りましょう。",
    link: { href: "/ryokin/", label: "料金相場の詳細はこちら" },
  },
  {
    q: "従業員が鍵をなくしました。すぐ交換すべきですか？",
    a: "その鍵で開く範囲がどこまでかで判断が変わります。一室だけなのか、建物の出入口や複数の扉に共通するのかで影響の広がりが違うためです。あわせて、失われた鍵から事業所の場所にたどり着けるかどうかも見ます。名刺や社員証、宛名入りの書類と同じケースに入れていたなら、交換を前向きに検討する場面です。まずは持ち出しの記録を確認し、どの範囲が開けられる状態かを把握してください。",
    link: { href: "/kagi-koukan-timing/", label: "鍵交換のタイミングはこちら" },
  },
  {
    q: "キーボックスで鍵を共有しています。気をつけることはありますか？",
    a: "暗証番号を知る人が増え続ける運用は、鍵を配り続けるのと変わりません。退職や担当替えのタイミングで番号を変えると決めておくこと、設置場所を人目に触れにくいところにすること、誰が使ったかを残す形を決めておくことが要点です。番号を口頭やメッセージで安易に共有しない運用も、あわせて取り決めておきましょう。",
    link: { href: "/key-box-kanri/", label: "キーボックスの管理はこちら" },
  },
  {
    q: "テナントとして入居しています。錠を替えるのに許可は必要ですか？",
    a: "扉や錠の変更は建物の設備に関わるため、貸主・管理会社の許可が必要になるのが一般的です。原状回復の取り決めにも関わるため、契約書の内容を確認したうえで事前に相談してください。許可を得ないまま工事を進めると、退去時に費用の問題が生じることがあります。",
    link: { href: "/tenpo-office-kagi-kanri/", label: "店舗・オフィスの鍵管理はこちら" },
  },
  {
    q: "東大阪市は「鍵猿」にも依頼できますか？",
  ...
  {
    q: "深夜や早朝でも東大阪市に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「大阪府」としての一次確認値のため、東大阪市での詳細は依頼時に各社へご確認ください。",
  },
];
...
export default function AreaPageHigashiosaka() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "東大阪市の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜東大阪市で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "東大阪市の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜東大阪市で鍵業者を呼ぶ前に確認したいこと（事業所・店舗の鍵は持ち出しの記録から）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜東大阪市の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">東大阪市の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["東大阪市", "事業所・店舗", "鍵管理", "従業員の鍵紛失", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              東大阪市は大阪府東部の中核市で、ものづくりの町として知られ、中小規模の工場や事業所、店舗が住宅地と近い距離で混在しています。住まいの鍵と違い、事業所や店舗の鍵は複数の人が使い、出入りの時間帯もばらつきます。誰がどの鍵を持っているかが曖昧なまま運用されていると、一本の紛失が全体の作り直しにつながることがあります。
            </p>
            <p>
              本記事では、東大阪市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、事業所や店舗で鍵をどう管理するか、従業員が鍵をなくしたときにどこまで交換を検討すべきかを整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">東大阪市で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            東大阪市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「大阪府」としての一次確認値のため、東大阪市での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-2">
            ※近隣エリアの比較は<Link href="/area/osaka/" className="text-primary underline">大阪版</Link>・<Link href="/area/osaka-hirano/" className="text-primary underline">大阪市平野区版</Link>・<Link href="/area/nara/" className="text-primary underline">奈良版</Link>もご覧ください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※東大阪市を含む関西エリアでは、関西中心に対応する<Link href="/agents/kagizaru/" className="text-primary underline">鍵猿</Link>も選択肢になります（<Link href="/area/osaka/" className="text-primary underline">大阪版の比較記事</Link>に掲載しています）。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">東大阪市の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            朝の開店前や納期前の作業日に鍵が開かないと、事業そのものが止まります。急ぎで開けてほしい場面に備えて、駆けつけの目安は押さえておきましょう。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            早朝の搬入や夜間の作業がある事業所では、トラブルに気づく時刻も一定しません。一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、遅い時間の依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            事業所の鍵交換では、扉の数と鍵の本数がそのまま金額に効いてきます。だからこそ内訳を示す方針かどうかが重要です。<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。本数や扉の数を先に伝えて見積もりを取れば、比較の土台がそろいます。
          </p>
          <p className="leading-loose mb-4">
          ...
          {/* 6｜エリア特有 */}
          <H2 id="tips" num="6">東大阪市で鍵業者を呼ぶ前に確認したいこと（事業所・店舗の鍵は「誰が持ち出したかの記録」から）</H2>
          <p className="leading-loose mb-4">
            事業所や店舗の鍵で最初に整えたいのは、<Hl>誰が、どの鍵を、いつ持ち出したかの記録</Hl>です。ここが曖昧なままだと、紛失に気づいた時点で「どの範囲が開けられる状態なのか」を判断できません。台帳でも表計算ファイルでもかまわないので、鍵の本数と持ち出しの記録を残す形を決めておきましょう。管理の具体的な進め方は<Link href="/tenpo-office-kagi-kanri/" className="text-primary underline">店舗・オフィスの鍵管理</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            共有の鍵をキーボックスで運用している場合は、<Hl>暗証番号を変えるきっかけを決めておく</Hl>ことが要点になります。番号を知る人が増え続ける運用は、鍵を配り続けるのと変わりません。退職や担当替えのタイミングで番号を変える、設置場所を人目に触れにくいところにするといった運用の設計が必要です。考え方は<Link href="/key-box-kanri/" className="text-primary underline">キーボックスの管理</Link>で扱っています。
          </p>
          <p className="leading-loose mb-4">
            従業員が鍵をなくしたときの判断は、<Hl>その鍵で開く範囲がどこまでか</Hl>で変わります。一室だけなのか、建物の出入口や複数の扉に共通するのかで、影響の広がりがまったく違います。あわせて、失われた鍵から事業所の場所にたどり着けるかどうかも見ます。名刺や社員証、宛名の入った書類と同じケースに入れていたなら、交換を前向きに検討する場面です。判断の目安は<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>、初動は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            交換の相談をするときは、扉の数、鍵の必要本数、扉側面の金属プレートに刻印された<Hl>錠前のメーカー名・型番</Hl>を控えておくと見積もりが具体的になります。複数の扉を同じ鍵で開けられるようにしたい、逆に区画ごとに分けたいといった要望があれば、その場で伝えてください。防犯性を上げる方向で検討するなら、防犯性能の高い建物部品として認定された製品を選ぶ考え方もあります。制度の見方は<Link href="/cp-nintei-jou/" className="text-primary underline">CP認定錠の解説</Link>で扱っています。なお、テナントとして入居している場合は、扉や錠の変更に貸主の許可が必要です。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">東大阪市の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。事業所の依頼は本数や扉の数で金額が動くため、事前の整理がそのまま比較のしやすさにつながります。
          </p>
          {steps.map((s) => (
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">東大阪市の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            東大阪市で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            事業所や店舗が身近にある東大阪市では、鍵の話が個人の持ち物にとどまりません。誰がどの鍵を持ち出したかを記録する、キーボックスの番号を変えるきっかけを決めておく、紛失時はその鍵で開く範囲から判断する。この3つを平時のうちに整えておけば、一本の紛失が事業全体の止まりにつながる事態を避けやすくなります。そのうえで、本記事の一次確認値を出発点に、扉の数と本数に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["東大阪市", "事業所", "店舗", "鍵管理", "キーボックス", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
