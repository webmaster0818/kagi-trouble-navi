...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】文京区の鍵開け・鍵交換業者おすすめ5選！マンションの専有部・共用部と鍵交換の手続きも分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "文京区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、分譲マンションの専有部・共用部の線引きと鍵交換の手続き3ステップまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】文京区の鍵開け・鍵交換業者おすすめ5選！マンションの専有部・共用部と鍵交換の手続きも分かる鍵屋比較",
    description:
      "文京区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。分譲マンションで鍵を交換するときの管理規約の確認・管理組合への届け出・施工の3ステップを手続きの順に解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、文京区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。文京区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "分譲マンションの鍵交換なら、依頼の前に管理規約の確認と管理組合・管理会社への相談を済ませ、指定部品・指定手順の有無をメモしておきましょう。ここが済んでいれば依頼内容が明確になり、見積もりの精度も上がります。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、文京区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "文京区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜や早朝でも文京区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、文京区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "分譲マンションの玄関の鍵は、自分の判断で交換してもいいですか？",
    a: "まず管理規約の確認が必要です。標準管理規約では玄関扉の錠は専有部分側の整理ですが、届け出や承認を条件とするマンションは多く、オートロックと連動する鍵の建物では勝手な交換でエントランスと連動しなくなる恐れもあります。規約確認→管理組合・管理会社への相談→業者手配、の順で進めるのが安全です。",
    link: { href: "/kagi-koukan-timing/", label: "鍵交換のタイミングはこちら" },
  },
  {
    q: "エントランスのオートロックも開けられる鍵をなくしました。どこに連絡すべきですか？",
    a: "玄関だけの鍵と違い、拾った人が建物のエントランスまで通過できてしまう可能性があるため、管理会社・管理組合への届け出を優先してください。あわせて警察への遺失届も出しましょう。この方式の鍵やシリンダーの再発行・交換は管理会社経由の手続きになるのが一般的で、対応方法や費用は建物ごとに異なります。",
    link: { href: "/kagi-funshitsu/", label: "鍵をなくしたときの対処はこちら" },
  },
  {
    q: "スマートロックを付けたいのですが、管理組合の許可は必要ですか？",
    a: "機種と規約によります。扉に穴を開けたり外観を変えたりする工事型は、承認が必要とされるのが一般的です。一方、室内側に粘着テープで取り付けるタイプは扉に加工をしないため扱いが緩やかな場合もありますが、規約・細則の解釈は建物ごとに異なります。事前に管理組合・管理会社へ確認してから機種を選ぶのが確実です。",
...
export default function AreaBunkyoKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】文京区の鍵開け・鍵交換業者おすすめ5選！マンションの専有部・共用部と鍵交換の手続きも分かる鍵屋比較",
    description:
      "文京区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "文京区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜文京区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "文京区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜文京区で鍵業者を呼ぶ前に確認したいこと（管理組合と専有部・共用部の鍵の手続き）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜文京区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">文京区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「玄関の鍵を替えたいけれど、管理組合にどう話を通せばいいのか分からない」——住宅地としての歴史が長く、分譲マンションで暮らす人も多い文京区では、鍵の性能そのものより「手続き」でつまずく相談が典型です。マンションの鍵は専有部と共用部の線引きが関わるため、業者を呼ぶ前に踏むべき手順があります。
            </p>
            <p>
              本記事では、文京区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、分譲マンションで鍵を交換するときの管理組合まわりの手続きを3ステップで整理しています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">文京区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            文京区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、文京区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">文京区の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            文京区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">文京区で鍵業者を呼ぶ前に確認したいこと（マンションの鍵交換は「規約確認→届け出→施工」の3ステップ）</H2>
          <p className="leading-loose mb-4">
            最初のステップは<Hl>管理規約と使用細則の確認</Hl>です。マンションの玄関扉まわりは専有部と共用部の線引きが規約で決まっており、国土交通省のマンション標準管理規約では<Hl>玄関扉のうち「錠」と「内部塗装部分」を専有部分とする</Hl>整理が示されています。つまりシリンダー（錠）の交換は専有部側の話とされるのが一般的ですが、扉本体や外観は共用部側のため、実際の可否と条件は自分のマンションの規約が基準になります。
          </p>
          <p className="leading-loose mb-4">
            次のステップは<Hl>管理組合・管理会社への届け出や申請</Hl>です。同等品への交換なら届け出だけで済む場合がある一方、電子錠化など扉の外観が変わる工事は理事会の承認を要する規約が多く見られます。また、1本の鍵で玄関とエントランスのオートロックの両方を開ける方式のマンションでは、<Hl>シリンダーを勝手に交換するとエントランスと連動しなくなる恐れがある</Hl>ため、管理会社指定の手順・部品での施工が前提です。スマートロックを検討する場合も、粘着テープ式で外観を変えないタイプかどうかで扱いが変わります（<Link href="/smart-lock/" className="text-primary underline">スマートロックの解説</Link>）。
          </p>
          <p className="leading-loose mb-4">
            承認が得られたら業者を手配し、<Hl>工事の記録（いつ・どの部品に替えたか）を残して</Hl>おきましょう。売却時や不具合時の説明に役立ちます。逆に、エントランスのオートロックや集合ポストなど<Hl>共用部そのものの不具合は管理組合側の管轄</Hl>で、個人で業者を呼ぶ場面ではありません。交換を考えるきっかけは<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>、オートロック連動の鍵をなくしたときの動き方は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>を参考にしてください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">文京区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">文京区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            文京区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            分譲マンションの鍵は、「規約確認→管理組合・管理会社への届け出→施工と記録」の3ステップを踏めば、後から指摘を受ける心配なく交換・グレードアップできます。オートロックと連動する鍵の建物では、指定手順・指定部品の確認が特に重要です。手続きが済んでいれば、業者選びは本記事の一次確認値の比較から進められます。状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["文京区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
