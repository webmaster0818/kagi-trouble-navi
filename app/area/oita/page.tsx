...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】大分の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "大分で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、温泉・銭湯のロッカーと家の鍵の管理・紛失予防まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】大分の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "大分で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。温泉・銭湯のロッカー利用時の家の鍵の管理と、紛失予防の習慣づくりも解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、大分市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」など状況と、鍵の種類をメモしておくと、電話での見積もり精度が上がります。外出先の施設内でなくした可能性があるなら、業者を呼ぶ前に施設への問い合わせと拾得物の確認を先に済ませましょう。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。紛失を機にしたシリンダー交換のように日程が読める依頼なら、じっくり比較する価値があります。今回の5社はいずれも全国対応型で大分から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs = [
  {
    q: "大分の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "温泉のロッカーの鍵をなくしてしまいました。鍵屋を呼ぶべきですか？",
    a: "いいえ、まず施設のスタッフに申し出てください。ロッカーは施設の設備であり、開錠や錠の交換は施設側の判断で行われます。実費を求められる場合はありますが、利用者が直接鍵業者を手配するのは避けましょう。",
  },
  {
    q: "入浴施設で家の鍵をなくしたときはどうすればよいですか？",
    a: "施設に申し出て、脱衣所や浴場の捜索と拾得物の確認を依頼します。見つからなければ警察へ遺失届を出しましょう。帰宅の手段がなければ鍵開け業者への依頼を検討し、住所の分かる物と一緒になくした場合はシリンダー交換まで視野に入れると安心です。",
  },
  {
    q: "家の鍵を持ち歩くときの紛失予防のコツはありますか？",
    a: "「入れる場所を1か所に固定する」のが基本です。かばんの決まったポケットやリール付きキーチェーンを使い、入浴や運動など鍵を体から離す場面では、ロッカーや貴重品ボックスに収める手順を習慣にしましょう。鍵に住所の分かるタグを付けないことも大切です。",
  },
  {
...
export default function AreaOitaPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】大分の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "大分で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "大分の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#oita5",
      label: "1｜大分で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "大分の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#oita-tips", label: "6｜大分で鍵業者を呼ぶ前に確認したいこと（入浴施設でのロッカー利用と家の鍵の紛失予防）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜大分の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">大分の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「温泉から上がったら、ロッカーに入れたはずの家の鍵が見当たらない」「脱衣所で家の鍵をどう管理すればよいか、いつも迷う」——温泉との付き合いが暮らしに根付いた大分では、入浴施設の脱衣所での鍵の扱いが意外と身近なテーマです。財布やスマートフォンほど注意を向けられない一方で、家の鍵の紛失は生活への影響が大きく、予防の習慣づくりが効いてきます。
            </p>
            <p>
              本記事では、大分市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、入浴施設でのロッカー利用時の鍵の管理と紛失予防のポイントもまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="oita5" num="1">大分で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            大分市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/fukuoka/" className="text-primary underline">福岡版</Link>・<Link href="/area/kitakyushu/" className="text-primary underline">北九州版</Link>・<Link href="/area/kumamoto/" className="text-primary underline">熊本版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">大分の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            大分でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜大分特有 */}
          <H2 id="oita-tips" num="6">大分で鍵業者を呼ぶ前に確認したいこと（入浴施設でのロッカー利用と家の鍵の紛失予防）</H2>
          <p className="leading-loose mb-4">
            入浴施設では、<Hl>家の鍵を財布と同じ「貴重品」として扱う</Hl>ことが出発点です。脱衣かごに入れたままにせず、ロッカーか貴重品ボックスに収めましょう。ロッカーの鍵を腕に着けるタイプでは、バンドの緩みからの脱落が起こりがちです。湯上がりに慌てないよう、「入る前にロッカーへ・出たら着替える前に確認」という順番を自分の習慣にしてしまうのが確実です。
          </p>
          <p className="leading-loose mb-4">
            持ち物を減らすことも有効な紛失予防です。<Hl>家の鍵・車の鍵・職場の鍵をまとめた大きなキーホルダーごと持ち歩くと、1回の置き忘れの影響が生活全体に及びます</Hl>。入浴に行く日は必要な鍵だけを持つ、鍵に住所の分かるタグを付けない、といった小さな工夫で、なくしたときの被害を限定できます。それでも紛失したときの手順は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>で説明しています。
          </p>
          <p className="leading-loose mb-4">
            なくした物によって連絡先が変わる点も押さえておきましょう。<Hl>ロッカーの鍵の紛失は施設の設備の問題なので、まず施設のスタッフへ</Hl>申し出ます。開錠や錠の交換は施設側の判断で行われ、実費を求められる場合があります。一方、家の鍵をなくして帰宅できないときは、遺失届と併せて鍵開け業者への依頼を検討します。<Link href="/shimedashi/" className="text-primary underline">締め出されたときの対処</Link>と<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>もあわせてご覧ください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">大分の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">大分の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            大分で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            入浴施設での鍵の管理は、貴重品として扱う・確認の順番を習慣にする・持って行く鍵を減らす、という小さな工夫の積み重ねです。ロッカーの鍵はまず施設へ、家の鍵の紛失は捜索と遺失届を経て業者への依頼と、なくした物によって動き方が変わる点も覚えておきましょう。交換のように日程が読める依頼は、複数社から総額の見積もりを取って比較するのが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["大分", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
