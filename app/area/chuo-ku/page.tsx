...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】中央区の鍵開け・鍵交換業者おすすめ5選！オートロック付きマンションの鍵の日常運用も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "中央区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、オートロック付きマンションで締め出されないための鍵の日常運用5か条まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】中央区の鍵開け・鍵交換業者おすすめ5選！オートロック付きマンションの鍵の日常運用も分かる鍵屋比較",
    description:
      "中央区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。エントランスと玄関の二重構造で起こる締め出しを防ぐ、オートロック付きマンションの鍵の日常運用も解説しています。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。中央区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、中央区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];
...
const faqs: AreaFaq[] = [
  ...
  },
  {
    ...
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、中央区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "ゴミ出しの数分の間にオートロックで締め出されました。どう動けばいいですか？",
    a: "まず同居家族への連絡、次に管理会社の緊急窓口の順で入室手段を探しましょう。オートロック付きマンションの締め出しでは、エントランスと玄関の両方の解錠が必要になる場合があり、管理会社経由のほうがスムーズなことも多いためです。連絡がつかない場合は24時間受付の鍵業者へ。解錠時は身分証などで居住確認を求められるのが一般的です。再発防止は「どんな短時間の外出でも解錠手段を持つ」の徹底に尽きます。",
    link: { href: "/autolock-shimedashi-yobou/", label: "オートロック締め出しの予防はこちら" },
  },
  {
    q: "玄関とエントランスを1本で開けるタイプの鍵をなくしました。普通の紛失と何が違いますか？",
    a: "エントランスのオートロックと連動する鍵は、建物全体のセキュリティに関わるため、自宅玄関だけの問題では済みません。まず管理会社へ連絡し、建物側の対応（登録の抹消や交換の要否）を確認しましょう。シリンダーを自己判断で交換すると連動が失われる恐れがあるため、交換する場合も管理会社指定の手順・部品で進めるのが前提です。",
...
export default function AreaChuoKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】中央区の鍵開け・鍵交換業者おすすめ5選！オートロック付きマンションの鍵の日常運用も分かる鍵屋比較",
    description:
      "中央区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "中央区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜中央区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "中央区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜中央区で鍵業者を呼ぶ前に確認したいこと（オートロック付きマンションの鍵の日常運用）" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">中央区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「ゴミ出しに出た数分のあいだに、オートロックで締め出された」——マンション暮らしの世帯が多い中央区では、オートロック付きマンションならではの鍵トラブルが日常の中に潜んでいます。エントランスと玄関の二重の扉は防犯の味方ですが、鍵を持たずに一歩外へ出れば、そのまま二重の壁にもなります。
            </p>
            <p>
              本記事では、中央区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、オートロック付きマンションで暮らすうえで押さえておきたい鍵の日常運用のコツもまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">中央区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            中央区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、中央区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          <p className="leading-loose mb-4">
            中央区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">中央区で鍵業者を呼ぶ前に確認したいこと（オートロック付きマンションの鍵の日常運用）</H2>
          <p className="leading-loose mb-4">
            オートロック付きマンションは、エントランスと玄関の二重の扉で守られている反面、<Hl>締め出しも二重に起こります</Hl>。典型は、ゴミ出しや宅配ボックスの荷物の受け取りなど、数分のつもりの外出でエントランスの外へ出てしまうパターンです。毎日の暮らしの中で次の5つを運用できていれば、トラブルの大半は防げます。
          </p>
          <div className="rounded-[2px] border border-border bg-[#F5F8FC] px-5 py-5 md:px-6 mb-4">
            <p className="font-bold text-sm md:text-base mb-3">オートロック付きマンションの鍵・5つの日常運用</p>
            <ol className="grid gap-2 text-sm leading-relaxed list-decimal pl-5">
              <li>どんな短時間の外出でも、解錠手段（鍵またはスマホ）を持って出る</li>
              <li>1本でエントランスと玄関の両方を開けるタイプは、シリンダー交換を管理会社経由で行う（自己判断で替えると連動しなくなる恐れ）</li>
              <li>合鍵は信頼できる家族に預ける（複製に条件がある鍵は管理規約・メーカーの手順に従う）</li>
              <li>オートロックを過信せず、在宅時・外出時とも玄関の施錠を省略しない</li>
              <li>エントランス連動タイプの鍵をなくしたら、業者より先に管理会社へ連絡する</li>
            </ol>
          </div>
          <p className="leading-loose mb-4">
            エントランスは、居住者の後に続けて入る「共連れ」で第三者が通過できてしまう場合があり、<Hl>オートロックがあっても玄関の施錠が最後の砦</Hl>になります。締め出しの予防は<Link href="/autolock-shimedashi-yobou/" className="text-primary underline">オートロック締め出しの予防</Link>、施錠の習慣づくりは<Link href="/kagi-shimewasure-taisaku/" className="text-primary underline">鍵の締め忘れ対策</Link>、玄関の強化を考えるなら<Link href="/hojo-jou/" className="text-primary underline">補助錠の基礎知識</Link>が参考になります。
          </p>
          <p className="leading-loose mb-4">
            締め出しが起きてしまったら、同居家族→管理会社の緊急窓口→24時間受付の鍵業者、の順で入室手段を確保しましょう（<Link href="/shimedashi/" className="text-primary underline">締め出されたときの対処</Link>）。エントランスと連動する鍵の紛失は建物全体のセキュリティに関わるため、<Hl>解錠して家に入れた後も必ず管理会社へ報告</Hl>してください（<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>）。
          </p>
          ...
          <p className="leading-loose mb-4">
            中央区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            オートロック付きマンションの鍵は、「どんな短時間の外出でも解錠手段を持つ」「連動タイプの交換・紛失は管理会社経由」という2つの原則を守るだけで、トラブルの多くを防げます。共連れ対策として、玄関の施錠も省略しないこと。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["中央区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-3">
              {[
                ...
                { href: "/area/toshima-ku/", label: "中央区の鍵開け・鍵交換業者おすすめ5選" },
