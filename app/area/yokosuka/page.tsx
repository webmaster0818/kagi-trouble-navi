...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】横須賀市の鍵開け・鍵交換業者おすすめ5選！玄関・勝手口・門扉の鍵を1本にまとめる「同一キー」の判断と進め方も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "横須賀市で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、玄関・勝手口・門扉の鍵を1本にまとめる「同一キー」にできる条件と、まとめないほうがよい場面まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】横須賀市の鍵開け・鍵交換業者おすすめ5選！玄関・勝手口・門扉の鍵を1本にまとめる「同一キー」の判断と進め方も分かる鍵屋比較",
    description:
      "横須賀市で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。複数の扉の鍵を1本にまとめる「同一キー」にできる条件、費用のかかり方、まとめないほうがよい場面も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、横須賀市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（神奈川県対応）", note: "神奈川県対応の一次確認値。横須賀市での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 対象の扉と錠を書き出す",
    b: "まず、鍵のかかる場所をすべて書き出します。玄関、勝手口、門扉、物置、車庫、内側の間仕切り。それぞれについて、錠のメーカー名と型番（扉側面の金属プレートや錠の表面に刻印があります）、鍵の形（ギザギザの刻みキーか、くぼみの並んだディンプルキーか）、いま何本あるかを控えておきましょう。この一覧があるかないかで、見積もりの精度が大きく変わります。",
  },
  {
    t: "② 出張費・夜間料金を含めた総額の見積もりを取る",
    b: "広告の「◯◯円〜」は最低価格で、実際は錠の種類・出張距離・時間帯で変わります。扉を複数まとめて頼む場合は、1か所目と2か所目以降で料金の扱いが違うことがあるため、扉ごとの内訳が分かる形で出してもらいましょう。見積もり無料・キャンセル無料の業者なら、金額を見てから断ることもできます。",
  },
  {
    t: "③ 運営会社を確認する",
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "鍵をまとめる・入れ替えるという作業は日程に余裕を持てる依頼なので、2〜3社から見積もりを取って総額・提案内容を比較する価値があります。今回の5社はいずれも全国対応型で、横須賀市からも依頼できます（対応エリアは「神奈川県」としての一次確認値のため、詳細条件は各社へ確認を）。扉ごとのメーカー名・型番を同じ条件で伝えれば、取り扱いの可否と金額の差が見えやすくなります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "横須賀市の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "玄関と勝手口の鍵を1本にまとめることはできますか？",
    a: "できる場合があります。同じメーカーの同じシリーズのシリンダーに揃え、同じ鍵で開くように組んでもらう方法で、一般に「同一キー」「同一シリンダー」などと呼ばれます。ただし、扉の厚みや錠の取り付け方が違うと同じシリンダーを入れられないことがあり、門扉や物置のように構造が異なる錠は対象外になることもあります。まずは各扉の錠のメーカー名・型番を控えて、対応できるかを業者に確認するところからです。",
    link: { href: "/joumae-type-zukan/", label: "錠前タイプ図鑑はこちら" },
  },
  {
    q: "1本にまとめると費用は安くなりますか？",
    a: "鍵の本数は減りますが、費用は対象の扉の数だけかかります。シリンダーを揃えるということは、揃える扉のシリンダーを入れ替えるということなので、部材費は扉の数だけ必要です。同じ日にまとめて作業すれば出張費が1回で済む分は抑えられますが、「1本にすると1か所分の値段」にはなりません。見積もりは扉ごとの内訳で出してもらいましょう。",
    link: { href: "/ryokin/", label: "料金相場の詳細はこちら" },
  },
  {
    q: "まとめないほうがよいのはどんな場合ですか？",
    a: "1本をなくしたときに、まとめたすべての扉が開けられる状態になる点が最大の注意点です。日常的に持ち歩く鍵、家族だけが使う鍵、人に貸すことがある鍵（物置や車庫など）は分けておいたほうが、なくしたときの影響を小さくできます。人の出入りが多い家や、物置を第三者に使わせることがある家では、玄関だけは別の鍵にしておくという分け方も選択肢です。",
    link: { href: "/aikagi/", label: "合鍵の作り方と考え方はこちら" },
  },
  {
    q: "賃貸でも鍵を1本にまとめられますか？",
    a: "借りている住まいのシリンダーは貸主の所有物なので、入居者の判断で入れ替えることはできません。交換したい理由があるなら管理会社・貸主へ相談し、許可・費用負担・退去時の扱い（元の錠に戻すのか、そのままでよいのか）を先に決めてください。共用部の鍵やオートロックのキーは、そもそも入居者側で変更できる対象ではありません。",
    link: { href: "/chintai-kagi-koukan-hiyou/", label: "賃貸の鍵交換費用はこちら" },
  },
  {
    q: "深夜や早朝でも横須賀市に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「神奈川県」としての一次確認値のため、横須賀市での詳細は依頼時に各社へご確認ください。",
  },
];
...
export default function AreaPageYokosuka() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "横須賀市の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜横須賀市で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "横須賀市の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#yokosuka-tips", label: "6｜横須賀市で鍵業者を呼ぶ前に確認したいこと（玄関・勝手口・門扉を1本の鍵にまとめる前に確かめること）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜横須賀市の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">横須賀市の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["横須賀市", "同一キー", "勝手口・門扉の錠", "シリンダー交換", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              横須賀市は三浦半島の東側、東京湾に面したまちです。横須賀中央・追浜・久里浜などの市街地の背後に丘と谷が入り組んだ住宅地が広がり、一戸建てでは玄関のほかに勝手口・門扉・物置と、鍵のかかる場所がいくつも並ぶ家が珍しくありません。扉が増えるほど持ち歩く鍵も増え、どれがどの扉のものか分からなくなる場面も出てきます。
            </p>
            <p>
              本記事では、横須賀市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、複数の扉の鍵を1本にまとめる「同一キー」という選び方について、できる条件・費用のかかり方・まとめないほうがよい場面を整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">横須賀市で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            横須賀市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「神奈川県」としての一次確認値のため、横須賀市での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※近隣エリアの比較は<Link href="/area/yokohama/" className="text-primary underline">横浜版</Link>・<Link href="/area/yokohama-totsuka/" className="text-primary underline">横浜市戸塚区版</Link>・<Link href="/area/fujisawa/" className="text-primary underline">藤沢市版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">横須賀市の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            鍵が開かない状態は、扉の前で試し続けるほど時間が過ぎていきます。出入口が複数ある家では、別の扉から入れないかを一通り確かめたあとに業者を探し始めることになり、その分だけ動き出しが遅れがちです。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            家に入れないまま夜を越すかどうかは、早い時間に決めたい判断です。勝手口や門扉まで確かめても入れないと分かったなら、翌朝を待つより夜のうちに手配したほうが早く片づくことがあります。一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、遅い時間の依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            鍵をまとめる・替えるという作業は、対象の扉が増えるほど金額が積み上がります。1か所あたりいくらなのか、2か所目以降はどう計算されるのかを先に示す方針かどうかが効いてきます。<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。今回の紹介では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。玄関以外の勝手口錠や門扉錠、物置の錠はメーカーや年式がばらばらなことが多いため、依頼時に「自社スタッフか協力店か」とあわせて、玄関以外の錠も同じ日に見てもらえるかを確認してみましょう。
          </p>

          {/* 6｜エリア特有 */}
          <H2 id="yokosuka-tips" num="6">横須賀市で鍵業者を呼ぶ前に確認したいこと（玄関・勝手口・門扉を1本の鍵にまとめる前に確かめること）</H2>
          <p className="leading-loose mb-4">
            出入口が複数ある家では、<Hl>玄関・勝手口・門扉・物置と鍵が一本ずつ増えていきます</Hl>。これを1本にまとめる方法が、同じメーカー・同じシリーズのシリンダーに揃えて同じ鍵で開くように組む、いわゆる同一キーの考え方です。持ち歩く鍵が減り、どれがどの扉か迷わなくなるのが利点ですが、まとめられるかどうかは錠の側の条件で決まります。扉の厚み、錠の取り付け方、シリンダーの規格が合わなければ同じものを入れられません。まずは各扉の錠のメーカー名と型番を控えるところからです。錠の型の見分け方は<Link href="/joumae-type-zukan/" className="text-primary underline">錠前タイプ図鑑</Link>、鍵の形の違いは<Link href="/kagi-shurui-zukan/" className="text-primary underline">鍵の種類図鑑</Link>で確認できます。
          </p>
          <p className="leading-loose mb-4">
            費用の考え方も先に押さえておきましょう。<Hl>鍵を1本にするということは、揃える扉のシリンダーをすべて入れ替えるということ</Hl>です。部材費は扉の数だけかかり、抑えられるのは同じ日にまとめて頼むことで出張費が1回で済む部分くらいです。見積もりは「一式」ではなく扉ごとの内訳で出してもらうと、どこを今回やり、どこを次の機会に回すかを自分で決められます。門扉や物置のように屋外にある錠は同じ規格の製品がない場合もあるため、玄関と勝手口だけを揃えて残りは別の鍵のままにする、という組み方も現実的です。相場感は<Link href="/ryokin/" className="text-primary underline">鍵開け・鍵交換の料金相場</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            まとめる前に考えたいのが、<Hl>1本をなくしたときに何が開くか</Hl>という点です。すべての扉が同じ鍵で開くなら、その1本が他人の手に渡ったときの影響は家全体に及びます。日常的に持ち歩く鍵と、家族だけが使う鍵、人に貸すことがある鍵を分けておけば、なくしたときに替える範囲を小さくできます。物置や車庫を親族や業者に使わせることがあるなら、そこは最初から別の鍵にしておくほうが扱いやすくなります。なくしたときの動き方は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>、合鍵の考え方は<Link href="/aikagi/" className="text-primary underline">合鍵の作り方と考え方</Link>で整理しています。
          </p>
          <p className="leading-loose mb-4">
            借りている住まいでは前提が変わります。<Hl>賃貸のシリンダーは貸主の所有物</Hl>なので、入居者の判断で揃えたり入れ替えたりすることはできません。事情があって交換したい場合は、管理会社・貸主へ相談し、許可と費用負担、退去時に元へ戻す必要があるかどうかを先に決めてください。共用部の鍵やオートロックのキーは、そもそも入居者側で変更できる対象ではありません。持ち家でも、一度に全部を替えるのではなく、まず玄関と勝手口、次の機会に門扉、と段階を分けて進めるほうが予算も段取りも組みやすくなります。費用の考え方は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用</Link>、替えどきの目安は<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>で扱っています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">横須賀市の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。扉が複数ある家では、どの扉を対象にするかで金額も工期も変わるぶん、伝える情報の精度が結果を左右します。
          </p>
          {steps.map((s) => (
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">横須賀市の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            横須賀市で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            出入口が複数ある家では、鍵を1本にまとめると日々の持ち歩きは楽になりますが、その1本をなくしたときの影響も1か所にまとまります。各扉の錠のメーカー名と型番を控える、費用は扉の数だけかかると理解しておく、なくしたときに何が開くかで分け方を決める、賃貸なら貸主の許可が先。この4つを押さえたうえで、本記事の一次確認値を出発点に、目的と予算に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["横須賀市", "同一キー", "シリンダーを揃える", "勝手口錠", "門扉錠", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
