...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】佐世保市の鍵開け・鍵交換業者おすすめ5選！机・ロッカー・キャビネットなど玄関以外の小さな錠の鍵をなくしたときの頼み方も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "佐世保市で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、事務机・ロッカー・キャビネットなど玄関以外の小さな錠の鍵をなくしたときの連絡順と、鍵業者への頼み方まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】佐世保市の鍵開け・鍵交換業者おすすめ5選！机・ロッカー・キャビネットなど玄関以外の小さな錠の鍵をなくしたときの頼み方も分かる鍵屋比較",
    description:
      "佐世保市で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。事務机・ロッカー・キャビネットなど玄関以外の小さな錠の鍵をなくしたときの連絡順と、鍵業者への頼み方も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、佐世保市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（長崎県対応）", note: "長崎県対応の一次確認値。佐世保市での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と錠の種類を整理する",
    b: "「鍵をなくした」だけでは足りません。対象が事務机の引き出しなのか、ロッカーなのか、キャビネットなのかで進め方が変わります。錠の形（鍵式か、ダイヤル式か、南京錠か）、鍵や錠に刻印された番号の有無、中身を取り出せればよいのか錠を使い続けたいのかを整理しておきましょう。職場の什器なら、管理部門や店主に報告済みかどうかも最初に伝えておくと話が早く進みます。",
  },
  {
    t: "② 出張費・夜間料金を含めた総額の見積もりを取る",
    b: "広告の「◯◯円〜」は玄関の鍵開けを前提にした最低価格であることが多く、什器の錠は別の料金区分が設けられている場合があります。開けるだけで済むのか、錠ごとの交換になるのか、メーカーから鍵を取り寄せるのかでも金額と日数が変わります。作業前に、出張費を含めた総額と、交換まで進んだ場合の費用を分けて確認しましょう。見積もり無料・キャンセル無料の業者なら、金額を見てから断ることもできます。",
  },
  {
    t: "③ 運営会社を確認する",
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "中身をすぐ取り出す必要がなければ、2〜3社から見積もりを取って総額・提案内容を比較する価値があります。錠の写真と刻印の番号を同じ条件で伝えれば、対応可否と金額の差が見えやすくなります。今回の5社はいずれも全国対応型で、佐世保市からも依頼できます（対応エリアは「長崎県」としての一次確認値のため、詳細条件は各社へ確認を）。取り寄せで済むなら、日数はかかっても費用を抑えられることがあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "佐世保市の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "事務机やキャビネットの鍵をなくしました。鍵屋に頼めますか？",
    a: "対応している業者もありますが、玄関の鍵開けとは料金区分や対応可否が別になっていることがあります。電話の段階で、対象が什器であること、錠の形、刻印された番号の有無、中身を取り出せればよいのか錠を使い続けたいのかを伝えてください。製品によっては、開けるより錠ごと交換するほうが早く安く済むこともあります。",
    link: { href: "/kagi-shurui-zukan/", label: "鍵の種類図鑑はこちら" },
  },
  {
    q: "鍵に刻まれた番号から同じ鍵を作れますか？",
    a: "什器用の小型の錠では、鍵や錠に刻印された番号をもとにメーカーや販売店から同じ鍵を取り寄せられる製品があります。日数はかかりますが、費用を抑えられることが多い方法です。ただし番号は合鍵を作る手がかりそのものでもあるため、控えは鍵と別の場所に保管し、写真を不特定の相手に共有しないようにしてください。",
    link: { href: "/key-number-aikagi/", label: "鍵番号からの合鍵作成はこちら" },
  },
  {
    q: "職場のロッカーの鍵をなくしました。自分で業者を呼んでいいですか？",
    a: "職場の什器は会社や店主の資産で、使う人は預かっている状態です。開ける・壊す・交換するという判断を個人で下すことはできないため、まず管理部門や店主へ報告し、誰が手配して誰が払うのかを決めてから動いてください。中に会社の書類や現金がある場合は、開けるときに誰が立ち会うのかまで決めておくと、あとの説明に困りません。",
    link: { href: "/tenpo-office-kagi-kanri/", label: "店舗・オフィスの鍵管理はこちら" },
  },
  {
    q: "什器の鍵は金庫と同じ扱いになりますか？",
    a: "別物として扱われるのが一般的です。金庫は小型のものでも専用の料金区分が設けられていることが多く、作業の時間も費用も什器の錠とは異なります。電話の段階で「金庫」なのか「机やキャビネットの錠」なのかをはっきり伝えると、見積もりの前提がそろいます。金庫の場合は、製造メーカー名・型番・開け方（鍵式かダイヤル式か）まで伝えられると話が早くなります。",
    link: { href: "/kinko-kaijo/", label: "金庫の解錠はこちら" },
  },
  {
    q: "深夜や早朝でも佐世保市に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「長崎県」としての一次確認値のため、佐世保市での詳細は依頼時に各社へご確認ください。",
  },
];
...
export default function AreaPageSasebo() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "佐世保市の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜佐世保市で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "佐世保市の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#sasebo-tips", label: "6｜佐世保市で鍵業者を呼ぶ前に確認したいこと（小さな錠は「対象」と「持ち主」を先に伝える）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜佐世保市の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">佐世保市の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["佐世保市", "机・キャビネットの錠", "ロッカーの錠", "鍵番号の刻印", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              佐世保市は長崎県の北部にあり、入り組んだ海岸線に面した港のまちとして知られ、駅の周りに商店街や事業所が集まっています。鍵のトラブルというと玄関や車が思い浮かびますが、事務所や店、寮などでは、玄関以外の小さな錠で仕事が止まってしまうことが少なくありません。事務机の引き出し、更衣室のロッカー、書類のキャビネット、什器の扉——どれも小さな鍵ひとつで開くぶん、一本なくしただけで中のものに手が届かなくなります。
            </p>
            <p>
              本記事では、佐世保市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、玄関以外の小さな錠の鍵をなくしたとき、誰に伝え、鍵業者にどう頼めばよいのかを整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">佐世保市で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            佐世保市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「長崎県」としての一次確認値のため、佐世保市での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※近隣エリアの比較は<Link href="/area/nagasaki/" className="text-primary underline">長崎版</Link>・<Link href="/area/saga/" className="text-primary underline">佐賀版</Link>・<Link href="/area/fukuoka/" className="text-primary underline">福岡版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">佐世保市の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            小さな錠の相談は急ぎに見えにくい一方で、中に今日使う書類や現金、私物が入っていると待てません。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            閉店後や夜勤帯に気づくことも多い相談です。ただし職場の什器は自分の判断だけで開けられないため、夜のうちに動くかどうかは、報告先に連絡がつくかで決まります。一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、遅い時間の依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            小さな錠は、開けるだけで済む場合と、錠ごと交換になる場合、メーカーから鍵を取り寄せる場合とで進め方も金額も変わります。どこまでが見積もりに含まれるのかを先に示す方針かどうかが効いてきます。<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。今回の紹介では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。什器の錠は製品ごとの当たり外れが大きく、現場で判断する人の経験がものを言うため、依頼時に「自社スタッフか協力店か」を確認してみましょう。
          </p>

          {/* 6｜エリア特有 */}
          <H2 id="sasebo-tips" num="6">佐世保市で鍵業者を呼ぶ前に確認したいこと（小さな錠は「対象」と「持ち主」を先に伝える）</H2>
          <p className="leading-loose mb-4">
            玄関以外の小さな錠にも、いくつかの型があります。<Hl>事務机やキャビネットは、錠前とシリンダーが一体になった小型の錠が使われていることが多く、鍵や錠の表面に番号が刻印されている製品もあります</Hl>。この番号が読めれば、同じ鍵をメーカーや販売店から取り寄せられる場合があります。更衣室のロッカーは、南京錠、ダイヤル式、シリンダー錠と幅があり、対処もそれぞれ違います。まずは対象の錠の形と、刻印の有無を確かめるところからです。錠の型の見分け方は<Link href="/kagi-shurui-zukan/" className="text-primary underline">鍵の種類図鑑</Link>、番号の扱いは<Link href="/key-number-aikagi/" className="text-primary underline">鍵番号からの合鍵作成</Link>で扱っています。
          </p>
          <p className="leading-loose mb-4">
            次に押さえたいのが、<Hl>その什器が誰の持ち物か</Hl>です。職場の机やロッカー、キャビネットは会社や店主の資産で、使う人はそれを預かっている状態にすぎません。開ける・壊す・交換するという判断を個人で下すことはできず、まず管理部門や店主へ報告し、誰が手配して誰が払うのかを決めてから動くのが順番になります。中に会社の書類や現金が入っている場合はなおさらで、開けるときに誰が立ち会うのかまで決めておくと、あとで説明に困りません。職場の鍵の運用は<Link href="/tenpo-office-kagi-kanri/" className="text-primary underline">店舗・オフィスの鍵管理</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            鍵業者に頼むときは、<Hl>対象が玄関ではなく什器であることを最初に伝える</Hl>のが肝心です。錠の写真、刻印されている番号、中身を取り出せればよいのか錠を残して使い続けたいのかまで伝えられると、対応の可否と費用の見当が電話の段階でつきます。小型の錠は、開けるより<Hl>錠ごと交換する、メーカーから鍵を取り寄せる</Hl>ほうが早く安く済むこともあります。なお、金庫は小型でも別の料金区分になるのが一般的なので、金庫なら金庫と伝えてください。金庫の扱いは<Link href="/kinko-kaijo/" className="text-primary underline">金庫の解錠</Link>、費用の目安は<Link href="/ryokin/" className="text-primary underline">料金相場ページ</Link>で確認できます。
          </p>
          <p className="leading-loose mb-4">
            同じことを繰り返さないための備えも、あわせて整えておきましょう。<Hl>扉ごとに番号を振って一覧にし、本数と持ち主を記録する</Hl>のが出発点です。鍵番号の控えは鍵とは別の場所に保管し、写真で共有しないこと。使う人が替わるときはその場で返してもらい、記録を更新すること。一覧があれば、なくしたときに「どの錠の鍵か」をすぐ言えるようになり、業者への説明も一度で済みます。預け方や保管の注意は<Link href="/key-box-kanri/" className="text-primary underline">キーボックスの管理</Link>、本数の考え方は<Link href="/aikagi/" className="text-primary underline">合鍵の作り方と考え方</Link>で扱っています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">佐世保市の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。什器の錠は対応の可否が分かれやすいぶん、最初の伝え方で差がつきます。
          </p>
          {steps.map((s) => (
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">佐世保市の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            佐世保市で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            玄関以外の小さな錠は、扱いが軽く見られがちなぶん、いざというときに「誰に言えばいいのか」から迷います。錠の形と刻印を確かめる、持ち主である会社や店主に先に報告する、鍵業者には対象が什器だと伝える、そして扉ごとの一覧を作って本数と持ち主を記録しておく。この4つを押さえておけば、同じことで二度止まらずに済みます。そのうえで、本記事の一次確認値を出発点に、目的と予算に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["佐世保市", "小型の錠", "ロッカーの鍵", "鍵番号", "職場の鍵管理", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-3">
              {[
                ...
                { href: "/area/kurume/", label: "久留米市の鍵開け・鍵交換業者おすすめ5選" },
