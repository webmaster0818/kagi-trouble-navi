...
const PATH = "/area/yokohama-tsurumi/";
...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】横浜市鶴見区の鍵開け・鍵交換業者おすすめ5選！工場・倉庫と社員寮の鍵管理まで分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "横浜市鶴見区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、工場・倉庫の鍵の貸与記録の作り方と、社員寮・社宅で締め出されたときの連絡順まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】横浜市鶴見区の鍵開け・鍵交換業者おすすめ5選！工場・倉庫と社員寮の鍵管理まで分かる鍵屋比較",
    description:
      "横浜市鶴見区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。事業所の鍵の台帳管理の考え方と、社員寮・社宅で鍵の困りごとが起きたときの連絡順も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、鶴見区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（神奈川県対応）", note: "神奈川県対応の一次確認値。鶴見区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「回らない」「抜けない」「なくした」といった状況に加えて、対象が事業所の鍵か寮・自宅の鍵かを最初に伝えると話が早く進みます。事業所や寮の鍵は所有者が会社・貸主であるため、担当者や管理者の承諾を得たうえで業者へ連絡するのが原則です。誰の判断で依頼するのかを整理してから電話しましょう。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "緊急性がない案件なら、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、鶴見区からも依頼できます（対応エリアは「神奈川県」としての一次確認値のため、詳細条件は各社へ確認を）。事業所の鍵のように複数箇所をまとめて交換する場合は、本数や箇所数を伝えたうえで合計額を出してもらうと比較しやすくなります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "鶴見区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "社員寮で締め出されました。会社と鍵業者のどちらに先に連絡すべきですか？",
    a: "寮の管理人・寮長がいればまずそこへ、いなければ社宅を担当する部署や管理会社へ連絡するのが基本です。合鍵を管理側が預かっている寮であれば、鍵業者を呼ばずに解決できることがあります。深夜で誰にもつながらない場合に鍵業者を検討する、という順番で考えてください。社員寮は契約主体が会社であることが多く、個人の判断でシリンダーを交換すると原状回復の対象になり得るため、交換は必ず管理側の承諾を得てから進めましょう。",
    link: { href: "/shimedashi/", label: "締め出されたときの対処はこちら" },
  },
  {
    q: "工場・倉庫の鍵を紛失しました。何から対応すべきですか？",
    a: "まず管理責任者へ速やかに報告してください。次に、その鍵でどの扉が開くのかを確認します。交換すべき範囲は開く扉の数で決まるため、マスターキー方式の場合は影響範囲が広くなります。あわせて、鍵の貸与台帳で誰に何本渡っているかを確認し、残りの鍵の所在を押さえてから交換の範囲と時期を決めると、無駄な作業を避けられます。",
    link: { href: "/tenpo-office-kagi-kanri/", label: "店舗・オフィスの鍵管理はこちら" },
  },
  {
    q: "深夜や早朝でも鶴見区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「神奈川県」としての一次確認値のため、鶴見区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "会社名義で鍵の作業を依頼する場合、見積書や領収書は出してもらえますか？",
    a: "多くの業者は書面の発行に対応していますが、宛名の指定や書類の形式は業者ごとに異なります。社内の経費処理に必要な書類がある場合は、依頼の電話の時点で「見積書と領収書を会社名義で発行してほしい」と伝えておくと確実です。あわせて、支払い方法（現金のみか、カードや請求書払いが可能か）も先に確認しておくと、当日のやり取りがスムーズになります。",
  },
  {
...
export default function AreaYokohamaTsurumiPage() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "横浜市鶴見区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜横浜市鶴見区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "横浜市鶴見区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜鶴見区で鍵業者を呼ぶ前に確認したいこと（事業所と社員寮の鍵の運用ルール）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜鶴見区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">横浜市鶴見区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["横浜市鶴見区", "工場・倉庫", "社員寮", "鍵管理", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「交代勤務の時間帯に倉庫の鍵が回らない」「寮に戻ったら鍵を持っていなかった」——臨海部に事業所が集まる鶴見区では、住まいの鍵だけでなく、事業所の鍵と、そこで働く人が暮らす社員寮・社宅の鍵という二種類の鍵が日常的に動いています。どちらも共通しているのは、鍵の持ち主が個人ではなく組織であるという点で、トラブル時の判断も個人宅とは手順が変わります。
            </p>
            <p>
              本記事では、鶴見区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、工場・倉庫など事業所の鍵を組織として管理するための考え方と、社員寮・社宅で鍵の困りごとが起きたときに誰へ連絡すべきかの順番も整理しています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">横浜市鶴見区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            横浜市鶴見区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「神奈川県」としての一次確認値のため、鶴見区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">鶴見区の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            生産や出荷の予定が決まっている現場では、扉が開かない時間がそのまま作業の停止につながります。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            交代勤務や早朝出勤のある職場では、鍵の困りごとが管理部門の勤務時間外に起きることが珍しくありません。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            会社として依頼する場合、金額の内訳がはっきりしていないと社内の承認が通りません。料金の透明性を重視するなら、<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。今回の5社では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。構内への立ち入りに手続きが必要な事業所では、誰が来るのかを事前に把握できるかどうかも判断材料になります。
          </p>

          {/* 6｜区特有 */}
          <H2 id="tips" num="6">鶴見区で鍵業者を呼ぶ前に確認したいこと（事業所・社員寮の鍵は「誰が持っているか」の管理が先）</H2>
          <p className="leading-loose mb-4">
            工場・倉庫・事務所といった事業所の鍵で最も多いつまずきは、故障よりも<Hl>「今その鍵を誰が持っているのか分からない」という状態</Hl>です。個人宅と違って持ち手が入れ替わるため、貸与した相手・貸与日・返却日を記録した台帳がないと、退職や異動のたびに所在不明の鍵が増えていきます。紙でもスプレッドシートでも構わないので、鍵の本数を確定し、1本ごとに管理番号を振り、誰に渡したかを記録する仕組みを先に作りましょう。管理の具体的なやり方は<Link href="/key-box-kanri/" className="text-primary underline">鍵の保管・キーボックス管理</Link>と<Link href="/tenpo-office-kagi-kanri/" className="text-primary underline">店舗・オフィスの鍵管理</Link>で解説しています。
          </p>
          <p className="leading-loose mb-4">
            <Hl>1本の鍵で複数の扉を開けられるマスターキー方式は、運用が楽な反面、その1本が行方不明になったときの影響範囲が一気に広がります</Hl>。どの扉まで開くのかを把握したうえで、持ち出せる人を限定する、退勤時は施錠可能な保管場所に戻す、といったルールを決めておくと安心です。鍵をなくしたときに交換すべき範囲は、その鍵で開く扉の数で決まります。事業所の鍵を紛失した際は、隠さず速やかに管理責任者へ報告する——この一点を社内に周知しておくだけで、被害の広がり方が変わります。
          </p>
          <p className="leading-loose mb-4">
            社員寮・社宅の鍵は、住まいでありながら契約主体が会社であることが多く、<Hl>個人が勝手にシリンダーを交換すると原状回復の対象になり得ます</Hl>。締め出しや紛失が起きたときの連絡順は、寮の管理人・寮長がいればまずそこへ、いなければ社宅を担当する部署や管理会社へ、というのが基本です。合鍵を管理側が預かっている寮であれば、鍵業者を呼ばずに解決することがあります。深夜で誰にもつながらない場合に業者を検討する、という順番で考えましょう。
          </p>
          <p className="leading-loose mb-4">
            寮や社宅では、<Hl>居室の鍵とは別に、建物の玄関・駐輪場・ゴミ置き場・浴室などの共用部の鍵が存在する</Hl>ことがあります。入居時にどの鍵を何本受け取ったのかを記録しておかないと、退去時に「返せない鍵」が出てきて交換費用を請求される原因になります。入居のたびに受け渡す鍵の一覧を作り、入退去時に同じ一覧で照合するのが確実です。管理側が鍵を入れ替える判断をする場面については<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>、退去時の注意点は<Link href="/taikyo-kagi-trouble/" className="text-primary underline">退去時の鍵トラブル</Link>を参考にしてください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">鶴見区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">鶴見区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            横浜市鶴見区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            事業所と社員寮が身近にある鶴見区では、鍵の持ち主が個人ではなく組織であることが多く、トラブル時の判断も個人宅とは変わります。事業所の鍵は貸与台帳で所在を押さえること、寮・社宅は管理側の窓口を先に通すこと。この二つを押さえておけば、いざというときの判断で迷いません。業者に依頼する場合も、誰の承諾で頼むのかを整理したうえで、本記事の一次確認値を出発点に選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["横浜市鶴見区", "工場・倉庫", "社員寮・社宅", "鍵の貸与管理", "鍵交換", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
