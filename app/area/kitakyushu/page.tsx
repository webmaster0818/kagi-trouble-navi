...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】北九州の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "北九州で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、築年数が経った持ち家の古い鍵を交換すべきかの判断基準（廃番シリンダー・ドアノブ一体型）まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】北九州の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "北九州で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。築年数が経った持ち家の古い鍵の交換判断（廃番シリンダー・ドアノブ一体型の扱い）も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、北九州市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「回りにくい」「抜けない」など症状と、玄関・勝手口・車といった対象、鍵の種類をメモしておくと、電話での見積もり精度が上がります。築年数が経った家の古い鍵なら、扉側面の金属プレートに刻印された錠前のメーカー名・型番も控えておくと、廃番かどうかの確認や部品の手配が早く進みます。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。古い鍵の交換のように緊急ではない依頼なら、部品の取り寄せ可否や交換候補の提案内容まで含めて比較する価値があります。今回の5社はいずれも全国対応型で北九州から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs = [
  {
    q: "北九州の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "何十年も前の古い鍵でも交換できますか？部品がないと言われました",
    a: "同じ型のシリンダーが廃番になっていても、後継品や他メーカーの対応品への交換、錠前ごとの交換といった方法で対応できるのが一般的です。扉側面の金属プレートに刻印されたメーカー名・型番を控えて業者に伝えると、対応可否と概算を確認しやすくなります。複数社に聞くと提案内容や費用が異なることもあるため、急ぎでなければ比較がおすすめです。",
  },
  {
    q: "ドアノブに鍵穴が付いている一体型の鍵はどうやって交換しますか？",
    a: "ドアノブと鍵穴が一体になったインテグラル錠などは、シリンダー単体ではなくノブごと交換するのが一般的です。同型の後継品に替える方法のほか、防犯性を上げたい場合はレバーハンドル錠と防犯性の高いシリンダーの組み合わせに切り替える選択肢もあります。部材費が変わるため、見積もりで総額を確認してから決めましょう。",
  },
  {
...
export default function AreaKitakyushuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】北九州の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "北九州で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "北九州の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#kitakyushu5",
      label: "1｜北九州で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "北九州の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#kitakyushu-tips", label: "6｜北九州で鍵業者を呼ぶ前に確認したいこと（古い鍵の交換判断）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜北九州の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">北九州の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「何十年も使ってきた玄関の鍵が回りにくくなった」「合鍵を作ろうとしたら、この鍵はもう部品がないと言われた」——築年数が経った持ち家に長く住み続ける世帯が多い北九州では、鍵そのものの経年劣化や、古い錠前ならではの部品供給の問題に起因するトラブルが起こりやすくなります。古い鍵は直して使い続けるべきか、思い切って交換すべきか。判断基準を知らないまま業者を呼ぶと、提案の妥当性を判断できません。
            </p>
            <p>
              本記事では、北九州市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。古い鍵を交換すべきかどうかの判断基準（寿命のサイン・廃番シリンダー・ドアノブ一体型の扱い）もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="kitakyushu5" num="1">北九州で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            北九州市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/fukuoka/" className="text-primary underline">福岡版</Link>・<Link href="/area/hiroshima/" className="text-primary underline">広島版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">北九州の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            経年劣化が進んだ鍵は、冷え込む早朝や深夜の帰宅時など、不意のタイミングで突然回らなくなることがあります。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜北九州特有 */}
          <H2 id="kitakyushu-tips" num="6">北九州で鍵業者を呼ぶ前に確認したいこと（古い鍵の交換判断）</H2>
          <p className="leading-loose mb-4">
            鍵・錠前は消耗品で、<Hl>回りにくい・鍵の抜き差しが引っかかる・鍵穴がぐらつくといった症状は寿命のサイン</Hl>です。鍵穴専用の潤滑剤で改善することもありますが、食用油や一般的な機械油はホコリを呼んで悪化させるため使ってはいけません。症状が繰り返すようなら、無理に使い続けるより交換を検討するのが安全です。判断の目安は<Link href="/kagi-jumyou/" className="text-primary underline">鍵の寿命の解説記事</Link>と<Link href="/kagi-mawaranai/" className="text-primary underline">鍵が回らないときの対処法</Link>で解説しています。
          </p>
          <p className="leading-loose mb-4">
            築年数が経った家で注意したいのが<Hl>廃番シリンダーの問題</Hl>です。古い錠前は製造が終了していて同じ型のシリンダーが手に入らないことがあり、その場合は後継品や他メーカーの対応品、あるいは錠前ごとの交換になります。依頼前に<Hl>扉側面の金属プレートに刻印されたメーカー名・型番</Hl>を控えて電話で伝えると、対応可否と概算の精度が上がります。錠前の型の見分け方は<Link href="/joumae-type-zukan/" className="text-primary underline">錠前タイプ図鑑</Link>が参考になります。
          </p>
          <p className="leading-loose mb-4">
            また、古い住宅に多い<Hl>ドアノブと鍵穴が一体になったタイプ（インテグラル錠など）は、シリンダーだけでなくノブごと交換になるのが一般的</Hl>です。交換手順や費用感は<Link href="/doorknob-koukan/" className="text-primary underline">ドアノブ交換の解説記事</Link>で説明しています。扉自体の劣化も気になる場合は、<Link href="/door-koukan-vs-kagi-koukan/" className="text-primary underline">ドア交換と鍵交換の比較記事</Link>で費用対効果を確認してから決めると納得感が高まります。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">北九州の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">北九州の鍵業者でよくある質問</H2>
                ...
                <p className="flex gap-3 px-5 py-4 text-sm leading-relaxed">
                  ...
                  <span>
                    ...
                    {f.q.includes("一体型") && (
                      <>
                        {" "}
                        <Link href="/doorknob-koukan/" className="text-primary underline">ドアノブ交換の詳細はこちら</Link>
                      </>
                    )}
                    {f.q.includes("賃貸") && (
                      <>
          ...
          <p className="leading-loose mb-4">
            北九州で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            築年数が経った持ち家の古い鍵は、回りにくさなどの症状が出たら無理に使い続けず、廃番や一体型の事情も踏まえて交換を検討するのが安全です。錠前のメーカー名・型番を控えてから電話し、作業前に総額の見積もりを確認することが失敗しないための共通ルールです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["北九州", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-2">
              {[
                ...
                { href: "/doorknob-koukan/", label: "ドアノブ交換の費用と手順" },
