...
const PATH = "/area/hiroshima-naka/";
...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】広島市中区の鍵開け・鍵交換業者おすすめ5選！終電後の帰宅で起きる締め出しと到着を待つ場所の選び方も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "広島市中区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、深夜営業の店から帰る動線で起きる締め出しと紛失の切り分け、到着を待つ場所の選び方まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】広島市中区の鍵開け・鍵交換業者おすすめ5選！終電後の帰宅で起きる締め出しと到着を待つ場所の選び方も分かる鍵屋比較",
    description:
      "広島市中区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。終電後の帰宅動線で起きる締め出しと紛失の切り分け、到着を待つ場所の選び方も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（広島県対応）", note: "広島県対応の一次確認値。中区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「なくした」のか「部屋の中に置いたまま出てしまった」のかを、電話の前にはっきりさせましょう。締め出しなら解錠だけで済み、紛失なら交換まで検討することになるため、必要な作業も費用も変わります。賃貸であれば管理会社・大家への連絡や、契約に付帯した緊急サポートの有無の確認が先です。玄関の錠の種類とオートロックの有無が分かれば、電話での見積もり精度が上がります。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、中区からも依頼できます（対応エリアは「広島県」としての一次確認値のため、詳細条件は各社へ確認を）。深夜は今すぐ来られる先が限られるぶん即決しがちですが、金額に納得できなければその場で断る選択肢も残しておきましょう。",
  },
];
...
const faqs: AreaFaq[] = [
  {
    ...
    a: "当サイトが一次確認した各社の公式表示では、鍵開けの基本料金はカギ110番6,600円〜、鍵のレスキュー8,000円〜、鍵の生活救急車・JBR8,800円〜（いずれも税込）、カギの救急車は公式目安11,000〜44,000円です。ただしこれは最低価格や目安で、鍵の種類（ディンプルキー等）・時間帯・出張距離で総額は変わります。終電後の時間帯は割増が上乗せされることもあるので、電話の段階で深夜料金の有無を尋ね、作業前に総額の見積もりを取りましょう。",
    link: { href: "/ryokin/", label: "料金相場の詳細はこちら" },
  },
  {
    q: "締め出されただけの場合、鍵の交換まで必要ですか？",
    a: "鍵が室内にあって出られなくなっただけであれば、必要なのは解錠で、錠そのものは無事なため原則として交換までは要りません。交換を検討するのは、鍵が手元から失われていて第三者の手に渡った可能性がある場合です。まずはポケットやかばんの中をすべて確認し、締め出しなのか紛失なのかをはっきりさせてください。この切り分けができていれば、電話の段階で必要な作業を正しく伝えられ、見積もりの精度も上がります。",
    link: { href: "/shimedashi/", label: "締め出しの対処はこちら" },
  },
  {
    q: "深夜に業者の到着を待つ間、どこにいれば安全ですか？",
    a: "自室の前や建物の外でひとり長く立っているのは避けたい状況です。建物のエントランス内に入れるならそこで待ち、入れない場合は、明るく人の目がある場所や終夜営業の店舗など、座って待てる場所を選んでください。待機場所は業者へ伝え、到着時に電話をもらう約束をしておくと行き違いを防げます。気温が厳しい時期は体調面のリスクもあるため、屋外で長時間待たずに済む場所を優先しましょう。",
  },
  {
    q: "お酒を飲んだ状態で契約しても大丈夫でしょうか？",
    a: "判断の質が落ちている前提で手順を決めておくと安全です。作業前に、出張費・部材費・深夜割増まで含めた総額を口頭だけでなく書面や画面で示してもらい、支払い方法も先に確認してください。金額に納得できなければ、その場で契約せずに断る選択肢があります。また、その夜のうちに必要なのは解錠だけで、錠の交換は翌日以降に落ち着いて相談するという分け方もできます。急ぎでない作業まで一度に決めないことが、後悔を防ぐ線引きです。",
  },
  {
    q: "深夜や早朝でも中区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「広島県」としての一次確認値のため、中区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "急いでいるときでも確認しておくべきことはありますか？",
    a: "広告の最低価格だけで決めず、作業前に出張費・部材費・深夜割増まで含めた総額を提示してもらってください。承諾のない追加請求がない方針かどうかと、運営会社がどこかの2点も押さえておくと安心です。鍵業界には名前のよく似た別会社のサービスが複数あり、サービス名だけでは取り違えが起こります。深夜は来られる先が限られるぶん即決しがちですが、金額に納得できないなら断ってよく、その夜は解錠だけにとどめる選択もできます。",
  },
];
...
export default function AreaPageHiroshimaNaka() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "広島の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}/area/hiroshima/` },
      { "@type": "ListItem", position: 3, name: "広島市中区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜広島市中区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "広島市中区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜中区で鍵業者を呼ぶ前に確認したいこと（深夜の締め出しは待つ場所から決める）" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li>
                <Link href="/area/hiroshima/" className="hover:underline">広島</Link>
              </li>
              <li>/</li>
              <li className="text-foreground">広島市中区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["広島市中区", "単身向け賃貸", "深夜の締め出し", "帰宅動線", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              広島市中区は、単身向けの賃貸が集まる居住エリアと、遅い時間まで営業する店が並ぶ区域が隣り合っています。終電のあとに徒歩やタクシーで帰る人が一定数いて、部屋の前に着いてから鍵が開けられないと気づく場面も起こります。ここで最初に分かれるのが、鍵を落としたのか、部屋の中に置いたまま出てしまったのかという点です。
            </p>
            <p>
              本記事では、広島市中区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、締め出しと紛失の切り分け方、深夜に業者の到着を待つ場所の選び方、飲酒後に契約するときの注意点を、帰宅の動線に沿って整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">広島市中区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            広島市中区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「広島県」としての一次確認値のため、中区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※<Link href="/area/hiroshima/" className="text-primary underline">広島全体の情報はこちら</Link>（広島版では同じ一次確認値で5社を比較しています）。
          </p>
          ...
          <p className="leading-loose mb-4">
            深夜に部屋の前で立ち往生している状況では、到着までの時間がそのまま屋外で過ごす時間になります。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            終電後の時間帯に頼れるのは24時間受付の業者です。一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、深夜の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            深夜割増が加わるかどうかで総額が変わるうえ、その場で判断を迫られやすい時間帯でもあります。<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。人通りが減った時間帯にひとりで到着を待つ場面では、どこの誰が来るのかが分かっているほうが落ち着いて待てます。今回の紹介では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。依頼時に「自社スタッフか協力店か」を確認してみましょう。
          </p>

          {/* 6｜区特有 */}
          <H2 id="tips" num="6">中区で鍵業者を呼ぶ前に確認したいこと（深夜の締め出しは「待つ場所」から決める）</H2>
          <p className="leading-loose mb-4">
            部屋に入れないと分かったら、まず<Hl>締め出しなのか紛失なのかを切り分けます</Hl>。鍵は室内にあり出られなくなっただけなら締め出しで、この場合に必要なのは解錠だけです。錠そのものは無事なので、原則として交換までは要りません。一方、鍵が手元から失われているなら、第三者の手に渡った可能性を考える必要があり、解錠のあとに交換を検討する場面が出てきます。かかる費用の見通しがこの時点で大きく変わるので、電話をかける前にポケットとかばんの中を一度すべて確認してください。締め出しの詳しい対処は<Link href="/shimedashi/" className="text-primary underline">締め出しの対処</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            次に確認したいのが、<Hl>賃貸契約に付帯した窓口がないか</Hl>です。物件によっては、管理会社が夜間も受け付ける緊急サポートを用意していたり、契約時に加入した保険・サービスに鍵の解錠が含まれていたりします。自分で業者を探す前に契約書類やスマートフォンに保存した書類を確認すると、費用を抑えられることがあります。オートロックの共用エントランスで締め出された場合も、建物側の窓口が先です。予防の考え方は<Link href="/autolock-shimedashi-yobou/" className="text-primary underline">オートロックの締め出し予防</Link>で解説しています。
          </p>
          <p className="leading-loose mb-4">
            業者を呼ぶと決めたら、<Hl>到着までどこで待つか</Hl>を決めてください。深夜に自室の前や建物の外でひとり長時間立っているのは避けたい状況です。建物のエントランス内に入れるならそこで、入れないなら、明るく人の目がある場所や終夜営業の店舗など、安全に座って待てる場所を選びましょう。その際は業者へ待機場所を伝え、到着時に電話をもらう約束をしておきます。屋外の気温が厳しい時期は、体調面のリスクも考えて場所を選ぶ必要があります。
          </p>
          <p className="leading-loose mb-4">
            飲酒後の依頼では、<Hl>判断の質が落ちている前提で手順を決めておく</Hl>ことが大切です。作業前に、出張費・部材費・深夜割増まで含めた総額を口頭ではなく書面や画面で示してもらい、支払い方法も先に確認してください。金額に納得できなければ、その場で契約せず断る選択肢があります。また、その夜のうちに必要なのは解錠だけで、錠の交換は翌日以降に落ち着いて相談するという分け方も可能です。急ぎでない作業まで一度に決めない。これが深夜の依頼で後悔しないための線引きです。判断の目安は<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>を参考にしてください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">中区の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。深夜は早く終わらせたい気持ちが先に立ちますが、数分の確認でトラブルの多くは防げます。
          </p>
          {steps.map((s) => (
          ...
          <p className="leading-loose mb-4">
            広島市中区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            終電後の帰宅動線でトラブルが起きやすい中区では、締め出しか紛失かの切り分けが最初の分岐点になります。そのうえで、契約に付帯した窓口の有無を確かめ、安全に待てる場所を決め、総額を確認してから契約する。この順番を守れば、深夜でも判断を誤りにくくなります。そのうえで、本記事の一次確認値を出発点に、時間帯と予算に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["広島市中区", "単身向け賃貸", "締め出し", "深夜対応", "鍵開け", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
