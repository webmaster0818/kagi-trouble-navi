...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】松山の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "松山で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、一人暮らしで鍵をなくしたときの連絡の順番（管理会社・警察・業者）まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】松山の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "松山で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。賃貸の一人暮らしで鍵をなくしたときに、誰へどの順番で連絡すべきかの基本も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、松山市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「なくした」「締め出された」「鍵が見当たらないだけかもしれない」など状況と、玄関・自転車・車といった対象をメモしておくと、電話での見積もり精度が上がります。賃貸で玄関の鍵をなくした場合は、業者より先に管理会社・大家への連絡が基本です。合鍵で開けてもらえれば、業者を呼ばずに済むこともあります。連絡がつかない深夜などやむを得ず業者を呼ぶ場合も、後から必ず管理会社に報告しましょう。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。紛失後のシリンダー交換のように日程に余裕がある依頼なら、管理会社経由で手配した場合の費用と比較する余地もあります（自分で業者を手配してよいかは必ず事前に管理会社へ確認を）。今回の5社はいずれも全国対応型で松山から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs = [
  {
    q: "松山の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "一人暮らしで鍵をなくしました。最初に何をすべきですか？",
    a: "まずその日の行動をさかのぼって立ち寄り先や交通機関に問い合わせ、見つからなければ警察へ遺失届を出しましょう。部屋に入れない場合は、鍵業者より先に管理会社・大家へ連絡するのが賃貸の基本です。合鍵やマスターキーで開けてもらえれば費用がかからないこともあります。無断でのシリンダー交換は契約違反となる恐れがあるため避けてください。",
  },
  {
    q: "深夜に締め出されて管理会社に連絡がつかないときは？",
    a: "契約書や物件の掲示に24時間対応の緊急窓口が記載されていないかをまず確認しましょう。見つからず、その夜どうしても部屋に入る必要がある場合は、24時間受付の鍵業者へ依頼するのが現実的です。その際も身分と入居の確認を求められるのが通常で、後日必ず管理会社へ経緯を報告してください。費用を抑えたいなら、到着目安と総額を電話で確認してから依頼しましょう。",
  },
  {
    q: "なくした鍵が後から見つかったら、交換しなくてもいいですか？",
    a: "見つかった場所と経緯によります。自宅内や自分のかばんの中など第三者の手に渡っていないと考えられる場合はそのまま使えることが多い一方、外で紛失して時間が経ってから戻ってきた場合は、複製されている可能性を否定できません。住所を知られている可能性があるなら、管理会社に相談のうえシリンダー交換を検討するのが安心です。",
  },
  {
...
export default function AreaMatsuyamaPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】松山の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "松山で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "松山の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#matsuyama5",
      label: "1｜松山で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "松山の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#matsuyama-tips", label: "6｜松山で鍵業者を呼ぶ前に確認したいこと（賃貸で鍵をなくしたときの連絡順）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜松山の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">松山の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「帰宅したらポケットに鍵がない。夜も遅いし、誰に連絡すればいいのか分からない」——進学や就職を機に一人暮らしを始めた人にとって、初めての鍵の紛失は想像以上に心細いものです。学生や単身赴任の方が多く暮らす松山でも、賃貸住まいの鍵トラブルは身近な相談です。慌てて鍵業者を呼ぶ前に知っておきたいのは、賃貸には「連絡の順番」があり、順番を間違えると余計な費用や契約上のトラブルにつながりかねない、という点です。
            </p>
            <p>
              本記事では、松山市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、賃貸で鍵をなくしたときの一般的な手順（管理会社・大家、警察、業者への連絡順）もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="matsuyama5" num="1">松山で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            松山市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/takamatsu/" className="text-primary underline">高松版</Link>・<Link href="/area/hiroshima/" className="text-primary underline">広島版</Link>・<Link href="/area/okayama/" className="text-primary underline">岡山版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">松山の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            松山でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜松山特有 */}
          <H2 id="matsuyama-tips" num="6">松山で鍵業者を呼ぶ前に確認したいこと（賃貸で鍵をなくしたときの連絡順）</H2>
          <p className="leading-loose mb-4">
            鍵が見当たらないと気づいたら、まずは<Hl>その日の行動をさかのぼり、立ち寄った店や利用した交通機関に問い合わせる</Hl>のが先決です。かばんの底やポケットなど身の回りを確認しても見つからなければ、<Hl>警察（交番）へ遺失届</Hl>を出しましょう。届け出ておけば、拾得物として届いたときに連絡を受けられます。住所の分かるものと一緒に落とした場合は悪用のリスクがあるため、早めの届け出が肝心です。手順の全体像は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの解説記事</Link>で詳しく説明しています。
          </p>
          <p className="leading-loose mb-4">
            賃貸で忘れてはいけないのが、<Hl>鍵業者より先に管理会社・大家へ連絡する</Hl>という順番です。物件によっては合鍵やマスターキーで開けてもらえることがあり、その場合は解錠費用がかからずに済みます。また、賃貸の鍵は貸主の管理物のため、無断でシリンダーを交換すると契約違反となる恐れがあります。深夜で管理会社につながらない場合は、契約書や掲示板に緊急連絡先（管理会社の24時間窓口）がないかを確認し、それでも解決しないときに業者への依頼を検討しましょう。締め出し時の動き方は<Link href="/shimedashi/" className="text-primary underline">締め出しの対処法</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            紛失後にシリンダー交換となった場合、<Hl>費用の負担は賃貸借契約の内容によって決まる</Hl>のが一般的です。借主の過失による紛失では借主負担とされることが多いものの、金額や手配方法は契約と管理会社の方針によるため、勝手に進めず必ず相談してから動きましょう。費用の相場感は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用の解説</Link>を、退去時に関わる注意点は<Link href="/taikyo-kagi-trouble/" className="text-primary underline">退去時の鍵トラブルの解説</Link>をご覧ください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">松山の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">松山の鍵業者でよくある質問</H2>
                ...
                <p className="flex gap-3 px-5 py-4 text-sm leading-relaxed">
                  ...
                  <span>
                    ...
                    {f.q.includes("最初に何を") && (
                      <>
                        {" "}
                        <Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの解説はこちら</Link>
                      </>
                    )}
                    {f.q.includes("連絡がつかない") && (
                      <>
                        {" "}
                        <Link href="/shimedashi/" className="text-primary underline">締め出し時の対処法はこちら</Link>
                      </>
                    )}
                    {f.q.includes("後から見つかったら") && (
                      <>
                        {" "}
          ...
          <p className="leading-loose mb-4">
            松山で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            賃貸の一人暮らしで鍵をなくしたときは、立ち寄り先の確認、警察への遺失届、そして鍵業者より先に管理会社・大家へ連絡、という順番が基本です。合鍵で開けてもらえれば費用がかからないこともあり、無断の交換は契約トラブルのもとになります。業者へ依頼する場合も、作業前の総額見積もりと管理会社への報告を忘れずに。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["松山", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-2">
              {[
                ...
                { href: "/shimedashi/", label: "締め出しの対処法" },
