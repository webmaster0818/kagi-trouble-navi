...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】千代田区の鍵開け・鍵交換業者おすすめ5選！通勤中の鍵紛失の探し方と遺失届の基本も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "千代田区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、通勤中に鍵をなくしたときの探し方・遺失届の出し方・職場の鍵の扱いまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】千代田区の鍵開け・鍵交換業者おすすめ5選！通勤中の鍵紛失の探し方と遺失届の基本も分かる鍵屋比較",
    description:
      "千代田区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。通勤動線をさかのぼる探し方、遺失届の基本、職場から貸与された鍵をなくしたときの連絡の順番も解説しています。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、千代田区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。千代田区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、千代田区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "千代田区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜や早朝でも千代田区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、千代田区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "どこで落としたか分からないのですが、遺失届はどこに出せばいいですか？",
    a: "落とした場所が特定できなくても、最寄りの交番・警察署で遺失届を出すことができます。届け出ておけば、鍵が拾得物として届けられた際に連絡を受けられる可能性が生まれます。鍵は持ち主を特定しにくい落とし物なので、本数・キーホルダーの特徴・落としたと思われる日時や経路をメモして伝えると照合しやすくなります。急がない相談は警察相談専用電話「#9110」も利用できます。",
    link: { href: "/kagi-funshitsu/", label: "鍵をなくしたときの対処はこちら" },
  },
  {
    q: "会社から貸与された鍵も一緒になくしました。何から連絡すべきですか？",
    a: "自宅の鍵より先に、勤務先への報告を優先しましょう。貸与された鍵は会社の管理物で、影響が自分の範囲を超えるためです。報告後の対応（錠交換の要否など）は会社側の判断になります。自宅側は、住所の分かる物と一緒に落としていた場合、第三者に自宅を特定される恐れがあるため、鍵交換を強く検討すべき状況です。",
...
export default function AreaChiyodaKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】千代田区の鍵開け・鍵交換業者おすすめ5選！通勤中の鍵紛失の探し方と遺失届の基本も分かる鍵屋比較",
    description:
      "千代田区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "千代田区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜千代田区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "千代田区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜千代田区で鍵業者を呼ぶ前に確認したいこと（通勤中の鍵紛失と遺失届の基本）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜千代田区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">千代田区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["千代田区", "鍵開け", "鍵交換", "24時間対応", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「会社に着いてから、家の鍵がないことに気づいた」——オフィス街を抱える千代田区では、通勤の行き帰りでの鍵の紛失が典型的なトラブルです。通勤中の紛失は動線が決まっているぶん探しやすい一方で、遺失届の出し方や職場から貸与された鍵の扱いなど、知らないと迷う手続きもあります。
            </p>
            <p>
              本記事では、千代田区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、通勤中に鍵をなくしたときの探し方と遺失届の基本、職場の鍵をなくしたときの連絡の順番も整理しています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">千代田区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            千代田区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、千代田区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">千代田区の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            千代田区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">千代田区で鍵業者を呼ぶ前に確認したいこと（通勤中の鍵紛失と遺失届の基本）</H2>
          <p className="leading-loose mb-4">
            通勤中の紛失には、実は探しやすい面があります。<Hl>「自宅→駅→電車→職場」と動線が決まっている</Hl>ため、さかのぼる範囲が限られるからです。順番は、①机・かばん・上着のポケットの総ざらい、②朝の動線を新しい順に逆にたどる、③利用した鉄道会社・バス会社の忘れ物窓口へ連絡する（路線・乗車時間帯・車両のおおよその位置を伝えると照合しやすくなります）、④立ち寄った店舗・施設への問い合わせ、です（<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>）。
          </p>
          <p className="leading-loose mb-4">
            見つからなければ<Hl>遺失届</Hl>を出します。落とした場所が特定できなくても、最寄りの交番・警察署で届け出ることができ、届け出ておくと拾得物として届いた際に連絡を受けられる可能性が生まれます。鍵は住所も名前も書かれていない落とし物なので、<Hl>本数・キーホルダーの特徴・落としたと思われる日時と経路</Hl>をメモして伝えると照合しやすくなります。急がない相談は警察相談専用電話「#9110」も利用できます。
          </p>
          <p className="leading-loose mb-4">
            職場から貸与された鍵を一緒になくしていた場合は、<Hl>自宅の対応より先に会社への報告を優先</Hl>しましょう。貸与された鍵は会社の管理物で、報告が遅れるほど影響の範囲が広がります（<Link href="/tenpo-office-kagi-kanri/" className="text-primary underline">店舗・オフィスの鍵管理</Link>）。自宅の鍵は、後日戻ってきたとしても、住所の分かる物と一緒に落としていたなら第三者に自宅を特定された可能性を否定できないため、<Hl>鍵交換を強く検討すべき状況</Hl>です（<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>）。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">千代田区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">千代田区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            千代田区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            通勤中の紛失は、持ち物の総ざらい→朝の動線の逆順確認→忘れ物窓口への連絡→遺失届、の順で落ち着いて動くのが確実です。職場の鍵が絡む場合は、自宅より先に会社への報告を。住所の分かる物と一緒に落とした鍵は、戻ってきても交換を検討しましょう。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["千代田区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-3">
              {[
                ...
                { href: "/area/toshima-ku/", label: "千代田区の鍵開け・鍵交換業者おすすめ5選" },
