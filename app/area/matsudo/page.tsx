...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】松戸市の鍵開け・鍵交換業者おすすめ5選！入居・退去のときの鍵交換は誰が負担するのかも分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "松戸市で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、入居時や退去時の鍵交換を誰がいつ行うのか、費用の負担がどう決まるのかまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】松戸市の鍵開け・鍵交換業者おすすめ5選！入居・退去のときの鍵交換は誰が負担するのかも分かる鍵屋比較",
    description:
      "松戸市で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。入居・退去のときの鍵交換を誰が負担するのか、いつ行うのかも解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、松戸市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（千葉県対応）", note: "千葉県対応の一次確認値。松戸市での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」のか、それとも入居・退去にともなう交換なのかを整理してから電話しましょう。賃貸の交換であれば、業者に連絡する前に管理会社・大家への確認が必要です。交換の見積もりを取る場合は、扉側面の金属プレートに刻印された錠前のメーカー名・型番と、必要な鍵の本数を控えておくと話が早く進みます。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。入居前の交換のように日程に余裕がある依頼なら、じっくり比較する価値があります。今回の5社はいずれも全国対応型で、松戸市からも依頼できます（対応エリアは「千葉県」としての一次確認値のため、詳細条件は各社へ確認を）。管理会社が手配する場合は費用の内訳を確認し、自分で手配できるかどうかもあわせて相談してみましょう。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "松戸市の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "入居時の鍵交換費用は借主と大家のどちらが負担しますか？",
    a: "契約の内容によって変わります。入居時の鍵交換費用を借主が負担する取り決めになっている物件は珍しくなく、その場合は契約時の費用として提示されます。まず賃貸借契約書と重要事項説明書で、鍵交換費用がどう扱われているかを確認してください。契約前であれば、その項目について説明を求めることもできます。",
    link: { href: "/chintai-kagi-koukan-hiyou/", label: "賃貸の鍵交換費用はこちら" },
  },
  {
    q: "入居する部屋の鍵が交換されているか、どう確かめればいいですか？",
    a: "管理会社に直接確認するのが確実です。交換されていない場合、以前の合鍵が出回っている可能性を否定できないため、交換を依頼できるかを相談してください。自分で手配したい場合も、賃貸物件の鍵交換は管理会社・大家の許可なしには行えません。入居時に受け取った鍵の本数も、その場で記録しておくと退去時のやり取りが楽になります。",
    link: { href: "/hikkoshi-kagi-checklist/", label: "引っ越し時の鍵チェックリストはこちら" },
  },
  {
    q: "退去時に鍵が1本足りません。どうなりますか？",
    a: "入居時に受け取った本数と返す本数が合わないと、紛失として扱われて交換費用を求められることがあります。自分で作った合鍵も原則として返却の対象です。所在が分からなくなっているなら、立ち会いの直前ではなく早めに管理会社へ伝えてください。時間に余裕があるほど、探す・相談するといった選べる手が増えます。",
    link: { href: "/taikyo-kagi-trouble/", label: "退去時の鍵トラブルはこちら" },
  },
  {
    q: "深夜や早朝でも松戸市に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「千葉県」としての一次確認値のため、松戸市での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "急いでいるときでも確認しておくべきことはありますか？",
    a: "広告の最低価格だけで決めず、作業前に出張費・部材費・深夜割増まで含めた総額を提示してもらってください。承諾のない追加請求がない方針かどうかと、運営会社がどこかの2点も押さえておくと安心です。鍵業界には名前のよく似た別会社のサービスが複数あり、サービス名だけでは取り違えが起こります。賃貸であれば、交換を決める前に管理会社への確認が必要な点も忘れないでください。",
  },
];
...
export default function AreaPageMatsudo() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "松戸市の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜松戸市で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "松戸市の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜松戸市で鍵業者を呼ぶ前に確認したいこと（入居・退去の鍵交換は契約書の確認から）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜松戸市の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">松戸市の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              松戸市は千葉県北西部にあり、江戸川を挟んで東京都と接するエリアです。JR常磐線をはじめ複数の路線が通り、都心へ通う人の住まいとして賃貸物件が数多くあります。人の出入りが多い地域では、鍵のトラブルと同じくらい、入居や退去にともなう鍵交換の扱いに迷う場面が出てきます。誰がいつ行い、費用を誰が負担するのかは、契約の内容によって変わるためです。
            </p>
            <p>
              本記事では、松戸市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、入居時と退去時の鍵交換をどう考えればよいか、費用の負担がどう決まるのかを整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">松戸市で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            松戸市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「千葉県」としての一次確認値のため、松戸市での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※近隣エリアの比較は<Link href="/area/chiba/" className="text-primary underline">千葉版</Link>・<Link href="/area/funabashi/" className="text-primary underline">船橋市版</Link>・<Link href="/area/katsushika-ku/" className="text-primary underline">葛飾区版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">松戸市の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            引っ越しの当日に鍵が開かない、渡された鍵が回らないといった事態は、作業全体を止めてしまいます。急ぎの場面に備えて、駆けつけの目安は押さえておきましょう。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            荷入れが夜遅くまで続いたり、退去の立ち会いが夕方以降になったりすると、対応を頼める時間帯が限られます。一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、遅い時間の依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            鍵交換は、作業費に加えて錠前やシリンダーの部材費がかかります。どこまでが見積もりに含まれるのかを先に示す方針かどうかが効いてきます。<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          {/* 6｜エリア特有 */}
          <H2 id="tips" num="6">松戸市で鍵業者を呼ぶ前に確認したいこと（入居・退去の鍵交換は「契約書の確認」から）</H2>
          <p className="leading-loose mb-4">
            賃貸で鍵交換の話が出たら、まず開くべきは<Hl>賃貸借契約書と重要事項説明書</Hl>です。入居時の鍵交換費用を借主が負担する取り決めになっている物件は珍しくなく、その場合は契約時の費用として提示されます。退去時にどう扱うかも契約に書かれていることがあります。金額や負担の話をする前に、契約上どうなっているかを確認しておくと、管理会社とのやり取りが噛み合いやすくなります。
          </p>
          <p className="leading-loose mb-4">
            入居時に押さえておきたいのが、<Hl>前の入居者が使っていた鍵がそのままか、交換されているか</Hl>です。交換されていない場合、以前の合鍵が出回っている可能性を否定できません。管理会社に確認し、交換されていないなら交換を依頼できるかを相談してください。自分で手配する場合も、賃貸物件の鍵交換は管理会社・大家の許可なしには行えません。引っ越し時に確認しておきたい項目は<Link href="/hikkoshi-kagi-checklist/" className="text-primary underline">引っ越し時の鍵チェックリスト</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            退去時は、<Hl>鍵の返却本数</Hl>が問題になりがちです。入居時に受け取った本数と、返す本数が合わないと、紛失として扱われて交換費用を求められることがあります。自分で作った合鍵がある場合も、原則として返却の対象です。鍵の所在が分からなくなっているなら、立ち会いの直前ではなく、早めに管理会社へ伝えるほうが選べる手が増えます。退去にまつわる鍵の扱いは<Link href="/taikyo-kagi-trouble/" className="text-primary underline">退去時の鍵トラブル</Link>で扱っています。
          </p>
          <p className="leading-loose mb-4">
            費用の負担は、<Hl>紛失によるものか、通常の入退去にともなうものか</Hl>で扱いが分かれるのが一般的な考え方です。入居者の紛失が原因の交換は入居者負担となることが多く、金額は錠前の種類によって幅があります。契約書に鍵の紛失時の取り決めが書かれていることもあるため、話をする前に確認しておきましょう。費用の考え方は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用</Link>、交換すべきタイミングは<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>にまとめています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">松戸市の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。引っ越しの前後は決めることが多い時期ですが、鍵まわりで押さえるべき点は多くありません。
          </p>
          {steps.map((s) => (
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">松戸市の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            松戸市で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            人の出入りが多い松戸市の賃貸では、鍵交換の話が入居と退去の両方で出てきます。まず契約書と重要事項説明書で扱いを確認し、入居時は前の入居者の鍵が交換されているかを管理会社に尋ね、退去時は返却本数を早めに揃える。この順番を押さえておけば、費用の話で行き違いが起きにくくなります。そのうえで、本記事の一次確認値を出発点に、目的と予算に合った業者を選んでください。
          </p>
