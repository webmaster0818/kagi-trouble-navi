...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】上尾市の鍵開け・鍵交換業者おすすめ5選！合鍵を作るとき店で作れる鍵と作れない鍵の違いも分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "上尾市で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、その場で作れる刻みキーとメーカー登録が要る鍵の違い、合鍵から合鍵を作る危うさまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】上尾市の鍵開け・鍵交換業者おすすめ5選！合鍵を作るとき店で作れる鍵と作れない鍵の違いも分かる鍵屋比較",
    description:
      "上尾市で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。その場で作れる合鍵と作れない合鍵の違い、登録制シリンダーの手順、合鍵から合鍵を作る問題も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、上尾市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（埼玉県対応）", note: "埼玉県対応の一次確認値。上尾市での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 鍵の種類と、手元にある鍵が元鍵かどうかを確かめる",
    b: "刻みキーかディンプルキーか、登録制のカードが付いているか。そして、手元にある鍵が購入時に受け取った元の鍵なのか、すでに複製なのか。この2点で、その場で作れるのか、メーカーへの注文になるのか、そもそも作らずシリンダーごと替えたほうが早いのかが決まります。鍵の頭に刻まれたメーカー名も控えておくと話が早くなります。",
  },
  {
    t: "② 出張費・夜間料金を含めた総額の見積もりを取る",
    b: "広告の「◯◯円〜」は最低価格で、実際は鍵の種類・作業内容・出張距離・時間帯で変わります。ディンプルキーや登録制の鍵は、解錠も交換も金額が上がりやすい領域です。合鍵の作成を頼む場合は、1本あたりの単価と納期を、交換を頼む場合は部材費と作業費を分けた内訳で出してもらいましょう。",
  },
  {
    t: "③ 運営会社を確認する",
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "合鍵の作成やシリンダーの交換は、締め出しと違って日程に余裕を持てる依頼です。2〜3社から見積もりを取って、総額と納期を比較する価値があります。今回の5社はいずれも全国対応型で、上尾市からも依頼できます（対応エリアは「埼玉県」としての一次確認値のため、詳細条件は各社へ確認を）。鍵の種類と本数を同じ条件で伝えれば、取り扱いの可否と金額の差が見えやすくなります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "上尾市の鍵開け料金の相場はいくらですか？",
    a: "当サイトが一次確認した各社の公式表示では、鍵開けの基本料金はカギ110番6,600円〜、鍵のレスキュー8,000円〜、鍵の生活救急車・JBR8,800円〜（いずれも税込）、カギの救急車は公式目安11,000〜44,000円です。ただしこれは最低価格や目安で、鍵の種類（ディンプルキー等）・時間帯・出張距離で総額は変わります。ディンプルキーは解錠の料金が上がりやすいため、鍵の種類を伝えたうえで総額の見積もりを取りましょう。",
    link: { href: "/ryokin/", label: "料金相場の詳細はこちら" },
  },
  {
    q: "ディンプルキーの合鍵はその場で作れますか？",
    a: "多くの場合、店頭の機械ではその場で作れません。鍵の面にくぼみが並んだディンプルキーは構造が複雑で、メーカーへの注文になることが一般的です。作れると案内された場合でも、精度が出ずに使ううちに回りにくくなることがあります。急いで本数を増やしたい事情があるなら、注文の納期を先に確認してから段取りを組んでください。",
    link: { href: "/dimple-key/", label: "ディンプルキーの解説はこちら" },
  },
  {
    q: "セキュリティカードをなくしました。合鍵は作れませんか？",
    a: "登録制のシリンダーは、カードを使ってメーカーに注文する仕組みのため、カードがないと原則として作れません。ただしメーカーによっては、所有者であることを確認したうえで再発行の手続きを用意している場合があります。手続きがない場合や、確認が取れない場合は、シリンダーごと交換して新しいカードを受け取る形になります。まずはシリンダーのメーカー名を調べるところからです。",
    link: { href: "/junsei-key-saihakkou/", label: "純正キーの再発行はこちら" },
  },
  {
    q: "合鍵から合鍵を作っても大丈夫ですか？",
    a: "おすすめできません。複製を重ねるとわずかな誤差が積み重なり、差し込んだときに引っかかる、途中で止まるといった症状が出ます。回りにくい鍵を無理に回すと、鍵穴の中で折れることもあります。作るときは、購入時に受け取った元の鍵（純正キー・マスターキー）を持っていってください。手元に複製しかない場合は、そのことを店に伝えて相談しましょう。",
    link: { href: "/kagi-ore/", label: "鍵が折れたときの対処はこちら" },
  },
  {
    q: "賃貸で合鍵を増やしてもいいですか？",
    a: "先に管理会社や大家へ相談してください。賃貸では、貸主側が渡した鍵の本数を把握していることが多く、退去時には受け取った本数を返すことになります。無断で増やした合鍵が数に合わないと、退去の精算で話がこじれます。同居人が増えるなど正当な理由があれば、たいていは相談に応じてもらえます。契約書に本数や複製の扱いが書かれていることもあるので、まず確認を。",
    link: { href: "/chintai-kagi-koukan-hiyou/", label: "賃貸の鍵まわりの費用はこちら" },
  },
  {
    q: "深夜や早朝でも上尾市に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「埼玉県」としての一次確認値のため、上尾市での詳細は依頼時に各社へご確認ください。",
  },
];
...
export default function AreaPageAgeo() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "上尾市の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜上尾市で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "上尾市の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#ageo-tips", label: "6｜上尾市で鍵業者を呼ぶ前に確認したいこと（合鍵を作るとき、店で作れる鍵と作れない鍵）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜上尾市の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">上尾市の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["上尾市", "合鍵", "ディンプルキー", "登録制シリンダー", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              上尾市は埼玉県の中東部、さいたま市の北に隣接するまちです。高崎線沿いに市街地が広がり、都心へ通う人が住む集合住宅と、古くからの戸建てが混ざって並んでいます。家族が増えたとき、親が通うようになったとき、自宅を人に貸すとき。合鍵をもう1本という場面はたびたび訪れますが、そこで初めて「この鍵は店では作れません」と言われて戸惑う人がいます。合鍵は、どの鍵でも同じように作れるわけではありません。
            </p>
            <p>
              本記事では、上尾市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、その場で作れる鍵とメーカーへの注文が要る鍵の違い、登録制シリンダーで必要になるもの、合鍵から合鍵を作ったときに起きることを整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">上尾市で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            上尾市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「埼玉県」としての一次確認値のため、上尾市での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※近隣エリアの比較は<Link href="/area/saitama/" className="text-primary underline">さいたま・埼玉版</Link>・<Link href="/area/saitama-omiya/" className="text-primary underline">さいたま市大宮区版</Link>・<Link href="/area/kawagoe/" className="text-primary underline">川越市版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">上尾市の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            合鍵がないまま鍵を1本なくすと、開けてもらうところから始めることになります。手元に予備があるかどうかで、その日の動き方はまったく変わります。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            家に入れないまま夜を越すかどうかは、早い時間に決めたい判断です。合鍵の作成は日中の店舗でしかできませんが、解錠は夜でも頼めます。一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、遅い時間の依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            ディンプルキーのように防犯性の高い鍵は、解錠も交換も金額が上がりやすい領域です。作業の前に総額を示す方針かどうかが効いてきます。<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。今回の紹介では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。合鍵の作成は現場での駆けつけとは別の仕事なので、鍵の種類を伝えたうえで、その場で作れるのか、メーカーへの注文になるのかを先に聞いておくと、二度手間になりません。
          </p>

          {/* 6｜エリア特有 */}
          <H2 id="ageo-tips" num="6">上尾市で鍵業者を呼ぶ前に確認したいこと（合鍵を作るとき、店で作れる鍵と作れない鍵）</H2>
          <p className="leading-loose mb-4">
            合鍵は「どこでも同じように作れる」ものではありません。分かれ目は鍵の種類です。<Hl>ギザギザの刻みキーは、街の合鍵店や量販店のカウンターでその場で作れることが多い</Hl>。一方、鍵の面にくぼみが並んだディンプルキーや、メーカーが複製を管理している登録制の鍵は、店頭の機械では作れません。作れたとしても精度が出ず、使っているうちに回りにくくなることがあります。まずは自分の鍵がどちらなのかを見てください。鍵の形の見分け方は<Link href="/kagi-shurui-zukan/" className="text-primary underline">鍵の種類図鑑</Link>、合鍵そのものの考え方は<Link href="/aikagi/" className="text-primary underline">合鍵の作り方と考え方</Link>、ディンプルキーの特徴は<Link href="/dimple-key/" className="text-primary underline">ディンプルキーとは</Link>で扱っています。
          </p>
          <p className="leading-loose mb-4">
            登録制のシリンダーは、購入時にカード（セキュリティカード、キーナンバーカードなどと呼ばれます）が付いてきます。<Hl>合鍵はこのカードを使ってメーカーに注文する仕組み</Hl>で、カードがないと原則として作れません。注文から手元に届くまで日数がかかるため、急ぎでは間に合わない点も押さえておきましょう。カードは鍵と一緒に持ち歩かず、家の中でも鍵とは別の場所に保管してください。カードをなくしてしまった場合は、メーカーによって再発行の手続きがある場合と、シリンダーごと交換になる場合があります。手続きの考え方は<Link href="/junsei-key-saihakkou/" className="text-primary underline">純正キーの再発行</Link>、防犯性の高い錠の位置づけは<Link href="/cp-nintei-jou/" className="text-primary underline">CP認定錠とは</Link>で解説しています。
          </p>
          <p className="leading-loose mb-4">
            作れる鍵であっても、注意したいのが元になる鍵です。<Hl>合鍵から合鍵を作ると、わずかな誤差が積み重なって精度が落ちます</Hl>。1本目はうまく回っても、その複製から作った3本目、4本目は差し込んだときに引っかかる、途中で止まる、といった症状が出ます。回りにくい鍵を無理に回すと、鍵穴の中で折れることもあります。作るときは、購入時に受け取った元の鍵（純正キー・マスターキー）を持っていってください。すでに手元にあるのが複製だけなら、そのことを店に伝えて相談を。回りにくい症状の対処は<Link href="/kagi-mawaranai/" className="text-primary underline">鍵が回らないときの対処</Link>、折れたときの対処は<Link href="/kagi-ore/" className="text-primary underline">鍵が折れたときの対処</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            郵送やインターネットで注文する場合は、やりとりする情報に気を配ってください。鍵の写真を送る、鍵に刻印された番号を伝える。<Hl>この番号だけで合鍵が作れてしまう場合があるため、誰に、どういう経路で渡すのかを確かめてから</Hl>にしましょう。もうひとつ、借りている住まいでは本数の話があります。賃貸では、貸主や管理会社が渡した鍵の本数を把握していることが多く、退去時には受け取った本数を返すことになります。無断で増やした合鍵が数に合わないと、退去の精算で話がこじれます。増やす必要があるなら、先に管理会社へ相談してください。鍵番号の扱いは<Link href="/key-number-aikagi/" className="text-primary underline">鍵番号からの合鍵作成</Link>、賃貸の費用の考え方は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用</Link>で整理しています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">上尾市の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。合鍵まわりの相談は日程に余裕があるぶん、条件をそろえて比べる余地があります。
          </p>
          {steps.map((s) => (
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">上尾市の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            上尾市で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            合鍵は鍵の種類で作り方が変わります。刻みキーはその場で作れることが多く、ディンプルキーや登録制の鍵はメーカーへの注文になり日数がかかる。登録制ならカードが要り、カードは鍵と別に保管する。作るときは複製ではなく元鍵から。賃貸なら本数を管理会社と合わせておく。この4つを押さえておけば、必要になったときに慌てずに済みます。本記事の一次確認値を出発点に、目的と予算に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["上尾市", "合鍵作成", "メーカー登録", "セキュリティカード", "純正キー", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-2">
              {[
                ...
                { href: "/junsei-key-saihakkou/", label: "純正キーの再発行" },
                { href: "/cp-nintei-jou/", label: "CP認定錠とは" },
                { href: "/kagi-ore/", label: "鍵が折れたときの対処" },
              ].map((l) => (
                <li key={l.href}>
                  <Link
                    href={l.href}
