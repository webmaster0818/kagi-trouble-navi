...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】仙台の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "仙台で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、冷え込む朝に鍵が回らないときの注意点、転勤・進学の引っ越しに伴う鍵交換まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】仙台の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "仙台で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。冷え込む朝に鍵が回らないときの注意点や、転勤・進学の引っ越しに伴う鍵まわりのポイントも解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、仙台市内をはじめ宮城県内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類・原因を整理する",
    b: "「開かない」「なくした」「折れた」など状況と、玄関・車・金庫といった対象、ディンプルキーなど鍵の種類をメモしておくと、電話での見積もり精度が上がります。冷え込みの厳しい朝に「鍵が入らない・回らない」場合は、故障ではなく鍵穴内部の凍結が原因のこともあります。無理に回す前に原因を切り分け、凍結が疑われるならその旨も業者に伝えましょう。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。転勤や進学に伴う鍵交換のように日程を選べる依頼なら、なおさら比較の価値があります。今回の5社はいずれも全国対応型で仙台から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs = [
  {
    q: "仙台の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "仙台でも冬に鍵穴が凍って開かなくなることはありますか？",
    a: "あります。仙台でも真冬は氷点下まで冷え込む朝があり、前日の雨や雪で鍵穴に入り込んだ水分が凍ると、鍵が奥まで差さらない・差さっても回らないという症状が出ることがあります。このとき力任せに回すと鍵折れや鍵穴の破損につながるため、まずは凍結を疑って対処するのが先決です。温め方の手順とやってはいけないNG行為（お湯をかける等）は、鍵穴の凍結対処ページで解説しています。対処しても開かない場合や鍵が折れた場合は、業者に相談しましょう。",
  },
  {
    q: "深夜や早朝でも仙台に来てくれますか？",
  ...
  {
    q: "転勤で仙台の賃貸に引っ越しました。前の入居者の鍵が心配なのですが、勝手に交換してもいいですか？",
    a: "賃貸の鍵は大家・管理会社の管理物にあたるため、不安があっても無断で交換してはいけません。入居時の鍵交換は管理会社経由で手配されるのが一般的で、交換済みかどうか・費用負担がどうなっているかは賃貸借契約書や管理会社への確認でわかります。締め出しや紛失のときも、まず管理会社・大家に連絡しましょう。合鍵やマスターキーで開けてもらえれば費用がかからずに済む場合があります。",
  },
  {
...
export default function AreaSendaiPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】仙台の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "仙台で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "仙台の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#sendai5",
      label: "1｜仙台で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "仙台の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#sendai-tips", label: "6｜仙台で鍵業者を呼ぶ前に確認したいこと（冷え込む朝の鍵穴・引っ越し）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜仙台の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">仙台の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「国分町からの帰りに鍵をなくした」「転勤で引っ越してきたばかりで、頼める鍵屋を知らない」——仙台で鍵トラブルにあったとき、どの業者に頼めばよいか迷う方は多いはずです。転勤や進学で新しく暮らし始める人が多い仙台では、土地勘のないまま緊急の鍵トラブルに直面しがちです。さらに、東北の冬は氷点下まで冷え込む朝もあり、「鍵が回らない」の原因が故障ではなく鍵穴の凍結だった、というケースも頭に入れておきたいところです。
            </p>
            <p>
              本記事では、仙台市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。冷え込む朝に鍵が回らないときにやってはいけないことや、引っ越し直後の鍵まわりで確認しておきたいこともまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="sendai5" num="1">仙台で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            仙台市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-2">
            ※他の都市の比較は、<Link href="/area/tokyo/" className="text-primary underline">東京版</Link>・<Link href="/area/sapporo/" className="text-primary underline">札幌版</Link>・<Link href="/area/osaka/" className="text-primary underline">大阪版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">仙台の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4 text-sm text-text-muted">
            ※いずれも「最短」の目安であり、拠点からの距離・交通状況・繁忙状況で到着時間は変わります。冬の寒い時期に屋外で待つ場合は、電話の際に到着目安を確認し、近くの店舗など暖の取れる場所で待ちましょう。
          </p>

          {/* 3｜深夜 */}
          <H2 id="shinya" num="3">深夜・早朝の鍵トラブルなら</H2>
          <p className="leading-loose mb-4">
            国分町など夜遅くまでにぎわう街で飲んだ帰りに「鍵がない」と気づくケースや、早朝の出勤前に鍵が回らないと慌てるケースでは、24時間受付かどうかがまず重要です。一次確認値では、<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜仙台特有：冷え込む朝の鍵穴・引っ越し */}
          <H2 id="sendai-tips" num="6">仙台で鍵業者を呼ぶ前に確認したいこと（冷え込む朝の鍵穴・引っ越し）</H2>
          <p className="leading-loose mb-4">
            仙台の冬は、氷点下まで下がる朝も珍しくありません。そんな朝に「鍵が入らない・回らない」と感じたら、故障と決めつける前に<Hl>鍵穴内部の水分が凍っている可能性</Hl>を疑ってみてください。前日の雨や雪のあとの冷え込みで起こりやすい症状で、<Hl>力任せに差し込んだり回したりすると鍵折れや鍵穴の破損を招き</Hl>、かえって高くつきます。お湯をかけるのも、残った水分が再凍結したり部品を傷めたりするためNGです。安全な温め方の手順とNG行為は<Link href="/kagiana-touketsu/" className="text-primary underline">鍵穴が凍結したときの対処法</Link>で解説しています。対処しても開かないときに、はじめて業者依頼を検討すれば十分です。
          </p>
          <p className="leading-loose mb-4">
            また、転勤・進学のシーズンに新生活を始める人が多い仙台では、<Hl>引っ越し直後の鍵まわりの確認</Hl>も大切です。入居時に鍵が交換済みか、合鍵は何本受け取ったか、紛失時の連絡先はどこかを最初に把握しておくと、いざというとき慌てずに済みます。チェック項目は<Link href="/hikkoshi-kagi-checklist/" className="text-primary underline">引っ越し時の鍵チェックリスト</Link>にまとめています。賃貸の鍵は大家・管理会社の管理物のため、<Hl>締め出しや紛失のときは、鍵業者より先に管理会社・大家へ連絡するのが基本</Hl>です。詳しくは<Link href="/shimedashi/" className="text-primary underline">鍵の閉じ込め・締め出しの対処法</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            業者に依頼する前に<Link href="/ryokin/" className="text-primary underline">料金相場</Link>をひと通り把握しておくと、見積もり金額が妥当かどうかを判断しやすくなります。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">仙台の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">仙台の鍵業者でよくある質問</H2>
                ...
                <p className="flex gap-3 px-5 py-4 text-sm leading-relaxed">
                  ...
                  <span>
                    ...
                    {f.q.includes("引っ越し") && (
                      <>
                        {" "}
                        <Link href="/hikkoshi-kagi-checklist/" className="text-primary underline">引っ越し時の鍵チェックリストはこちら</Link>
                      </>
                    )}
                  </span>
                </p>
          ...
          <p className="leading-loose mb-4">
            仙台で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            冷え込む朝の「鍵が回らない」は凍結の可能性をまず疑い、力任せに回さないこと。引っ越し直後なら鍵まわりの基本情報を早めに把握しておくこと。賃貸ならまず管理会社・大家への連絡を済ませたうえで、作業前に総額の見積もりを確認することが失敗しないための共通ルールです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["仙台", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
