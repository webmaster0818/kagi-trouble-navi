...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】大津の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "大津で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、賃貸マンションのオートロックで締め出されたときの連絡の順番まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】大津の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "大津で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。共用部と専有部で管理主体が違うという基本と、締め出し時に誰へどの順で連絡するかも解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、大津市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「エントランスで入れない」のか「自室の前で入れない」のか、鍵をなくしたのか持ち忘れたのかを整理しておくと、連絡先の判断と電話での見積もり精度が上がります。賃貸では自室の錠も大家の所有物のため、業者より先に管理会社・大家へ連絡するのが基本です。夜間は賃貸借契約書や管理会社の緊急連絡先の控えが役立ちます。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。紛失を機にしたシリンダー交換のように日程が読める依頼なら、じっくり比較する価値があります。今回の5社はいずれも全国対応型で大津から依頼でき、関西エリアでは鍵猿も候補になります。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "大津の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "オートロックのマンションで締め出されました。最初に連絡すべきなのは誰ですか？",
    a: "まず合鍵を持つ同居人・家族、次に管理会社（日中なら管理員）です。エントランスのオートロックは共用部の設備で管理会社・管理組合の管轄のため、入居者の判断で錠に手を加えることはできません。管理会社が予備鍵や入館の手段を案内できる場合もあります。大家・鍵業者への連絡はそのあとの選択肢です。",
    link: { href: "/autolock-shimedashi-yobou/", label: "オートロック締め出しの予防はこちら" },
  },
  {
    q: "深夜で管理会社に連絡がつかないときはどうすればよいですか？",
    a: "契約書や物件の掲示にある緊急連絡先（24時間コールセンター等）を確認しましょう。それでもつながらず入室が必要な場合は、鍵業者に自室の解錠を依頼する選択肢がありますが、その部屋に住んでいることの確認（身分証等）を求められるのが一般的です。エントランスの扱いは業者任せにせず、事後でも管理会社に報告してください。",
  },
  {
    q: "大津は「鍵猿」にも依頼できますか？",
    a: "鍵猿は大阪に本社を置き関西エリアを中心に対応する鍵業者で、大津を含む滋賀の方も候補に加えられます。当サイトでは大阪版の比較記事で詳しく紹介しています。対応可否・料金・到着目安は依頼時に必ず確認してください。",
...
export default function AreaOtsuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】大津の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "大津で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "大津の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#otsu5",
      label: "1｜大津で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "大津の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#otsu-tips", label: "6｜大津で鍵業者を呼ぶ前に確認したいこと（賃貸マンションのオートロックと連絡の順番）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜大津の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">大津の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「オートロックのエントランスは通れたのに、自室の鍵がなくて部屋に入れない」「そもそもエントランスの前で立ち往生してしまい、管理会社と鍵業者のどちらに先に連絡すべきか分からない」——オートロック付きマンションの締め出しは、連絡する順番を知っているかどうかで解決までの時間が大きく変わります。マンション暮らしの方も多い大津で慌てないために、共用部と専有部では管理する主体が違うという基本から整理しておきましょう。
            </p>
            <p>
              本記事では、大津市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、オートロック付き賃貸マンションで締め出されたときの連絡の順番もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="otsu5" num="1">大津で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            大津市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/kyoto/" className="text-primary underline">京都版</Link>・<Link href="/area/osaka/" className="text-primary underline">大阪版</Link>・<Link href="/area/kobe/" className="text-primary underline">神戸版</Link>もご覧ください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※大津市を含む関西エリアでは、関西中心に対応する<Link href="/agents/kagizaru/" className="text-primary underline">鍵猿</Link>も選択肢になります（<Link href="/area/osaka/" className="text-primary underline">大阪版の比較記事</Link>に掲載しています）。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">大津の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            大津でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜大津特有 */}
          <H2 id="otsu-tips" num="6">大津で鍵業者を呼ぶ前に確認したいこと（賃貸マンションのオートロックと連絡の順番）</H2>
          <p className="leading-loose mb-4">
            前提として押さえたいのは、<Hl>エントランスのオートロックは共用部＝管理会社・管理組合の管轄、自室の錠は専有部だが賃貸では大家の所有物</Hl>という区分です。入居者の判断で共用部の設備に手を加えることはできず、自室の錠の交換にも管理会社・大家の承諾が要ります。この区分を知っておくと、トラブルのたびに「誰に連絡すべきか」を迷わずに済みます。予防策は<Link href="/autolock-shimedashi-yobou/" className="text-primary underline">オートロック締め出しの予防記事</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            締め出されたときの連絡の順番は、<Hl>①合鍵を持つ同居人・家族 ②管理会社・管理員 ③大家 ④鍵業者</Hl>が基本形です。管理会社は予備鍵や入館手段を案内できる場合があり、費用をかけずに解決できる可能性がもっとも高い窓口です。深夜につながらないときに備えて、契約書に記載の緊急連絡先を携帯電話に控えておきましょう。業者に頼む場合も依頼するのは自室の解錠で、居住確認（身分証等）を求められるのが一般的です。全体の手順は<Link href="/shimedashi/" className="text-primary underline">締め出されたときの対処</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            鍵を「持ち忘れた」のではなく「なくした」場合は、開けて終わりにしないことが大切です。<Hl>部屋番号が分かる物と一緒に鍵を落とすと、オートロックを通過されるリスクにつながる</Hl>ため、管理会社へ報告のうえ、シリンダー交換を相談しましょう（費用負担は契約によります）。<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>と<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用の解説記事</Link>もあわせて参考にしてください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">大津の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">大津の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            大津で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。関西エリアの大津では鍵猿も候補に加えられます。
          </p>
          <p className="leading-loose mb-8">
            オートロック付きマンションの締め出しは、共用部と専有部の管理主体の違いを押さえ、同居人・家族→管理会社→大家→鍵業者という連絡の順番を決めておくことで、費用も時間も抑えやすくなります。紛失時は解錠だけで済ませず、管理会社への報告と交換の相談までがひとつの流れです。交換のように日程が読める依頼は、複数社から総額の見積もりを取って比較することが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["大津", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
