...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】名古屋市中区の鍵開け・鍵交換業者おすすめ5選！深夜営業の店舗で働く人の鍵紛失と勤務先から預かった鍵の扱いも分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "名古屋市中区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、栄や錦など深夜営業の店舗で働く人が閉店後に鍵をなくしたときの連絡の順番と、勤務先から預かった鍵の扱い方まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】名古屋市中区の鍵開け・鍵交換業者おすすめ5選！深夜営業の店舗で働く人の鍵紛失と勤務先から預かった鍵の扱いも分かる鍵屋比較",
    description:
      "名古屋市中区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。深夜営業の店舗で働く人が閉店後に鍵をなくしたときの連絡の順番と、勤務先から預かった鍵の扱い方も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、中区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（愛知県対応）", note: "愛知県対応の一次確認値。中区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「なくした」のか「開かない」のかに加えて、手元から失われた鍵に勤務先のものが含まれていないかを整理してから電話しましょう。事業者の鍵は従業員が預かっているだけで、処分や交換を個人で決められるものではありません。自宅の鍵についても、賃貸なら管理会社・大家への連絡が先です。玄関の錠の種類が分かれば、電話での見積もり精度が上がります。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、中区からも依頼できます（対応エリアは「愛知県」としての一次確認値のため、詳細条件は各社へ確認を）。深夜は「今すぐ来られる先」が限られるぶん即決しがちですが、金額に納得できなければその場で断る選択肢も残しておきましょう。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "中区の鍵開け料金の相場はいくらですか？",
    a: "当サイトが一次確認した各社の公式表示では、鍵開けの基本料金はカギ110番6,600円〜、鍵のレスキュー8,000円〜、鍵の生活救急車・JBR8,800円〜（いずれも税込）、カギの救急車は公式目安11,000〜44,000円です。ただしこれは最低価格や目安で、鍵の種類（ディンプルキー等）・時間帯・出張距離で総額は変わります。深夜帯の依頼では割増が加わることもあるため、作業前に総額の見積もりを取りましょう。",
    link: { href: "/ryokin/", label: "料金相場の詳細はこちら" },
  },
  {
    q: "閉店後に店舗の鍵をなくしました。まず誰に連絡すべきですか？",
    a: "店舗の鍵は会社や店主の資産で、従業員は預かっている立場です。時間帯にかかわらず、まず店長・オーナーへ連絡してください。その鍵で開くのが店内だけなのか、ビルの共用部やテナント区画まで含むのかは従業員側では判断できず、店側が取るべき対応の重さが変わります。翌日の開店担当が鍵を使える状態かどうかも店側でしか把握できません。テナントとして入居している建物であれば、店舗からビル管理者へ話が上がる流れになります。従業員が管理会社へ直接連絡する必要は基本的にありません。",
    link: { href: "/tenpo-office-kagi-kanri/", label: "店舗・オフィスの鍵管理はこちら" },
  },
  {
    q: "勤務先の鍵と自宅の鍵は同じキーホルダーにまとめないほうがいいですか？",
    a: "まとめると持ち出しは楽ですが、一度の紛失で会社と自宅の両方に影響が及びます。分けて持てば被害の範囲を限定できる一方、持ち物が増えて置き忘れが起きやすくなる面もあります。どちらを選ぶにせよ、勤務先から預かっている鍵は自分の判断で複製したり処分したりできないため、保管方法について職場のルールがあるかを確認しておくと安心です。ルールが定められていない職場なら、紛失時の報告先だけでも先に聞いておくと、いざというときに迷いません。",
  },
  {
    q: "退勤後の帰り道で鍵をなくしたかもしれません。探す順番はありますか？",
    a: "まず店内の動線から確認するのが効率的です。ロッカー、休憩室、バックヤード、レジ周りなど、勤務中に鍵へ触れた場所をたどり直してください。店内で出てこなければ、退勤後に立ち寄った場所へ連絡し、利用した交通機関の遺失物窓口へ問い合わせ、それでも見つからなければ警察に遺失届を出します。届け出のときは、本数やキーホルダーの色・形といった特徴を伝えられると照合が進みやすくなります。",
    link: { href: "/kagi-funshitsu/", label: "鍵をなくしたときの対処はこちら" },
  },
  {
    q: "深夜や早朝でも中区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「愛知県」としての一次確認値のため、中区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "急いでいるときでも確認しておくべきことはありますか？",
    a: "広告の最低価格だけで決めず、作業前に出張費・部材費・深夜割増まで含めた総額を提示してもらってください。承諾のない追加請求がない方針かどうか、運営会社はどこかという2点も確認しておくと安心です。鍵業界には名前のよく似た別会社のサービスが複数あるため、サービス名だけでは取り違えが起こります。提示額に納得できない場合は、その場で契約せず一度断る選択肢も残しておきましょう。",
  },
];
...
export default function AreaNagoyaNakaPage() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "名古屋市中区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜名古屋市中区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "名古屋市中区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜中区で鍵業者を呼ぶ前に確認したいこと（閉店後の鍵は連絡の順番で決まる）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜中区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">名古屋市中区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["名古屋市中区", "栄・錦", "鍵紛失", "勤務先の鍵", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              栄・錦・大須といった商業地と、官公庁やオフィスが同じ範囲に重なっている名古屋市中区は、人の動きが夜まで途切れないエリアです。飲食店や接客業のように閉店作業まで担当する職場では、退勤の時刻が公共交通の動く時間から外れることも珍しくありません。その時間帯に「鍵が見当たらない」と気づいた場合、日中に同じことが起きたときとは、相談できる相手の顔ぶれも、動ける順番も変わります。
            </p>
            <p>
              本記事では、中区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、閉店後に店舗の鍵や自宅の鍵が手元から消えたときに誰へどの順番で連絡するか、勤務先から預かっている鍵をどう扱うべきかを、働く側の動線に沿って整理しています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">名古屋市中区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            名古屋市中区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「愛知県」としての一次確認値のため、中区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">中区の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            閉店作業を終えてから鍵の紛失に気づくと、その日のうちに帰宅できるかどうかが到着時間で決まります。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            中区では、退勤時刻そのものが深夜という働き方が一定数あります。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            深夜帯の依頼は割増の有無で総額が変わりやすく、金額の内訳がはっきりしているかどうかが判断材料になります。料金の透明性を重視するなら、<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。人通りが減った時間帯にひとりで到着を待つ場面では、対応者の所属が分かっているほうが落ち着いて待てます。今回の5社では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。依頼時に「自社スタッフか協力店か」を確認してみましょう。
          </p>

          {/* 6｜区特有 */}
          <H2 id="tips" num="6">中区で鍵業者を呼ぶ前に確認したいこと（閉店後の鍵は「連絡の順番」で決まる）</H2>
          <p className="leading-loose mb-4">
            夜間営業の店舗で働いている場合、退勤時に手元にある鍵は一種類とは限りません。<Hl>店舗の出入口、レジやバックヤードの什器、従業員用ロッカー、そして自宅の玄関</Hl>——これらが同じキーホルダーにまとまっていると、一度の紛失で影響が複数方向に広がります。まず確認したいのは、手元から失われた鍵のうち<Hl>どれが事業者の所有物か</Hl>です。店舗の鍵は会社や店主の資産で、従業員はそれを預かっている状態にすぎません。開くのが店内だけなのか、ビル共用部やテナント区画まで含むのかを個人で判断することはできないため、この線引きが最初の分かれ目になります。
          </p>
          <p className="leading-loose mb-4">
            事業者の鍵が含まれるなら、<Hl>閉店後であっても店長・オーナーへの連絡を後回しにしない</Hl>のが基本です。翌日の開店担当が鍵を使えるかどうか、シフト交代のタイミングで受け渡す予定があったかどうかは、店側でしか把握できません。連絡が遅れるほど、店側が取れる手（施錠方法の切り替え、開店時間の調整、鍵の交換手配）が減っていきます。テナントとして入居している建物なら、店側からビル管理者へ話が上がることもあります。従業員が管理会社へ直接掛け合う必要はなく、<Hl>店舗からビル管理者へという順番を崩さない</Hl>ほうが結果的に早く進みます。事業所側の管理の考え方は<Link href="/tenpo-office-kagi-kanri/" className="text-primary underline">店舗・オフィスの鍵管理</Link>で解説しています。
          </p>
          <p className="leading-loose mb-4">
            自宅の鍵については、賃貸なら管理会社・大家が最初の相談先です。深夜で連絡がつかない、または合鍵の預かりがない場合に鍵業者の出番となります。ここで判断が要るのが、<Hl>解錠だけで済ませるか、交換まで進めるか</Hl>という点です。目安になるのは、失われた鍵から住所にたどり着けるかどうか。従業員証や名札、宛名の書かれた書類と同じケースに入れていた場合は、住所と鍵がセットで他人の手に渡った可能性を考える必要があります。判断の目安は<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            探す順番も、勤務時間帯に合わせて考えると迷いません。<Hl>店内の動線（ロッカー、休憩室、バックヤード、レジ周り）をたどり直してから外に出る</Hl>ほうが、心当たりを絞りやすくなります。それでも出てこない場合は、退勤後に立ち寄った場所、利用した交通機関の遺失物窓口、最後に警察への遺失届という流れです。なお、鍵に刻印された番号から合鍵を作れてしまう仕組みがあるため、<Hl>鍵の写真や番号をそのまま人に見せない</Hl>点にも注意してください。仕組みは<Link href="/key-number-aikagi/" className="text-primary underline">鍵番号と合鍵の解説</Link>、初動は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>で扱っています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">中区の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。人の少ない時間帯ほど「早く終わらせたい」と即決しがちですが、数分の確認でトラブルの多くは防げます。
          </p>
          {steps.map((s) => (
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">中区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            名古屋市中区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            夜遅い時間まで人が動く中区では、鍵の紛失に気づく時刻そのものが深夜になりがちです。手元から失われた鍵のうち、どれが事業者の所有物かを最初に切り分け、店舗の鍵は店長・オーナーへ、自宅の鍵は管理会社・大家へと連絡の順番を守るのが最短ルートになります。そのうえで、本記事の一次確認値を出発点に、時間帯と予算に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["名古屋市中区", "栄・錦", "深夜勤務", "勤務先の鍵", "鍵開け", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
