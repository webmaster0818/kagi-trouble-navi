...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】宇都宮の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "宇都宮で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、戸建てで後回しになりがちな勝手口・物置・ガレージの鍵の見直し方まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】宇都宮の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "宇都宮で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。玄関に比べて後回しになりがちな勝手口・物置・ガレージの鍵の防犯と管理の考え方も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、宇都宮市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」「回りにくい」など状況と、玄関・勝手口・物置・ガレージ・車といった対象、刻みキーかディンプルキーかなど鍵の種類をメモしておくと、電話での見積もり精度が上がります。勝手口や物置の鍵をまとめて見直したい場合は、対象の数と種類を先に伝えると、まとめての見積もりが取りやすくなります。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。勝手口の2ロック化や物置・ガレージの錠の掛け替えのように日程に余裕がある依頼なら、複数箇所をまとめて見積もってもらい、総額で比較する価値があります。今回の5社はいずれも全国対応型で宇都宮から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs = [
  {
    q: "宇都宮の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "勝手口の鍵は玄関ほど頑丈でなくても大丈夫ですか？",
    a: "勝手口は人目につきにくく、侵入経路として狙われやすいとされる場所です。玄関だけ強化しても勝手口が手薄では効果が半減するため、勝手口にもピッキングに強いシリンダーへの交換や補助錠の追加（1ドア2ロック）を検討するのがおすすめです。ドアの種類によって使える製品が異なるので、見積もり時に確認しましょう。",
  },
  {
    q: "物置やガレージの鍵も鍵業者に頼めますか？",
    a: "多くの鍵業者は、住宅の玄関だけでなく物置・ガレージ・門扉などの錠にも対応しています。南京錠の掛け替えやシャッター用の錠、後付け補助錠など選択肢は複数あるため、収納している物の価値と合わせて相談すると提案を受けやすくなります。対象が複数あるなら、まとめて見積もりを取って総額で比較しましょう。",
  },
  {
    q: "家の鍵が多くて管理しきれません。よい方法はありますか？",
    a: "まず「どこに・何の鍵が・何本あるか」を書き出して現状を把握するのが第一歩です。使っていない錠や行方不明の合鍵が見つかったら、その場所は交換を検討しましょう。家族での共有には保管場所を1か所に決める方法が有効で、屋外にキーボックスを使う場合は設置場所と暗証番号の管理に注意が必要です。",
  },
  {
...
export default function AreaUtsunomiyaPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】宇都宮の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "宇都宮で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "宇都宮の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#utsunomiya5",
      label: "1｜宇都宮で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "宇都宮の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#utsunomiya-tips", label: "6｜宇都宮で鍵業者を呼ぶ前に確認したいこと（勝手口・物置・ガレージの鍵管理）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜宇都宮の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">宇都宮の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「玄関の鍵は新しくしたのに、勝手口は建てたときのまま」「物置とガレージの鍵、どこにしまったか思い出せない」——車での移動が中心で戸建て暮らしの多い宇都宮では、玄関以外にも勝手口・物置・ガレージ・門扉と、鍵のかかる場所が家のあちこちにあります。ところが防犯の見直しは玄関に集中しがちで、それ以外の鍵は古いまま・管理もあいまいなまま、という家は少なくありません。
            </p>
            <p>
              本記事では、宇都宮市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、勝手口・物置・ガレージといった「玄関以外の鍵」を見直すときの考え方もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="utsunomiya5" num="1">宇都宮で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            宇都宮市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/saitama/" className="text-primary underline">さいたま版</Link>・<Link href="/area/tokyo/" className="text-primary underline">東京版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">宇都宮の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            宇都宮でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜宇都宮特有 */}
          <H2 id="utsunomiya-tips" num="6">宇都宮で鍵業者を呼ぶ前に確認したいこと（勝手口・物置・ガレージの鍵管理）</H2>
          <p className="leading-loose mb-4">
            勝手口は、通りから見えにくい家の裏手や側面にあることが多く、<Hl>人目につきにくいぶん侵入経路として狙われやすい</Hl>とされる場所です。それにもかかわらず、玄関より簡素な鍵が1つだけ、という家も少なくありません。勝手口にも玄関と同じ発想で、ピッキングに強いシリンダーへの交換や、補助錠を足して<Hl>1ドア2ロック</Hl>にする対策を検討しましょう。具体的な考え方は<Link href="/katteguchi-mado-bouhan/" className="text-primary underline">勝手口・窓の防犯の解説記事</Link>と<Link href="/hojo-jou/" className="text-primary underline">補助錠の選び方</Link>で説明しています。
          </p>
          <p className="leading-loose mb-4">
            物置やガレージは、付属の簡易な錠や南京錠だけで運用されがちですが、中に自転車・工具・タイヤなど換金しやすい物を保管しているなら、<Hl>収納物の価値に見合った錠に掛け替える</Hl>のが基本です。南京錠は太さや素材で強度が変わり、シャッターや引き戸には後付けの補助錠という選択肢もあります。家族で共有する鍵が増えてきたら、保管場所を決めて本数を把握しておくことも大切で、<Link href="/key-box-kanri/" className="text-primary underline">キーボックスでの鍵管理の解説記事</Link>が参考になります。
          </p>
          <p className="leading-loose mb-4">
            鍵のかかる場所が多い戸建てでは、<Hl>「どこに・何の鍵が・何本あるか」を一度書き出してみる</Hl>と、古いまま放置されている錠や行方不明の合鍵が見つかることがあります。屋外の錠前は雨風で劣化が進みやすいため、回りが渋くなっていないかの動作確認もあわせて行いましょう。優先順位をつける際は、侵入されやすい場所から手当てするのが定石です。<Link href="/akisu-shinnyu-teguchi/" className="text-primary underline">空き巣の侵入手口と対策</Link>と<Link href="/kagi-shimewasure-taisaku/" className="text-primary underline">閉め忘れ対策の解説記事</Link>もあわせてご覧ください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">宇都宮の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">宇都宮の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            宇都宮で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            戸建ての防犯は玄関だけで完結しません。人目につきにくい勝手口の2ロック化、収納物の価値に見合った物置・ガレージの錠の掛け替え、そして家中の鍵の本数把握が、見落とされがちな基本です。複数箇所をまとめて見直すなら、日程に余裕を持って複数社から総額の見積もりを取り、比較してから依頼しましょう。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["宇都宮", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
