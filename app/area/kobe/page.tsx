...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】神戸の鍵開け・鍵交換業者おすすめ6選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "神戸で鍵開け・鍵交換を頼める鍵業者6社を一次確認値で比較。関西を中心に対応する鍵猿（鍵開け8,800円〜税込）を含め、カギ110番（鍵開け6,600円〜）・鍵の生活救急車・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、オートロックの締め出しで呼ぶ前に確認すべきことまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】神戸の鍵開け・鍵交換業者おすすめ6選！24時間対応の鍵屋を徹底比較",
    description:
      "神戸で鍵開け・鍵交換を頼める鍵業者6社を、料金・受付時間の一次確認値で比較。関西中心の鍵猿を含む6社の選び方や、オートロックの締め出しで業者を呼ぶ前の確認事項も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、神戸市をはじめ兵庫県内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
  ...
  },
  {
    ...
    intro: (
      <>
        ...
        で、関西エリア（大阪・京都・兵庫・奈良・滋賀・和歌山）を中心に豊富な実績を持ち、神戸市を含む兵庫県も対応エリアに含まれます。
        <Hl>下請けを使わない自社施工</Hl>
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」「折れた」など状況と、玄関・車・金庫といった対象、ディンプルキーなど鍵の種類をメモしておくと、電話での見積もり精度が上がります。オートロック付きマンションの場合は「エントランスに入れないのか」「自室の鍵のトラブルか」も切り分けて伝えましょう。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "神戸は全国型の業者に加えて関西中心の業者（鍵猿）も選択肢に入るエリアです。締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs = [
  {
    q: "神戸の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "鍵猿は神戸で依頼できますか？",
    a: "はい。鍵猿は大阪に本社を置くSLS株式会社が運営し、関西エリア（大阪・京都・兵庫・奈良・滋賀・和歌山）を中心に豊富な実績を持つサービスで、神戸市を含む兵庫県も対応エリアに含まれます。下請けを使わない自社施工で、鍵開けは8,800円〜（税込）、基本料・見積料・出張料は0円、1年間の無料工事保証があります。電話受付は8:00〜23:00のため、深夜に新規で連絡する場合は受付時間に注意してください。",
  },
  {
    q: "オートロックのマンションで締め出されたら鍵業者を呼べば解決しますか？",
    a: "オートロックのエントランスは共用部のため、鍵業者が勝手に開けることは基本的にできません。まずは管理会社・大家・管理人に連絡するのが先で、他の入居者の出入りに合わせて共連れで入るのはトラブルのもとになるため避けましょう。自室の鍵のトラブル（紛失・鍵開け）であれば鍵業者に依頼できます。オートロック物件で締め出しを防ぐ習慣は、オートロックの締め出し予防ページで解説しています。",
  },
  {
...
export default function AreaKobePage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】神戸の鍵開け・鍵交換業者おすすめ6選！24時間対応の鍵屋を徹底比較",
    description:
      "神戸で鍵開け・鍵交換を頼める鍵業者6社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "神戸の鍵開け・鍵交換業者おすすめ6選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#kobe6",
      label: "1｜神戸で人気のおすすめ・優良の鍵業者6選",
      sub: [
        { href: "#hikaku", label: "神戸の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#kobe-tips", label: "6｜神戸で鍵業者を呼ぶ前に確認したいこと" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜神戸の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">神戸の鍵開け・鍵交換業者おすすめ6選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「マンションのオートロックの外で鍵がないことに気づいた」「引っ越しを機に鍵を交換したい」——神戸で鍵トラブルにあったとき、どの業者に頼めばよいか迷う方は多いはずです。神戸は大阪と同じく、全国展開の鍵業者に加えて関西を中心に活動する業者（鍵猿）も選択肢に入るエリアです。またオートロック付きマンションの締め出しでは、鍵業者を呼ぶ前に「どこまでが業者で解決できる範囲か」を知っておくことが大切です。
            </p>
            <p>
              本記事では、神戸市内から依頼できる主要な鍵業者6社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。全国型5社に、関西中心の鍵猿を加えた6社構成です。急ぎ・深夜・料金重視・自社施工重視といった目的別の選び方や、オートロック物件で業者を呼ぶ前の確認事項もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜6選 */}
          <H2 id="kobe6" num="1">神戸で人気のおすすめ・優良の鍵業者6選</H2>
          <p className="leading-loose mb-2">
            神戸市内から依頼できる主要な鍵業者6社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※同じ関西の<Link href="/area/osaka/" className="text-primary underline">大阪版</Link>・<Link href="/area/kyoto/" className="text-primary underline">京都版</Link>や、<Link href="/area/tokyo/" className="text-primary underline">東京版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">神戸の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。神戸で自社施工にこだわるなら、<Hl>兵庫県を含む関西エリア中心に対応する鍵猿が選択肢</Hl>になります。下請けを使わない自社施工（自社コールセンター＋自社スタッフ）で、1年間の無料工事保証・無償商品保証があり、防犯設備士も在籍しています。全国型では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。
          </p>
          <p className="leading-loose mb-4">
          ...
          {/* 6｜神戸特有 */}
          <H2 id="kobe-tips" num="6">神戸で鍵業者を呼ぶ前に確認したいこと</H2>
          <p className="leading-loose mb-4">
            オートロック付きマンションで締め出された場合、<Hl>エントランス（共用部）の解錠は鍵業者に頼んでも基本的に解決できません</Hl>。共用部は管理会社・管理組合の管理範囲のため、まず管理会社・大家・管理人への連絡が先です。鍵業者に依頼できるのは、自室玄関の鍵開けや紛失時の鍵交換など専有部のトラブルです。電話の際は「エントランスに入れない」のか「自室の鍵のトラブル」なのかを切り分けて伝えましょう。オートロック物件で締め出しを防ぐ習慣は<Link href="/autolock-shimedashi-yobou/" className="text-primary underline">オートロックの締め出し予防</Link>で解説しています。
          </p>
          <p className="leading-loose mb-4">
            賃貸の鍵は大家・管理会社の管理物にあたるため、<Hl>締め出しや紛失のときは、鍵業者より先に管理会社・大家へ連絡するのが基本</Hl>です。合鍵やマスターキーで開けてもらえれば費用がかからずに済む場合があります。詳しい手順は<Link href="/shimedashi/" className="text-primary underline">鍵の閉じ込め・締め出しの対処法</Link>を、鍵をなくした場合は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処法</Link>（警察への遺失届を含む）をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            また、神戸は全国型の業者と関西中心の業者の両方から選べるエリアです。急ぎでなければ、タイプの異なる業者（紹介型・自社施工型）から見積もりを取り、<Link href="/ryokin/" className="text-primary underline">料金相場</Link>と照らして比較すると、金額の妥当性を判断しやすくなります。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">神戸の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">神戸の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            神戸で鍵開け・鍵交換を頼める主要6社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車・鍵猿・カギ110番、深夜・早朝の新規受付ならカギ110番・JBR・鍵のレスキュー、自社施工と保証重視なら関西中心の鍵猿、料金の明確さ重視なら鍵のレスキューや鍵猿、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            オートロック物件の締め出しは、共用部か専有部かで頼る先が変わります。エントランスの問題はまず管理会社へ、自室の鍵のトラブルは鍵業者へ、という切り分けを覚えておきましょう。どの業者を選ぶ場合も、賃貸ならまず管理会社・大家への連絡、紛失なら警察への遺失届を済ませたうえで、作業前に総額の見積もりを確認することが失敗しないための共通ルールです。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["神戸", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
