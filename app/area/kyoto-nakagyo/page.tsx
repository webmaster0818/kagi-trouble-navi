...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】京都市中京区の鍵開け・鍵交換業者おすすめ6選！賃貸の町家で鍵を替える前の確認事項と引き戸の錠も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "京都市中京区で鍵開け・鍵交換を頼める鍵業者6社を一次確認値で比較。関西中心・自社施工の鍵猿（鍵開け8,800円〜税込）を含め、カギ110番（鍵開け6,600円〜）・鍵の生活救急車・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、賃貸の町家で鍵を替える前に貸主へ確認すべき加工の可否や引き戸の錠の考え方まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】京都市中京区の鍵開け・鍵交換業者おすすめ6選！賃貸の町家で鍵を替える前の確認事項と引き戸の錠も分かる鍵屋比較",
    description:
      "京都市中京区で鍵開け・鍵交換を頼める鍵業者6社を、料金・受付時間の一次確認値で比較。賃貸の町家で鍵を替える前に確認すべき加工の可否と、引き戸の錠の考え方も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、中京区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（京都府対応）", note: "京都府対応の一次確認値。中京区での詳細な対応可否・条件は依頼時に各社へ確認" },
  ...
  },
  {
    ...
    table: [
      ...
      { k: "対応エリア", v: "関西エリア中心（京都府対応）", note: "京都府対応の一次確認値。中京区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「なくした」のか「開かない」のか、あるいは「替えたい」のかを分けて考え、対象が持ち家か賃貸かを整理してから電話しましょう。賃貸で建具に加工を伴う工事をする場合は、貸主・管理者の許可が先です。玄関が開き戸か引き戸か、錠のメーカーや形式が分かれば、電話での見積もり精度が上がります。古い建具では現物を見ないと判断できない部分も多いため、現地見積もりが無料かどうかも確認しておきましょう。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。京都は全国型の業者に加えて関西中心の業者（鍵猿）も選択肢に入るエリアです（対応エリアは「京都府」としての一次確認値のため、中京区での詳細条件は各社へ確認を）。古い建具の工事は、対応できる範囲や提案内容が業者によって分かれやすいため、比較する意味がとくに大きい場面です。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "中京区の鍵開け料金の相場はいくらですか？",
    a: "当サイトが一次確認した各社の公式表示では、鍵開けの基本料金はカギ110番6,600円〜、鍵のレスキュー8,000円〜、鍵の生活救急車・JBR・鍵猿8,800円〜（いずれも税込）、カギの救急車は公式目安11,000〜44,000円です。ただしこれは最低価格や目安で、鍵の種類（ディンプルキー等）・時間帯・出張距離で総額は変わります。古い建具では部材の追加や加工が必要になることもあるため、現地で総額の見積もりを取りましょう。",
    link: { href: "/ryokin/", label: "料金相場の詳細はこちら" },
  },
  {
    q: "賃貸の町家で防犯性の高い鍵に替えたいのですが、自分で決めていいですか？",
    a: "借主の判断だけで進めるのは避けてください。現代の錠を古い建具に取り付けるには彫り込みや穴あけといった加工が必要になることが多く、建物に手を入れる行為にあたります。賃貸には原状回復の考え方があるため、まず管理者や貸主へ、加工してよいか、費用は誰が負担するか、退去時に元へ戻す必要があるかを確認してください。許可が取れない場合でも、加工の少ない方法や補助錠という選択肢があります。",
    link: { href: "/chintai-kagi-koukan-hiyou/", label: "賃貸の鍵交換費用はこちら" },
  },
  {
    q: "古い引き戸の錠は交換できますか？部品はまだありますか？",
    a: "引き戸は召し合わせ部分に錠を設ける構造で、開き戸のシリンダー交換のように部品だけを差し替えれば済むとは限りません。長く使われてきた製品では同じ部品がすでに作られておらず、同型での交換ができない場合があります。その場合は、形状の合う別の製品へ交換するか、既存の錠を残したまま別の錠を足す方法を検討します。現物を見ないと判断できないことが多いため、写真を用意したうえで現地見積もりを依頼するとやり取りが早く進みます。",
    link: { href: "/hikido-kagi/", label: "引き戸の鍵の解説はこちら" },
  },
  {
    q: "建具を加工せずに防犯性を上げる方法はありますか？",
    a: "既存の錠を残したまま別の錠を追加する補助錠が代表的な選択肢です。開錠の手間を増やす考え方で、取り付け方によっては大きな加工を伴いません。ただし固定の方法によって強度は変わり、両面テープ等で留めるタイプは強く引かれる力に対しては限界があります。どの程度の効果を見込めるかは製品と取り付け方次第です。賃貸の場合は、加工を伴わない補助錠であっても、設置前に貸主へ一報を入れておくと後のやり取りが円滑になります。",
    link: { href: "/hojo-jou/", label: "補助錠の選び方はこちら" },
  },
  {
    q: "鍵猿は京都市中京区で依頼できますか？",
    a: "鍵猿は大阪に本社を置くSLS株式会社が運営し、関西エリア（大阪・京都・兵庫・奈良・滋賀・和歌山）を中心に実績を持つサービスで、京都府も対応エリアに含まれます。下請けを使わない自社施工で、鍵開けは8,800円〜（税込）、基本料・見積料・出張料は0円、1年間の無料工事保証・無償商品保証があります。電話受付は8:00〜23:00のため、深夜に新規で連絡する場合は受付時間に注意してください。なお対応エリアは「京都府」としての一次確認値のため、中京区での詳細な対応可否・条件は依頼時に各社へご確認ください。",
    link: { href: "/agents/kagizaru/", label: "鍵猿の詳細レビューはこちら" },
  },
  {
    q: "深夜や早朝でも中京区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。鍵猿は電話受付が8:00〜23:00ですが、施工は深夜・早朝も対応とされています。深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「京都府」としての一次確認値のため、中京区での詳細は依頼時に各社へご確認ください。",
  },
];
...
export default function AreaPageKyotoNakagyo() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "京都市中京区の鍵開け・鍵交換業者おすすめ6選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜京都市中京区で人気のおすすめ・優良の鍵業者6選",
      sub: [
        { href: "#hikaku", label: "京都市中京区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜中京区で鍵業者を呼ぶ前に確認したいこと（賃貸の町家は加工の可否を先に確認）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜中京区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">京都市中京区の鍵開け・鍵交換業者おすすめ6選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["京都市中京区", "町家の鍵", "引き戸", "賃貸の加工制限", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              京都市中京区は、通りに面して間口の狭い建物が連なる町並みが残り、木造の町家がいまも住まいや店舗として使われているエリアです。玄関が引き戸だったり、建具そのものが古かったりすると、防犯性の高い鍵に替えたいと思っても、現代の建物と同じようには進みません。とくに借りている町家では、工事の前に確認しておくべきことがあります。
            </p>
            <p>
              本記事では、中京区内から依頼できる主要な鍵業者6社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、賃貸の町家で鍵を替える前に貸主へ確認しておきたい加工の可否と原状回復の考え方、古い引き戸の錠をどう扱うかを整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">京都市中京区で人気のおすすめ・優良の鍵業者6選</H2>
          <p className="leading-loose mb-2">
            京都市中京区内から依頼できる主要な鍵業者6社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「京都府」としての一次確認値のため、中京区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">中京区の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            古い建具では、無理に開けようとして傷めるより、道具を持った人に任せたほうが結果的に早く済みます。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。関西を中心に展開する鍵猿も最短15分到着をうたっています。
          </p>
          ...
          <p className="leading-loose mb-4">
            観光地が近い区域では、夜遅くに帰宅してから鍵の不具合に気づくこともあります。一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。<Hl>鍵猿は電話受付が8:00〜23:00</Hl>で、施工は深夜・早朝も対応とされています。深夜に新規で電話するなら受付時間の違いに注意してください。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            古い建具に手を入れる作業は、部材の追加や加工の手間が読みにくく、見積もりの取り方が結果を左右します。<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。<Hl>鍵猿は基本料・見積料・出張料が0円</Hl>で、作業前に合計金額を提示し追加請求しない方式を掲げています。
          </p>
          <p className="leading-loose mb-4">
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。既存の建具を残したまま作業する場面では、事前の説明と当日の作業者が同じ会社でつながっているほうが行き違いが起きにくくなります。今回の紹介では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。関西エリア中心の<Hl>鍵猿も下請けを使わない自社施工</Hl>で、1年間の無料工事保証を掲げています。依頼時に「自社スタッフか協力店か」を確認してみましょう。
          </p>

          {/* 6｜区特有 */}
          <H2 id="tips" num="6">中京区で鍵業者を呼ぶ前に確認したいこと（賃貸の町家は「加工の可否」を先に確認）</H2>
          <p className="leading-loose mb-4">
            借りている町家で鍵を替えたいと思ったら、最初に確認するのは製品選びではなく<Hl>貸主が加工を認めるかどうか</Hl>です。現代の錠を古い建具に取り付ける場合、彫り込みや穴あけといった加工が必要になることが多く、これは建物に手を入れる行為にあたります。賃貸では原状回復の考え方があり、借主の判断だけで進めると退去時の精算で問題になりかねません。まず管理者や貸主へ、加工してよいか、費用は誰が負担するか、退去時に元へ戻す必要があるかの3点を確認してください。費用の分担の考え方は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用</Link>で整理しています。
          </p>
          <p className="leading-loose mb-4">
            建物の外観に関わる部分では、<Hl>地区や建物の種類によって別の手続きや制限が設けられている場合がある</Hl>点にも注意が必要です。町並みの保全を目的とした取り決めがある区域では、通りから見える部分の変更にあたって届け出が求められることがあります。該当するかどうかは建物ごとに異なるため、自分で判断せず、貸主や管理者を通して確認するのが確実です。玄関まわりに手を入れる工事は、こうした確認の対象になりやすいことを覚えておくと、後戻りを避けられます。
          </p>
          <p className="leading-loose mb-4">
            古い引き戸の錠そのものにも、現代の開き戸とは違う事情があります。<Hl>引き戸は召し合わせ部分に錠を設ける構造</Hl>で、開き戸のシリンダー交換のように部品だけを差し替えれば済むとは限りません。長く使われてきた製品では同じ部品がすでに作られておらず、同型での交換ができないこともあります。その場合は、形状の合う別の製品への交換か、既存の錠を残したまま別の錠を足す方法を検討することになります。引き戸の錠の種類と選び方は<Link href="/hikido-kagi/" className="text-primary underline">引き戸の鍵の解説</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            加工の許可が下りない、あるいは建具を傷めたくないという場合の現実的な選択肢が<Hl>補助錠</Hl>です。既存の錠はそのままに、別の錠を追加して開錠の手間を増やす考え方で、取り付け方によっては大きな加工を伴いません。ただし、固定の方法によって強度は変わり、両面テープ等で留めるタイプは強く引かれる力に対しては限界があります。どの程度の効果を見込むかは製品と取り付け方次第なので、選ぶ前に<Link href="/hojo-jou/" className="text-primary underline">補助錠の選び方</Link>を確認し、賃貸の場合は補助錠の設置についても貸主へ一報を入れておくと安心です。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">中京区の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。古い建具が絡む工事は、確認の抜けがそのまま追加費用につながります。
          </p>
          {steps.map((s) => (
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">中京区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            京都市中京区で鍵開け・鍵交換を頼める主要6社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキューや鍵猿、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            町家や古い建具が残る中京区では、鍵を替えるという判断の前に、加工してよいかという確認が挟まります。貸主・管理者への確認、外観に関わる部分の取り決め、そして引き戸ならではの構造。この3点を押さえてから業者へ相談すれば、見積もりの精度も上がり、やり直しも避けられます。そのうえで、本記事の一次確認値を出発点に、目的と予算に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["京都市中京区", "町家", "引き戸の錠", "賃貸の原状回復", "鍵交換", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
