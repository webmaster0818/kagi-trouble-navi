...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】荒川区の鍵開け・鍵交換業者おすすめ5選！古い賃貸の鍵の不調と大家さんへの相談順も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "荒川区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、古い賃貸アパートで鍵が回りにくいときの応急対処と大家さん・管理会社への相談の順番まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】荒川区の鍵開け・鍵交換業者おすすめ5選！古い賃貸の鍵の不調と大家さんへの相談順も分かる鍵屋比較",
    description:
      "荒川区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。古いアパートの鍵の不調の切り分け、賃貸で守るべき相談の順番、経年劣化時の費用負担の考え方も解説しています。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、荒川区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。荒川区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、荒川区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "荒川区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜や早朝でも荒川区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、荒川区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "古いアパートで鍵が回りにくいです。自分で鍵業者を呼んで直していいですか？",
    a: "先に大家さん・管理会社へ連絡するのが賃貸の基本です。通常の使用による経年劣化の不調は貸主側の負担で修繕されるのが一般的な整理で、自己判断で業者を呼んで交換すると、費用を負担することになったり退去時に揉めたりする原因になります。締め出しなどの緊急時で連絡がつかない場合は、24時間受付の鍵業者に依頼したうえで、事後でも必ず管理会社へ報告しましょう。",
    link: { href: "/kagi-mawaranai/", label: "鍵が回らないときの対処はこちら" },
  },
  {
    q: "経年劣化で鍵が壊れた場合、交換費用は入居者と大家のどちらの負担ですか？",
    a: "通常の使用による経年劣化・自然損耗の修繕は貸主（大家さん）負担とするのが一般的な整理です。一方、鍵の紛失や入居者の過失による破損は入居者負担となるのが通例です。ただし実際の負担区分は賃貸借契約の定めによるため、まず契約書を確認し、管理会社に相談してから進めましょう。",
...
export default function AreaArakawaKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】荒川区の鍵開け・鍵交換業者おすすめ5選！古い賃貸の鍵の不調と大家さんへの相談順も分かる鍵屋比較",
    description:
      "荒川区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "荒川区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜荒川区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "荒川区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜荒川区で鍵業者を呼ぶ前に確認したいこと（古い賃貸の鍵の不調と相談の順番）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜荒川区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">荒川区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「最近、鍵の回りが渋い。古いアパートだから仕方ないのだろうか」——昔ながらの住宅が残る荒川区では、築年数を重ねた賃貸アパートの鍵の不調に関する悩みが典型です。賃貸の鍵は自己判断で交換すると揉め事のもとになるため、「誰に・どの順番で相談するか」を知っておくことが何より大切です。
            </p>
            <p>
              本記事では、荒川区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、古い賃貸で鍵の不調が出たときの相談の順番と、応急対処の注意点もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">荒川区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            荒川区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、荒川区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">荒川区の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            荒川区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">荒川区で鍵業者を呼ぶ前に確認したいこと（古い賃貸の鍵の不調と相談の順番）</H2>
          <p className="leading-loose mb-4">
            築年数を重ねたアパートで多いのは、「鍵が回りにくい」「抜けにくい」「引っかかる」という不調です。原因は鍵穴内部の汚れや鍵山の摩耗だけでなく、<Hl>建物の経年で扉が歪み、デッドボルトと受け座の位置がずれているケース</Hl>もあります。無理に回すと鍵が折れる恐れがあるため、まず症状を切り分けましょう（<Link href="/kagi-mawaranai/" className="text-primary underline">鍵が回らないときの対処</Link>・<Link href="/kagi-nukenai/" className="text-primary underline">鍵が抜けないときの対処</Link>）。応急の潤滑には鍵穴専用のパウダータイプを使い、<Hl>食用油や一般の機械油は内部で固まって症状を悪化させるためNG</Hl>です。
          </p>
          <p className="leading-loose mb-4">
            賃貸で何より大切なのは<Hl>相談の順番</Hl>です。①不調に気づいたら、まず大家さん・管理会社へ連絡する——通常の使用による経年劣化の修繕は貸主負担で行われるのが一般的な整理です（実際の負担区分は契約によります）。②自己判断で業者を呼んで交換すると、費用を自分で負担することになったり、退去時の原状回復で揉めたりする原因になります。③締め出しなど緊急で連絡がつかない場合は24時間受付の鍵業者に依頼し、<Hl>事後でも必ず管理会社へ報告</Hl>しましょう。
          </p>
          <p className="leading-loose mb-4">
            交換になった場合の費用感と負担区分の考え方は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用</Link>、鍵の寿命の一般的な目安は<Link href="/kagi-jumyou/" className="text-primary underline">鍵の寿命と交換時期</Link>で解説しています。古い建物で防犯面が不安な場合も、賃貸では扉に穴を開ける工事を勝手に行うことはできません。<Hl>原状回復できる範囲の対策から始める</Hl>のが賃貸の鉄則です（<Link href="/chintai-bouhan/" className="text-primary underline">賃貸でもできる防犯対策</Link>）。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">荒川区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">荒川区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            荒川区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            古い賃貸の鍵の不調は、「まず大家さん・管理会社へ連絡」が鉄則です。経年劣化の修繕は貸主負担が一般的な整理ですが、負担区分は契約次第。自己判断で交換して揉める前に、相談の順番を守りましょう。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["荒川区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-3">
              {[
                ...
                { href: "/area/toshima-ku/", label: "荒川区の鍵開け・鍵交換業者おすすめ5選" },
