...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】津の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "津で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、車通勤でのインロックを防ぐ降車時の習慣とスペアキーの持ち方まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】津の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "津で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。インロックを起こさない降車時のルーティンづくりと、スペアキーをどこに置くかの考え方も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、津市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「家の鍵か車の鍵か」「開かないのか、なくしたのか」を整理し、車の場合は車種と鍵のタイプ（スマートキーか金属キーか）をメモしておくと、電話での見積もり精度が上がります。車のインロックは、鍵業者のほかに自動車保険付帯のロードサービスやJAFという選択肢もあるため、加入状況を先に確認しておくと判断が早くなります。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。スペアキーの作製や自宅の鍵交換のように日程が読める依頼なら、じっくり比較する価値があります。今回の5社はいずれも全国対応型で津から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];
...
const faqs: AreaFaq[] = [
  ...
  {
    q: "スマートキーの車でもインロックは起きますか？",
    a: "起こり得ます。スマートキーの電池が消耗していたり、電波の状況によって車が鍵を検知できなかったりすると、鍵が車内にあるのに施錠されてしまうことがあります。電池残量の警告を放置しない・定期的に電池を交換する・鍵は必ず身に着けて降りる、という習慣で多くは防げます。",
    link: { href: "/smart-key-denchi/", label: "スマートキーの電池切れの解説はこちら" },
  },
  {
    q: "スペアキーを車内に置いておくのはだめですか？",
    a: "おすすめできません。インロック時は車内のスペアキーには手が届かず備えとして機能しないうえ、車上荒らしに遭った場合は車ごと盗まれるリスクを高めます。スペアキーは自宅の決まった場所に保管し、家族と場所を共有しておくのが基本です。スマートキーに内蔵されたメカニカルキーの取り出し方と使い方も、平時に一度確認しておきましょう。",
    link: { href: "/kuruma-spare-key/", label: "車のスペアキーの解説はこちら" },
  },
  {
    q: "通勤先の駐車場でインロックしてしまいました。何から確認すべきですか？",
    a: "まず家族がスペアキーを届けられるかを確認し、難しければ加入している自動車保険のロードサービスやJAF、鍵業者の順に検討します。それぞれ到着目安と費用（会員条件・無料範囲）を電話で確認してから依頼しましょう。子どもやペットが車内に残っている場合は、ためらわず救助を要請してください。",
...
export default function AreaTsuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】津の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "津で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "津の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#tsu5",
      label: "1｜津で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "津の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tsu-tips", label: "6｜津で鍵業者を呼ぶ前に確認したいこと（車通勤のインロック予防とスペアキーの持ち方）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜津の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">津の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「エンジンを切って荷物を取ろうとした一瞬のすきに、鍵を車内に残したままドアがロックされてしまった」——車のインロックは、慌ただしい乗り降りのふとした拍子に起こります。車での通勤や送迎が暮らしの前提になっている津では、インロックの影響がその日の予定に直結するため、「起こさない習慣」と「起きてもすぐ動ける備え」の両方を用意しておく価値があります。
            </p>
            <p>
              本記事では、津市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、インロックを防ぐ降車時の習慣づくりと、スペアキーの持ち方もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="tsu5" num="1">津で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            津市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/nagoya/" className="text-primary underline">名古屋版</Link>・<Link href="/area/gifu/" className="text-primary underline">岐阜版</Link>・<Link href="/area/hamamatsu/" className="text-primary underline">浜松版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">津の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            津でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜津特有 */}
          <H2 id="tsu-tips" num="6">津で鍵業者を呼ぶ前に確認したいこと（車通勤のインロック予防とスペアキーの持ち方）</H2>
          <p className="leading-loose mb-4">
            インロック予防の核心は、<Hl>「鍵を身に着けてからドアを閉める」動作を毎回同じ順番で行う</Hl>ことです。エンジン停止、鍵をポケットやかばんの定位置へ、それからドアを閉める——この順番を崩さないだけで、荷物の積み下ろしや送迎のあわただしい場面でも鍵を車内に残しにくくなります。スマートキーは「持っていれば施錠されないはず」と過信しがちですが、電池の消耗や電波状況によっては検知されないことがあります。<Link href="/smart-key-denchi/" className="text-primary underline">スマートキーの電池切れの解説記事</Link>で仕組みを押さえておきましょう。
          </p>
          <p className="leading-loose mb-4">
            スペアキーの置き場所は「車内以外」が大前提です。<Hl>自宅の決まった場所に保管し、家族と場所を共有しておく</Hl>ことで、通勤先でのインロックでも家族に届けてもらう選択肢が生まれます。長距離の外出や旅行のときだけ、同行者がもう1本を別のかばんで持つ方法も有効です。また、スマートキーに内蔵されたメカニカルキー（物理キー）の取り出し方は車種ごとに違うため、平時に一度試しておくと電池切れの際に慌てません。詳しくは<Link href="/kuruma-spare-key/" className="text-primary underline">車のスペアキーの解説記事</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            それでもインロックが起きたときは、<Hl>家族のスペア搬送→自動車保険付帯のロードサービスやJAF→鍵業者</Hl>という順で検討すると、費用と時間のバランスを取りやすくなります。それぞれ到着目安と費用の条件（会員か否か・無料範囲）が異なるため、電話で確認してから依頼しましょう。子どもやペットが車内に残っている・炎天下であるといった場合は、費用の比較よりも安全を優先し、ためらわず救助を要請してください。手順の全体像は<Link href="/car-inlock/" className="text-primary underline">車のインロックの対処記事</Link>で説明しています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">津の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">津の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            津で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            車通勤のインロック対策は、降車時の順番を固定する・スペアキーを車内に置かず自宅で家族と共有する・起きたときの連絡順（スペア搬送→ロードサービス→業者）を決めておく、という3点に集約できます。スマートキーでも電池切れなどで起こり得るため、電池交換とメカニカルキーの確認も習慣にしましょう。急がない依頼は複数社から総額の見積もりを取って比較することが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["津", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
