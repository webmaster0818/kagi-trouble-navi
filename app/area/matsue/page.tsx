...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】松江の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "松江で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、湿気による鍵・建具の渋りの切り分けと日常の手入れまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】松江の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "松江で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。湿気の多い時期に起こる鍵・建具の渋りの原因切り分けと、正しい手入れの方法も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、松江市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「回りにくい」「抜けにくい」「掛かりが浅い」など症状と、開き戸か引き戸か・鍵の種類をメモしておくと、電話での見積もり精度が上がります。戸を持ち上げると掛かるかどうかなど、建て付け由来か錠由来かの手がかりを伝えられるとさらにスムーズです。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。鍵の交換や合鍵の追加のように日程が読める依頼なら、じっくり比較する価値があります。今回の5社はいずれも全国対応型で松江から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "松江の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "湿気の多い時期だけ鍵の掛かりが渋くなります。故障ですか？",
    a: "建具が湿気を含んで膨らみ、錠の噛み合わせが季節的にずれている可能性があります。戸を少し持ち上げる・押すと掛かるなら建て付け側、戸の位置に関係なく渋いなら錠側が疑われます。力任せに回すと故障につながるため、症状が続くなら業者に切り分けを相談しましょう。",
    link: { href: "/kagi-mawaranai/", label: "鍵が回らないときの対処はこちら" },
  },
  {
    q: "渋い鍵穴に油を差してもよいですか？",
    a: "食用油やミシン油、汎用の潤滑スプレーは使わないでください。油分がほこりを呼び込み、内部で固まって固着を悪化させます。使ってよいのは鍵穴専用のパウダータイプ潤滑剤か、鍵の側面に鉛筆の黒鉛を塗る方法です。すでに油を差して動きが悪くなった場合は、業者に洗浄や交換を相談しましょう。",
  },
  {
    q: "引き戸の鍵の交換も対応してもらえますか？",
    a: "引き戸錠も多くの鍵業者で対応対象です。ただし開き戸のシリンダー錠と構造が異なり、戸の厚みや召し合わせ部分の形状で選べる製品が変わるため、依頼時に「引き戸である」ことと戸の種類を伝えると見積もりがスムーズです。",
...
export default function AreaMatsuePage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】松江の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "松江で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "松江の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#matsue5",
      label: "1｜松江で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "松江の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#matsue-tips", label: "6｜松江で鍵業者を呼ぶ前に確認したいこと（湿気による鍵・建具の渋りと日常の手入れ）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜松江の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">松江の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「湿気の多い時期になると、玄関の鍵の掛かりが渋くなる」「引き戸が重くなって、錠がうまく合わなくなる」——こうした症状は、鍵そのものの故障とは限りません。木製の建具や引き戸は湿気を含むとわずかに膨らみ、錠の噛み合わせがずれて渋りとして現れることがあります。松江の住まいで起きやすいこの症状について、原因の切り分け方と正しい手入れを整理します。
            </p>
            <p>
              本記事では、松江市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、湿気による鍵・建具の渋りの切り分けと日常の手入れもまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="matsue5" num="1">松江で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            松江市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/tottori/" className="text-primary underline">鳥取版</Link>・<Link href="/area/hiroshima/" className="text-primary underline">広島版</Link>・<Link href="/area/okayama/" className="text-primary underline">岡山版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">松江の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            松江でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜松江特有 */}
          <H2 id="matsue-tips" num="6">松江で鍵業者を呼ぶ前に確認したいこと（湿気による鍵・建具の渋りと日常の手入れ）</H2>
          <p className="leading-loose mb-4">
            渋りを感じたら、まず<Hl>「錠そのものの不調」か「建て付けのずれ」かを切り分ける</Hl>のが第一歩です。ドアや戸を少し持ち上げたり押し付けたりした状態なら錠が掛かる場合は、建具側のずれや膨らみが疑われます。逆に、戸の位置に関係なく鍵の抜き差しや回転が渋いなら、鍵穴内部の汚れや摩耗が原因のことが多くなります。引き戸の錠は開き戸と構造が異なるため、症状の見方は<Link href="/hikido-kagi/" className="text-primary underline">引き戸の鍵の解説記事</Link>も参考にしてください。
          </p>
          <p className="leading-loose mb-4">
            手入れの基本は、<Hl>鍵穴には鍵穴専用の潤滑剤（パウダータイプ）か鉛筆の黒鉛を使い、食用油や汎用の潤滑スプレーは使わない</Hl>ことです。油分はほこりを呼び込み、内部で固まってかえって固着を悪化させます。鍵本体は乾いた布で拭き、歯ブラシで溝の汚れを落とす程度で十分です。「回らない」「抜けない」が出たときの対処は<Link href="/kagi-mawaranai/" className="text-primary underline">鍵が回らないときの解説記事</Link>と<Link href="/kagi-nukenai/" className="text-primary underline">鍵が抜けないときの解説記事</Link>で詳しく説明しています。
          </p>
          <p className="leading-loose mb-4">
            正しい手入れをしても渋りが戻ってくる場合は、部品の摩耗が進んでいるサインかもしれません。<Hl>一般に錠前の耐用年数は10年程度が目安</Hl>とされ、症状が繰り返す古いシリンダーは、修理よりも交換のほうが結果的に安上がりなことがあります。寿命の見極めは<Link href="/kagi-jumyou/" className="text-primary underline">鍵の寿命の解説記事</Link>を、交換を決めるときの考え方は<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>をご覧ください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">松江の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">松江の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            松江で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            湿気の多い時期の鍵や建具の渋りは、建て付け由来か錠由来かを切り分けたうえで、鍵穴専用潤滑剤や鉛筆の黒鉛といった正しい方法で手入れするのが基本です。食用油や汎用の潤滑スプレーは固着を悪化させるため使わないでください。手入れをしても繰り返す渋りは寿命のサインなので、交換も含めて複数社から総額の見積もりを取って比較することが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["松江", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
