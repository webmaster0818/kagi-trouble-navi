...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】港区の鍵開け・鍵交換業者おすすめ5選！オフィスの鍵とセキュリティカードの紛失時の連絡順も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "港区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、オフィスの鍵・セキュリティカードをなくしたときの連絡順とビル管理会社との管轄の切り分けまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】港区の鍵開け・鍵交換業者おすすめ5選！オフィスの鍵とセキュリティカードの紛失時の連絡順も分かる鍵屋比較",
    description:
      "港区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。オフィスビルの共用部・専有部の管轄の切り分けと、物理鍵とセキュリティカードを併用する職場での紛失時の連絡順も整理しています。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、港区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。港区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、港区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "港区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜や早朝でも港区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、港区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "会社で借りているオフィスの鍵をなくしました。まず誰に連絡すべきですか？",
    a: "個人の判断で鍵業者を呼ぶ前に、まず上長・総務など社内の管理部門へ報告しましょう。オフィスの鍵は会社の管理物であり、錠の交換にはビル管理会社への届け出や指定業者の利用が必要な場合もあります。セキュリティカードを併用している場合は、発行元（ビル管理会社・セキュリティ会社）への利用停止連絡を最優先に。対応の速さ以上に、連絡の順番を守ることがトラブルを小さくします。",
    link: { href: "/tenpo-office-kagi-kanri/", label: "店舗・オフィスの鍵管理はこちら" },
  },
  {
    q: "オフィスの入口を電子錠やスマートロックにしたいのですが、テナントの判断で工事できますか？",
    a: "賃貸オフィスの場合、扉や錠まわりの工事は賃貸借契約やビル管理規則で事前承認・指定業者が定められていることが多く、テナントの独断で進めるのは避けるべきです。まずビル管理会社（または貸主）へ相談し、承認の要否・原状回復の条件を確認しましょう。扉に穴を開けない貼り付け型の機器でも、外観や共用部の運用に関わる場合は事前に確認しておくと安心です。",
...
export default function AreaMinatoKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】港区の鍵開け・鍵交換業者おすすめ5選！オフィスの鍵とセキュリティカードの紛失時の連絡順も分かる鍵屋比較",
    description:
      "港区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "港区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜港区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "港区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜港区で鍵業者を呼ぶ前に確認したいこと（オフィスの鍵・セキュリティカードの管理と連絡順）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜港区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">港区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「オフィスの鍵とセキュリティカード、どちらを先に止めればいいのか分からない」——オフィスビルが集積する港区では、法人・職場がらみの鍵トラブルの相談先に迷うケースが目立ちます。オフィスの鍵は自宅と違い、ビル管理会社・総務・セキュリティ会社と関係者が多く、連絡の順番を間違えると対応が遅れがちです。
            </p>
            <p>
              本記事では、港区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、オフィスの鍵・セキュリティカードの管理の基本と、なくしたときに連絡すべき順番も整理しています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">港区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            港区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、港区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">港区の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            港区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">港区で鍵業者を呼ぶ前に確認したいこと（オフィスの鍵・セキュリティカードの管理と連絡順）</H2>
          <p className="leading-loose mb-4">
            オフィスの鍵トラブルで最初に必要なのは、<Hl>「どこまでが自分たちの管轄か」の切り分け</Hl>です。エントランスの自動ドアや共用部のカードリーダーなど建物側の設備はビル管理会社の管轄で、テナントが個別に鍵業者を呼ぶ場面ではありません。一方、自社が借りている専有部の扉の錠は借主側で手配するのが一般的です。ただし、<Hl>賃貸借契約やビル管理規則で指定業者や事前届け出が定められていることがある</Hl>ため、交換・増設の前にビル管理会社へ確認するのが安全です。
          </p>
          <p className="leading-loose mb-4">
            物理鍵とセキュリティカードを併用する職場では、<Hl>なくした物によって連絡の順番が変わります</Hl>。カードをなくしたら、発行元（ビル管理会社やセキュリティ会社）への利用停止連絡が最優先です。多くの入退室システムは該当カードだけを無効化できるため、連絡が早いほどリスクを小さくできます。物理鍵をなくしたら、上長・総務へ報告し、本数管理の記録を更新したうえで、錠交換の要否を会社として判断します。<Hl>個人の判断で先に鍵業者を呼ぶのは避けましょう</Hl>。
          </p>
          <p className="leading-loose mb-4">
            日常の管理では、鍵・カードの持ち出しの記録、退職・異動時の確実な回収、マスターキーの厳重保管が基本です。仕組みづくりは<Link href="/tenpo-office-kagi-kanri/" className="text-primary underline">店舗・オフィスの鍵管理</Link>、本数管理の実務は<Link href="/key-box-kanri/" className="text-primary underline">キーボックスでの鍵管理</Link>で詳しく解説しています。通勤中に自宅の鍵をなくした場合の動き方は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>を参考にしてください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">港区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">港区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            港区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            オフィスの鍵トラブルは、共用部はビル管理会社・専有部は借主側という管轄の切り分けと、カードは発行元へ利用停止・物理鍵は社内へ報告という連絡の順番を押さえておけば、慌てずに対処できます。錠の工事や交換はビル管理会社への確認が先です。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["港区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-3">
              {[
                ...
                { href: "/area/toshima-ku/", label: "港区の鍵開け・鍵交換業者おすすめ5選" },
