...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】福岡市中央区の鍵開け・鍵交換業者おすすめ5選！天神周辺での鍵紛失と交通機関・店舗への問い合わせ順も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "福岡市中央区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、天神周辺で鍵をなくしたときに店舗・タクシー・鉄道のどこから問い合わせるかの順番と、回答を待つあいだの動き方まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】福岡市中央区の鍵開け・鍵交換業者おすすめ5選！天神周辺での鍵紛失と交通機関・店舗への問い合わせ順も分かる鍵屋比較",
    description:
      "福岡市中央区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。天神周辺で鍵をなくしたときの問い合わせ先の並べ方と、回答を待つあいだにどう動くかも解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（福岡県対応）", note: "福岡県対応の一次確認値。中央区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「なくした」のか「開かない」のかを整理し、なくした場合は最後に鍵を使った場所と、その後に立ち寄った先を書き出してから電話しましょう。賃貸なら管理会社・大家、分譲マンションなら管理員や管理会社の緊急窓口への連絡が先です。合鍵の預かりがあれば費用をかけずに解決できることがあります。玄関の錠の種類が分かれば、電話での見積もり精度が上がります。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、中央区からも依頼できます（対応エリアは「福岡県」としての一次確認値のため、詳細条件は各社へ確認を）。鍵が見つかる可能性が残っている段階では、解錠だけで止める選択肢も含めて金額を比べ、納得できなければその場で断りましょう。",
  },
];
...
const faqs: AreaFaq[] = [
  {
    ...
    a: "当サイトが一次確認した各社の公式表示では、鍵開けの基本料金はカギ110番6,600円〜、鍵のレスキュー8,000円〜、鍵の生活救急車・JBR8,800円〜（いずれも税込）、カギの救急車は公式目安11,000〜44,000円です。ただしこれは最低価格や目安で、鍵の種類（ディンプルキー等）・時間帯・出張距離で総額は変わります。解錠だけで止めるか交換まで進めるかでも金額が変わるため、作業前に両方の総額を確認しておくと判断しやすくなります。",
    link: { href: "/ryokin/", label: "料金相場の詳細はこちら" },
  },
  {
    q: "どの交通機関に問い合わせればいいか分かりません。記録が残っていないときは？",
    a: "交通系ICカードや決済アプリの利用履歴をたどると、乗降した駅や利用した店舗が分かります。現金で支払っていて記録がない場合は、移動した時刻と経路を書き出し、通った可能性のある事業者へ順に問い合わせてください。鉄道やバスは拾得物が駅・営業所から集約先へ移るまでに時間がかかることがあり、遅い時間の問い合わせでは翌営業日の回答になることもあります。営業中の店舗のように即答が期待できる先から当たると、無駄が減ります。",
    link: { href: "/kagi-funshitsu/", label: "鍵をなくしたときの対処はこちら" },
  },
  {
    q: "タクシーに忘れた可能性があります。会社が分からないときはどうしますか？",
    a: "領収書があれば会社名と車両番号が記載されているため、そこから直接問い合わせられます。領収書がない場合でも、決済アプリやクレジットカードの利用履歴に事業者名が残っていることがあります。それも見当たらないときは、乗車した時刻・乗った場所・降りた場所を整理したうえで、地域のタクシー事業者や協会の窓口に相談するのが現実的です。いずれの場合も、乗務員への確認が入るため回答まで時間差が生じます。",
  },
  {
    q: "今夜中に返事が来そうにありません。先に開けてもらってもいいですか？",
    a: "自宅に入れない状態のまま屋外で待つより、まず入れる状態を作るほうが安全です。賃貸なら管理会社・大家、分譲マンションなら管理員や管理会社の緊急窓口が最初の相談先で、合鍵の預かりがあれば鍵業者を呼ばずに済むことがあります。解錠だけで止めて家に入り、探す作業を翌日以降も続ける進め方であれば、鍵が見つかった場合に交換を取りやめる余地も残せます。ただし住所が分かるものと一緒に失った場合は、交換を前提に考えてください。",
    link: { href: "/kagi-koukan-timing/", label: "鍵交換のタイミングはこちら" },
  },
  {
    q: "深夜や早朝でも中央区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「福岡県」としての一次確認値のため、中央区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "業者を選ぶときに見るべきポイントはどこですか？",
    a: "広告の最低価格だけで選ばず、作業前に出張費・部材費まで含めた総額の見積もりを提示してもらってください。解錠のみと交換まで行った場合の両方の金額を聞いておくと、後から方針を変えるときに慌てずに済みます。承諾のない追加請求がない方針かどうか、運営会社はどこかという2点も確認しておきましょう。鍵業界には名前のよく似た別会社のサービスが複数あるため、サービス名だけでは取り違えが起こります。",
  },
];
...
export default function AreaFukuokaChuoPage() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "福岡市中央区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜福岡市中央区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "福岡市中央区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜中央区で鍵業者を呼ぶ前に確認したいこと（問い合わせ先は回答が返る速さで並べる）" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">福岡市中央区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              天神を抱える福岡市中央区は、短い時間のうちに複数の店舗と交通機関を使う人の多いエリアです。そのぶん、鍵をなくしたときに心当たりが同時に何か所も出てきます。手当たり次第に連絡すると時間だけが過ぎ、深夜になるほど「今日は誰にも聞けない」という状態に近づいていきます。実は問い合わせ先には、その場で回答が返るものと、翌営業日まで待つことになるものがあり、この違いを知っているだけで動き方が変わります。
            </p>
            <p>
              福岡市中央区内から依頼できる主要な鍵業者5社について、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけを使って比較したのが本記事です。あわせて、問い合わせ先を回答の速さで並べる考え方、手元の記録から連絡先を特定する方法、返事を待つあいだに自宅へ入れる状態をどう作るかを整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">福岡市中央区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            福岡市中央区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「福岡県」としての一次確認値のため、中央区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          <p className="leading-loose mb-4">
            問い合わせの結果を待つ前に、まず自宅へ入れる状態を作りたい場面もあります。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            紛失に気づくのが遅い時間になるほど、問い合わせ先の多くは翌営業日回しになります。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            鍵が見つかるかどうか分からない段階では、解錠で止めるか交換まで進めるかで総額が大きく変わります。料金の透明性を重視するなら、<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。夜間にひとりで到着を待つ場面では、誰が来るのかが分かっているほうが落ち着いて待てます。今回の5社では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。依頼時に「自社スタッフか協力店か」を確認してみましょう。
          </p>

          {/* 6｜区特有 */}
          <H2 id="tips" num="6">中央区で鍵業者を呼ぶ前に確認したいこと（問い合わせ先は「回答が返る速さ」で並べる）</H2>
          <p className="leading-loose mb-4">
            天神周辺のように、短時間で複数の店舗や交通機関を使う場所で鍵をなくすと、心当たりが同時に何か所も出てきます。手当たり次第に連絡すると時間だけが過ぎるため、<Hl>「その場で回答が返ってくる先」から順に当たる</Hl>と考えると整理できます。目安として、営業中の店舗は即答が期待できます。タクシー会社は乗務員に確認が入るぶん時間差が生じます。鉄道やバスは、拾得物が駅・営業所から集約先に移るまでに時間がかかることがあり、遅い時間の問い合わせでは翌営業日の回答になることも珍しくありません。
          </p>
          <p className="leading-loose mb-4">
            問い合わせ先の特定には、<Hl>手元に残る記録</Hl>が役立ちます。交通系ICカードや決済アプリの利用履歴からは乗降した駅や利用した店舗が分かり、タクシーであれば領収書に会社名と車両番号が記載されています。領収書を受け取っていない場合でも、決済履歴に事業者名が残っていることがあります。<Hl>記録をたどって連絡先を確定させてから電話する</Hl>ほうが、記憶だけで探すより早く、伝えられる情報も正確になります。届け出のときは、本数やキーホルダーの色・形といった特徴も添えましょう。
          </p>
          <p className="leading-loose mb-4">
            回答を待つあいだに決めておきたいのが、<Hl>今夜どこで過ごすか</Hl>です。「明日には見つかるかもしれない」という見込みで屋外に留まるのは避け、まず自宅に入れる状態を作ることを優先してください。賃貸なら管理会社・大家、分譲マンションなら管理員や管理会社の緊急窓口が最初の相談先で、合鍵の預かりがあれば鍵業者を呼ばずに済むことがあります。<Hl>解錠して家に入ったうえで、探す作業を翌日以降も続ける</Hl>という進め方であれば、見つかった場合に交換を取りやめる余地も残せます。
          </p>
          <p className="leading-loose mb-4">
            一方で、<Hl>すぐに交換まで踏み切るべきケース</Hl>もあります。目安は、失われた鍵から住所にたどり着けるかどうかです。免許証や社員証、宛名入りの書類と同じケースに入れていた場合は、鍵と住所がセットで他人の手に渡った可能性を考える必要があります。また、鍵に刻印された番号から合鍵を作れてしまう仕組みがあるため、<Hl>特徴を伝えるときも、番号や写真を不特定の相手に見せない</Hl>よう注意してください。判断の目安は<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>、仕組みは<Link href="/key-number-aikagi/" className="text-primary underline">鍵番号と合鍵の解説</Link>で扱っています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">中央区の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。探し物と業者選びを同時に進める場面ほど焦りやすいので、確認する順番を決めておきましょう。
          </p>
          {steps.map((s) => (
          ...
          <p className="leading-loose mb-4">
            福岡市中央区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            天神周辺で鍵をなくしたときは、心当たりの多さそのものが問題になります。営業中の店舗のように即答が返る先から順に当たり、記録をたどって連絡先を確定させてから電話する。返事を待つあいだは屋外に留まらず、まず自宅へ入れる状態を作る。この三つの順番を決めておけば、深夜でも判断に迷いません。段取りを固めたうえで、本記事の一次確認値を出発点に業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["福岡市中央区", "天神", "鍵紛失", "遺失物窓口", "鍵交換", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
