...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】江戸川区の鍵開け・鍵交換業者おすすめ5選！自転車移動派の紛失対策も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "江戸川区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、自転車移動中の鍵の持ち歩き方と落とした後の届け出の動線まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】江戸川区の鍵開け・鍵交換業者おすすめ5選！自転車移動派の紛失対策も分かる鍵屋比較",
    description:
      "江戸川区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。自転車移動中に鍵を落とさない持ち歩き方と、落とした後の届け出の動線も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、江戸川区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。江戸川区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「走行中に落としたらしい」「家に入れない」など状況と、その日に走ったルート・立ち寄った場所をメモしておくと、電話での見積もり精度が上がるうえ、捜索や交番への届け出にも役立ちます。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、江戸川区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "江戸川区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜や早朝でも江戸川区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、江戸川区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "自転車で走行中に鍵を落としたようです。どう探せばよいですか？",
    a: "走ったルートを同じ道順でゆっくり引き返し、立ち寄った店の駐輪場・レジまわり・自転車のかごも確認します。見つからなければ最寄りの交番・警察署に遺失届を出しましょう。道路や店先で拾われた鍵は警察に届けられることが多く、届け出ておけば連絡を受けられます。",
    link: { href: "/kagi-funshitsu/", label: "鍵をなくしたときの対処はこちら" },
  },
  {
    q: "落とした場所が分からなくても遺失届は出せますか？",
    a: "出せます。遺失届は落とした場所が特定できなくても、最寄りの交番・警察署で提出できます。「いつ頃・どのあたりで持っていたのが最後か」「鍵の本数やキーホルダーの特徴」を伝えると照会しやすくなります。日を置いてから届くこともあるため、届け出後もあきらめずに待ちましょう。",
  },
  {
    q: "家の鍵と自転車の鍵はまとめて持つべきですか？",
    a: "ひとつにまとめると管理は楽ですが、落とすと家にも入れず自転車も動かせない状態になります。まとめる場合はファスナー付きポケットやキーチェーンで落下対策を徹底し、不安がある場合は分けて持つのがおすすめです。どちらの場合も、住所の分かる物と同じ場所に入れないことが大切です。",
  },
  {
...
export default function AreaEdogawaKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】江戸川区の鍵開け・鍵交換業者おすすめ5選！自転車移動派の紛失対策も分かる鍵屋比較",
    description:
      "江戸川区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "江戸川区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜江戸川区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "江戸川区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜江戸川区で鍵業者を呼ぶ前に確認したいこと（自転車移動と鍵の持ち歩き）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜江戸川区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">江戸川区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <header className="grid gap-4">
            ...
            <ul className="flex flex-wrap gap-2">
              {["江戸川区", "鍵開け", "鍵交換", "24時間対応", "業者比較"].map((t) => (
                <li
                  key={t}
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「自転車で買い物から帰ったら、ポケットに入れたはずの家の鍵がない」——駅までの移動や買い物に自転車を使う人が多い江戸川区では、走行中の振動でポケットから鍵が落ちるといった、自転車移動ならではの紛失が起こりがちです。落とさない持ち歩き方と、落としてしまった後の捜索・届け出の動線を整理します。
            </p>
            <p>
              本記事では、江戸川区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、自転車移動が中心の人の鍵の持ち歩き方と、落とした後の動き方もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">江戸川区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            江戸川区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、江戸川区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">江戸川区の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            江戸川区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">江戸川区で鍵業者を呼ぶ前に確認したいこと（自転車移動中の紛失対策と届け出の動線）</H2>
          <p className="leading-loose mb-4">
            自転車移動での紛失の典型は、<Hl>浅いポケットに入れた鍵が走行中の振動で落ちる</Hl>パターンです。走るときはファスナー付きのポケットかかばんに入れ、かばんの中では定位置を決めてキーチェーンでつなぐのが基本です。また、家の鍵と自転車の鍵をひとつのキーホルダーにまとめている場合、落とすと両方を同時に失います。まとめるなら落下対策を徹底し、不安なら分けて持ちましょう。自転車の鍵自体のトラブルは<Link href="/jitensha-kagi/" className="text-primary underline">自転車の鍵の解説</Link>で扱っています。
          </p>
          <p className="leading-loose mb-4">
            落としたことに気づいたら、<Hl>走ったルートを同じ道順でゆっくり引き返して探す</Hl>のが第一歩です。立ち寄った店の駐輪場・レジまわり・かごの中も確認しましょう。見つからなければ最寄りの交番・警察署へ遺失届を提出します。<Hl>道路や店先で拾われた鍵は警察に届けられることが多く、遺失届を出しておけば照会・連絡を受けられます</Hl>。落とした場所が分からなくても届け出は可能です。基本の手順は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            家に入れない場合は、同居家族・合鍵→賃貸なら管理会社・大家→鍵業者、の順で当たります（<Link href="/shimedashi/" className="text-primary underline">締め出されたときの対処</Link>）。鍵が戻らないときの交換判断は、<Hl>住所の分かる物と一緒に落としたかどうか</Hl>が大きな目安です。日頃の備えとしては、家族と<Link href="/aikagi/" className="text-primary underline">合鍵</Link>を共有しておくと、紛失時にも落ち着いて動けます。交換の考え方は<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>で解説しています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">江戸川区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">江戸川区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            江戸川区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            自転車移動での鍵の紛失は、ファスナー付きポケットやキーチェーンといった持ち歩き方の工夫でほとんど防げます。落としてしまったら、ルートを引き返して捜索→立ち寄り先への確認→最寄りの交番・警察署へ遺失届→帰宅手段の確保、の順で動きましょう。住所の分かる物と一緒に落とした場合は鍵交換までを視野に入れてください。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["江戸川区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-2">
              {[
                ...
                { href: "/jitensha-kagi/", label: "自転車の鍵のトラブル対処" },
