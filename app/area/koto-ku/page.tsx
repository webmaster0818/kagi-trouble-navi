...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】江東区の鍵開け・鍵交換業者おすすめ5選！タワーマンションの鍵・オートロックの相談順も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "江東区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、タワーマンション・大規模マンションのカードキーやオートロックのトラブルで管理会社と鍵業者のどちらへ先に相談すべきかまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】江東区の鍵開け・鍵交換業者おすすめ5選！タワーマンションの鍵・オートロックの相談順も分かる鍵屋比較",
    description:
      "江東区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。タワーマンション・大規模マンションの鍵トラブルで、鍵業者より先に管理形態を確認すべき理由と相談の順番も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、江東区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。江東区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「オートロックで締め出された」「カードキーをなくした」など状況に加えて、住戸の玄関がシリンダー錠かカードキー・電子錠かをメモしておくと、業者に対応可否を確認しやすくなります。マンションの場合は、鍵業者より先に管理員・管理会社への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、江東区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "江東区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜や早朝でも江東区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、江東区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "タワーマンションのカードキー・非接触キーをなくしました。鍵業者で作ってもらえますか？",
    a: "多くの場合、鍵業者では複製・再発行できません。カードキーや非接触キーはマンション側のシステムに登録して使うもので、再発行は管理会社・管理組合経由の手続きになるのが一般的です。費用や日数はマンションごとに異なります。玄関の物理鍵と併用している建物では、物理鍵側の対応（合鍵・交換）と分けて確認しましょう。",
    link: { href: "/kagi-funshitsu/", label: "鍵をなくしたときの対処はこちら" },
  },
  {
    q: "オートロックと連動する鍵とは何ですか？なくすと何が起きますか？",
    a: "1本の鍵で自宅玄関とエントランスのオートロックの両方を開けられるように設計された鍵システムのことで、大規模マンションで広く使われています。この方式の鍵をなくすと、拾った人がエントランスまで通過できてしまう可能性があるため、管理会社への届け出を早めに行いましょう。また、玄関のシリンダーだけを勝手に交換するとエントランスと連動しなくなる恐れがあるため、交換は管理会社指定の手順で進めるのが基本です。",
  },
  {
    q: "分譲マンションでも玄関の鍵を自由に交換できますか？",
    a: "建物の管理規約によります。玄関扉まわりは共用部と専有部の境界にあたるため、シリンダー交換でも届け出や承認を求める規約は珍しくありません。まず管理規約を確認し、管理会社・管理組合に相談してから業者を手配すると手戻りがありません。賃貸として借りている場合は、貸主（所有者）の承諾も必要です。",
...
export default function AreaKotoKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】江東区の鍵開け・鍵交換業者おすすめ5選！タワーマンションの鍵・オートロックの相談順も分かる鍵屋比較",
    description:
      "江東区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "江東区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜江東区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "江東区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜江東区で鍵業者を呼ぶ前に確認したいこと（タワーマンションの鍵は管理形態の確認が先）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜江東区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">江東区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「エントランスは入れたのに、エレベーターが動かせない」「カードキーをなくしたけれど、どこに言えばいいのか分からない」——湾岸部を中心にタワーマンション・大規模マンションが多い江東区では、オートロックや非接触キーが何重にも組み合わさった建物ならではの鍵トラブルが起こりがちです。こうした建物では、鍵業者に電話する前に「管理形態の確認」から始めるほうが早く解決する場面が少なくありません。
            </p>
            <p>
              本記事では、江東区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、タワーマンション・大規模マンションで鍵のトラブルが起きたとき、管理会社と鍵業者のどちらに先に相談すべきかの整理もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">江東区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            江東区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、江東区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">江東区の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            江東区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">江東区で鍵業者を呼ぶ前に確認したいこと（タワーマンションの鍵は「管理形態の確認」が先）</H2>
          <p className="leading-loose mb-4">
            タワーマンション・大規模マンションの鍵トラブルでまず確認したいのは、自分の建物の管理形態です。管理員が日中だけ常駐するのか、24時間有人管理なのか、管理会社の緊急コールセンターがあるのか——掲示板・契約書類・管理規約のいずれかに連絡先が載っているはずです。エントランスのオートロックやエレベーターに使う<Hl>カードキー・非接触キーは、マンション側のシステムで管理されているのが一般的で、街の鍵業者では複製も再発行もできないことがほとんど</Hl>です。なくした場合は、管理会社・管理組合への届け出と再発行手続きが基本ルートになります。
          </p>
          <p className="leading-loose mb-4">
            締め出されたときの相談順は、<Hl>同居家族の合鍵→管理員・コンシェルジュや管理会社の緊急窓口→鍵業者</Hl>です。住戸の玄関が一般的なシリンダー錠なら鍵業者が解錠できますが、<Hl>カードキーや電子錠タイプの玄関ドアは、業者によって対応できない場合があります</Hl>。電話の段階で玄関の鍵の種類を伝えて対応可否を確認すると、駆けつけてもらったのに開けられなかったという空振りを防げます。基本の動き方は<Link href="/shimedashi/" className="text-primary underline">締め出されたときの対処</Link>と<Link href="/autolock-shimedashi-yobou/" className="text-primary underline">オートロックの締め出し予防</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            鍵の交換やグレードアップを考える場合も、分譲マンションでは一足飛びに業者へ発注しないのが安全です。<Hl>玄関扉やその錠前は管理規約上の扱いが建物ごとに決まっており、シリンダー交換でも届け出や承認が必要な場合があります</Hl>。また、エントランスのオートロックと連動する鍵のマンションでは、勝手に交換するとエントランスを通れなくなる恐れがあるため、管理会社指定の手順に従いましょう。交換を考えるタイミングは<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>、スマートロック化は<Link href="/smart-lock/" className="text-primary underline">スマートロックの解説</Link>を参考にしてください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">江東区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">江東区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            江東区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            タワーマンション・大規模マンションの鍵は、カードキーの再発行やオートロックとの連動など「管理会社経由でしか進められない手続き」が多いのが特徴です。トラブルの際は管理形態と鍵の種類を確認し、管理側の窓口と鍵業者を使い分けましょう。業者に頼む場合も、玄関の鍵の種類を伝えて対応可否と総額を電話で確認してから依頼するのが失敗しないコツです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["江東区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
