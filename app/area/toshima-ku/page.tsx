...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】豊島区の鍵開け・鍵交換業者おすすめ5選！繁華街で鍵をなくした夜のチェックリストも分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "豊島区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、繁華街で鍵をなくした夜にその場で使える手順チェックリストまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】豊島区の鍵開け・鍵交換業者おすすめ5選！繁華街で鍵をなくした夜のチェックリストも分かる鍵屋比較",
    description:
      "豊島区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。繁華街で鍵をなくした夜に、持ち物の総ざらいから今夜の入室手段の確保まで順番につぶせるチェックリストも掲載しています。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、豊島区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。豊島区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、豊島区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "豊島区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜や早朝でも豊島区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、豊島区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "飲んだ帰りで、どこで落としたのか記憶が曖昧です。どこから探すべきですか？",
    a: "最後に鍵を使った・見た時点を思い出し、そこから今までの立ち寄り先を新しい順にさかのぼるのが効率的です。営業中のうちに電話できる店が多く残っているためです。店に電話する際は、座っていた席や会計した時間帯を伝えると探してもらいやすくなります。並行して、交通機関の忘れ物窓口への連絡と遺失届も忘れずに。",
  ...
  {
    q: "翌朝になれば店が開くので、今夜は我慢すべきでしょうか？",
    a: "状況によります。近くに頼れる家族・知人がいて安全に朝を待てるなら、翌日に店舗へ問い合わせてから判断するのも合理的です。一方、深夜に屋外で待ち続けるのは安全面でおすすめできません。賃貸なら管理会社の緊急窓口、難しければ24時間受付の鍵業者で入室を確保しましょう。鍵が戻っても、住所の分かる物と一緒に落としていた場合は交換の検討をおすすめします。",
...
export default function AreaToshimaKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】豊島区の鍵開け・鍵交換業者おすすめ5選！繁華街で鍵をなくした夜のチェックリストも分かる鍵屋比較",
    description:
      "豊島区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "豊島区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜豊島区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "豊島区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜豊島区で鍵業者を呼ぶ前に確認したいこと（繁華街で鍵をなくした夜のチェックリスト）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜豊島区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">豊島区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「終電を逃して気づいたら、かばんに鍵がない」——大きな繁華街を抱える豊島区では、飲食や買い物の帰りに鍵の紛失へ気づくトラブルが典型です。立ち寄った場所が多いほど記憶は曖昧になりますが、夜のうちにやるべきことは実は決まっています。焦って業者に電話する前に、上から順につぶしていくためのチェックリストを用意しました。
            </p>
            <p>
              本記事では、豊島区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、繁華街で鍵をなくした夜にその場で使えるチェックリストもまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">豊島区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            豊島区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、豊島区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">豊島区の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            豊島区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">豊島区で鍵業者を呼ぶ前に確認したいこと（繁華街で鍵をなくした夜のチェックリスト）</H2>
          <p className="leading-loose mb-4">
            繁華街での紛失は、立ち寄った場所が多く記憶も曖昧になりがちです。次のチェックリストを上から順につぶしていくと、探し漏れなく「今夜どう家に入るか」までたどり着けます。
          </p>
          <div className="rounded-[2px] border border-border bg-[#F5F8FC] px-5 py-5 md:px-6 mb-4">
            <p className="font-bold text-sm md:text-base mb-3">鍵をなくした夜のチェックリスト</p>
            <ol className="grid gap-2 text-sm leading-relaxed list-decimal pl-5">
              <li>持ち物の総ざらい——コート・ズボンの全ポケット、かばんの内ポケット、買い物袋の中まで、その場でもう一度確認する</li>
              <li>動線の書き出し——最後に鍵を見た時点から今までの立ち寄り先を、新しい順に書き出す</li>
              <li>店舗への電話——営業中の店から順に電話し、閉店している店は翌日の開店時間をメモしておく</li>
              <li>交通機関の忘れ物窓口——電車・バス・タクシーを使ったなら、各社の忘れ物窓口へ連絡する</li>
              <li>遺失届——見つからなければ最寄りの交番・警察署へ遺失届を出す（急がない相談は警察相談専用電話「#9110」も利用可）</li>
              <li>今夜の入室手段——同居家族・合鍵→賃貸は管理会社の緊急窓口→24時間受付の鍵業者、の順で確保する</li>
            </ol>
          </div>
          <p className="leading-loose mb-4">
            6番まで来て鍵業者を呼ぶ場合は、<Hl>深夜料金の有無を含めた総額と到着目安を電話で確認してから</Hl>依頼しましょう。紛失時の基本の動きは<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>、締め出しの動き方は<Link href="/shimedashi/" className="text-primary underline">締め出されたときの対処</Link>でも詳しく解説しています。
          </p>
          <p className="leading-loose mb-4">
            翌日以降にやるのは、閉店していた店への問い合わせと、鍵が戻らない場合の交換判断です。<Hl>住所の分かる物と一緒に落としたなら、拾った人が自宅を特定できるため、鍵交換を強く検討すべき</Hl>状況です（<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>）。再発防止には、かばんの中の鍵の定位置を決めてキーチェーンでつなぐ・鍵と住所につながる物を分けて持つ、の2つが手軽で効果的です。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">豊島区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">豊島区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            豊島区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            繁華街での紛失は、持ち物の総ざらい→動線の書き出し→店舗・交通機関への連絡→遺失届→今夜の入室手段、と順番を決めて動くのが確実な近道です。チェックリストをスマホにメモしておけば、いざというとき落ち着いて動けます。業者を呼ぶ場合は、深夜料金を含む総額の確認を忘れずに。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["豊島区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
