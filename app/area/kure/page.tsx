...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】呉市の鍵開け・鍵交換業者おすすめ5選！なくした鍵が見つからないまま数日たったときの交換判断と当面の備えも分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "呉市で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、なくした鍵が見つからないまま時間が経ったときに交換へ踏み切る判断材料と、それまでの間にできる備えまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】呉市の鍵開け・鍵交換業者おすすめ5選！なくした鍵が見つからないまま数日たったときの交換判断と当面の備えも分かる鍵屋比較",
    description:
      "呉市で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。見つからない鍵をいつまで待つかの判断材料と、交換までの間にできる備え、共用部の鍵を含むときの報告の順番も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、呉市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（広島県対応）", note: "広島県対応の一次確認値。呉市での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① なくした鍵と錠の情報を整理する",
    b: "どの扉の鍵か、何本の束だったか、キーホルダーに何が付いていたかを書き出します。交換まで考えるなら、錠のメーカー名と型番（扉側面の金属プレートに刻印があります）と、鍵の形（刻みキーかディンプルキーか）を控えておきましょう。賃貸か持ち家か、共用部の鍵を含むかも最初に伝えると、連絡先の切り分けが早くなります。",
  },
  {
    t: "② 出張費・夜間料金を含めた総額の見積もりを取る",
    b: "広告の「◯◯円〜」は最低価格で、実際は錠の種類・出張距離・時間帯で変わります。開けるだけの費用と、シリンダー交換まで進んだ場合の費用は分けて確認しておきましょう。防犯性の高い錠ほど部材費が上がるため、交換を検討しているなら部材費の内訳まで聞いておくと比較しやすくなります。",
  },
  {
    t: "③ 運営会社を確認する",
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "数日たってから交換を決める場合は日程に余裕を持てる依頼なので、2〜3社から見積もりを取って総額・提案内容を比較する価値があります。今回の5社はいずれも全国対応型で、呉市からも依頼できます（対応エリアは「広島県」としての一次確認値のため、詳細条件は各社へ確認を）。メーカー名・型番を同じ条件で伝えれば、取り扱いの可否と金額の差が見えやすくなります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "呉市の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "なくした鍵は何日くらい待ってから交換を決めればいいですか？",
    a: "日数そのものに決まりはありません。判断の軸になるのは、失われた鍵と一緒に自宅の場所が分かるものまで手放していないか、その鍵で開く扉がいくつあるか、共用部の鍵が含まれていないかの3つです。心当たりの場所に一通り問い合わせて出てこなかった段階で、この3つを並べて考えるのが現実的です。待っている間も家の側の状態は何も変わらない点は意識しておいてください。",
    link: { href: "/kagi-koukan-timing/", label: "鍵交換のタイミングはこちら" },
  },
  {
    q: "交換するまでの間、何かできることはありますか？",
    a: "内側から補助錠をかける、在宅時は必ず施錠する、就寝時に貴重品を玄関から離れた場所へ移すといった当面の手当てができます。工事の要らない補助錠なら短期間の備えとして使えますが、賃貸では扉や枠に穴を開ける取り付け方はできません。あくまで交換までのつなぎで、根本の解決にはならない点は押さえておいてください。",
    link: { href: "/hojo-jou/", label: "補助錠の選び方はこちら" },
  },
  {
    q: "マンションの共用部の鍵も一緒になくしました。",
    a: "自分だけで判断できない案件になります。エントランスの鍵やオートロックのキー、宅配ボックスの鍵などは建物全体に関わるため、気づいた時点で管理会社・管理組合へ報告してください。報告が遅れるほど、建物側で取れる対応の選択肢が狭まります。専有部の錠をどうするかも、共用部の扱いが決まってからのほうが段取りを組みやすくなります。",
    link: { href: "/chintai-kagi-koukan-hiyou/", label: "賃貸の鍵交換費用はこちら" },
  },
  {
    q: "交換したあとに、なくした鍵が出てきました。どうすればいいですか？",
    a: "交換済みであれば、その鍵はもう自宅の錠を開けません。ただし、どこでどう保管されていたのかが分からないまま戻ってきた鍵は、一度他人の手に渡っていた可能性があります。不要になった鍵は、そのまま捨てずに形が分からなくなる形で処分してください。賃貸で貸主から預かっていた鍵なら、勝手に処分せず管理会社・貸主へ返します。",
    link: { href: "/kagi-shobun/", label: "不要になった鍵の処分はこちら" },
  },
  {
    q: "深夜や早朝でも呉市に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「広島県」としての一次確認値のため、呉市での詳細は依頼時に各社へご確認ください。",
  },
];
...
export default function AreaPageKure() {
  const articleSchema = {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "呉市の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜呉市で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "呉市の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#kure-tips", label: "6｜呉市で鍵業者を呼ぶ前に確認したいこと（見つからない鍵をいつまで待つか。交換に踏み切る線の引き方）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜呉市の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">呉市の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              呉市は広島県の南西部、瀬戸内海に面したまちです。海沿いの市街地の背後に山がせまり、斜面に沿って住宅地が広がっています。市内の移動はもちろん、広島市方面や島しょ部との行き来も日常的で、鍵をなくしたときに「どこで落としたか」の見当がつきにくいことがあります。見つかるかもしれないと待つうちに数日が過ぎてしまうのも、よくある話です。
            </p>
            <p>
              本記事では、呉市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、なくした鍵が見つからないまま時間が経ったときに交換へ踏み切るかどうかをどう決めるか、踏み切るまでの間に何ができるかを整理しています。
            </p>
          </div>
          ...
          {/* 1｜業者一覧 */}
          <H2 id="top5" num="1">呉市で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            呉市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「広島県」としての一次確認値のため、呉市での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※近隣エリアの比較は<Link href="/area/hiroshima/" className="text-primary underline">広島版</Link>・<Link href="/area/hiroshima-naka/" className="text-primary underline">広島市中区版</Link>・<Link href="/area/fukuyama/" className="text-primary underline">福山市版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">呉市の鍵業者をとにかく急いで開けてほしいなら</H2>
          <p className="leading-loose mb-4">
            鍵がないと分かった直後は、家に入る手立てをどうするかが先の判断になります。探しに戻るか、開けてもらうか。外で探し続けている時間も、結果としては待ち時間と変わりません。駆けつけスピードの目安を公表している業者としては、<Hl>鍵の生活救急車が最短15分の駆けつけと全国約2,270店の拠点網</Hl>をうたっており、拠点数の多さは到着スピードの裏付けになります。また、<Hl>カギ110番は電話一本で最短5分の手配</Hl>（全国47都道府県の加盟店ネットワークから近隣の業者を手配）を掲げています。
          </p>
          ...
          <p className="leading-loose mb-4">
            夜に鍵をなくしたと気づいたときは、立ち寄り先の問い合わせ先も届け出の窓口も翌朝まで動きません。その夜に家へ入る必要があるなら、探すことと並行して手配を進めるほうが早く片づきます。一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、遅い時間の依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          <p className="leading-loose mb-4">
            鍵をなくしたときの作業は、開けるだけで終わる場合と、交換まで進む場合とで金額が大きく変わります。どこまでが見積もりに含まれるのかを先に示す方針かどうかが効いてきます。<Hl>鍵のレスキューは料金をホームページで明示し、現場での追加費用がない</Hl>と掲げています（鍵開け8,000円〜＋出張料0円〜8,800円）。また、<Hl>鍵の生活救急車は出張・見積もり・キャンセルが無料</Hl>のため、提示された金額を見てから依頼するかどうかを判断できます。
          </p>
          <p className="leading-loose mb-4">
          ...
          <p className="leading-loose mb-4">
            鍵業者には、依頼を受けて加盟店を手配する「紹介型」と、自社のスタッフが直接対応する「自社対応型」があります。紹介型は対応範囲が広い一方、店舗によって技術・対応に差が出ることがあります。今回の紹介では、<Hl>鍵のレスキューが全国約20拠点を自社社員で対応する自社対応型</Hl>で、女性スタッフの指名にも対応しています。交換まで進む場合はシリンダーの部材が必要になるため、依頼時に「自社スタッフか協力店か」とあわせて、その錠の部材を持っているか取り寄せになるかも確認してみましょう。
          </p>

          {/* 6｜エリア特有 */}
          <H2 id="kure-tips" num="6">呉市で鍵業者を呼ぶ前に確認したいこと（見つからない鍵をいつまで待つか。交換に踏み切る線の引き方）</H2>
          <p className="leading-loose mb-4">
            鍵が見つからないまま数日が過ぎると、<Hl>そのうち出てくるかもしれないという気持ちと、替えたほうがいいという気持ちが行き来します</Hl>。日数で線を引く決まりはありませんが、判断の軸は3つに整理できます。ひとつ目は、失われた鍵と一緒に自宅の場所につながるもの——定期券、名札、社員証、郵便物、住所の書かれたカード類——まで手放していないか。ふたつ目は、その鍵で開く扉がいくつあるか。玄関だけなのか、勝手口や物置、車庫まで同じ鍵なのかで替える範囲が変わります。三つ目は、共用部の鍵が含まれているかどうかです。心当たりの場所に一通り問い合わせて出てこなかった段階で、この3つを並べて考えてください。初動の手順は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            <Hl>待っている間、家の側の状態は何も変わりません</Hl>。ここが見落とされやすい点です。出てくるかもしれないと様子を見る日々も、鍵が他人の手にある可能性を抱えたまま過ぎていきます。交換に踏み切るまでの間は、当面の手当てをしておきましょう。内側から補助錠をかける、在宅時は必ず施錠する、就寝時に貴重品を玄関から離れた場所へ移す。工事の要らない補助錠なら短期の備えとして使えますが、賃貸では扉や枠に穴を開ける取り付け方はできません。あくまでつなぎで、根本の解決にはならない点は押さえておいてください。選び方は<Link href="/hojo-jou/" className="text-primary underline">補助錠の選び方</Link>、内側からの対策は<Link href="/thumb-turn-taisaku/" className="text-primary underline">サムターン回し対策</Link>で扱っています。
          </p>
          <p className="leading-loose mb-4">
            <Hl>自分だけで決められない場合があります</Hl>。借りている住まいの錠は貸主の所有物なので、交換するかどうかも、誰が費用を持つかも、管理会社・貸主との相談で決まります。分譲マンションでも、エントランスの鍵やオートロックのキー、宅配ボックスの鍵といった共用部に関わるものは、建物全体の問題として管理組合へ報告するのが先です。報告が遅れるほど、建物側で取れる対応の選択肢が狭まります。自分の部屋の錠をどうするかは、共用部の扱いが決まってからのほうが段取りを組みやすくなります。費用の考え方は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用</Link>で解説しています。
          </p>
          <p className="leading-loose mb-4">
            交換したあとに<Hl>なくした鍵が出てくることもあります</Hl>。すでに錠を替えていれば、その鍵で自宅は開きません。ただ、どこでどう保管されていたのかが分からないまま戻ってきた鍵は、一度他人の手に渡っていた可能性があります。住所の手がかりが一緒に付いていたなら、出てきたこと自体で安心してよいとは限りません。不要になった鍵は、そのまま捨てずに形が分からなくなる形で処分してください。賃貸で貸主から預かっていた鍵なら、勝手に処分せず管理会社・貸主へ返します。処分の考え方は<Link href="/kagi-shobun/" className="text-primary underline">不要になった鍵の処分</Link>、防犯面の考え方は<Link href="/akisu-shinnyu-teguchi/" className="text-primary underline">空き巣の侵入手口</Link>にまとめています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">呉市の鍵業者の失敗しない選び方</H2>
          <p className="leading-loose mb-6">
            鍵業者選びで後悔しないためには、次の4つのステップを踏むことが大切です。開けるだけで済ませるか交換まで進めるかで金額も段取りも変わるぶん、伝える情報の精度が結果を左右します。
          </p>
          {steps.map((s) => (
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">呉市の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            呉市で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の受付ならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            なくした鍵が見つからないとき、交換に踏み切るかどうかは日数ではなく中身で決まります。自宅の場所につながるものを一緒に失っていないか、その鍵で開く扉がいくつあるか、共用部の鍵を含んでいないか。この3つを並べて判断し、踏み切るまでの間は補助錠と施錠の習慣でつなぎ、賃貸や分譲なら報告を先に済ませる。そのうえで、本記事の一次確認値を出発点に、目的と予算に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["呉市", "鍵をなくした", "交換の見きわめ", "補助錠", "共用部の鍵", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
