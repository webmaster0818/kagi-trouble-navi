...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】山形の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "山形で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、果樹園の作業小屋・農機具倉庫など屋外設備の鍵の劣化対策と共有管理まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】山形の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "山形で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。屋外の南京錠・倉庫錠の固着への対処と、家族で共有する鍵の管理の考え方も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、山形市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」「小屋の錠を交換したい」など状況と、鍵の種類をメモしておくと、電話での見積もり精度が上がります。屋外の南京錠や倉庫の錠は、メーカー名や型番が分かると話が早く、写真を撮っておくと現地でのやり取りもスムーズです。母屋から離れた場所なら、目印になる住所・位置の伝え方も考えておきましょう。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。小屋や倉庫の錠の交換・取り付けのように日程が読める依頼なら、農作業の合間に合わせてじっくり比較する価値があります。今回の5社はいずれも全国対応型で山形から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "山形の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "屋外の南京錠が固くて開きません。壊すしかないですか？",
    a: "まず鍵穴専用の潤滑剤を試してください。油や一般的な潤滑スプレーは内部でほこりを固めるため使わないのが鉄則です。改善しなければ、無理にこじらず業者に相談を。南京錠は切断・破錠で開けて新しい錠に替えるのが現実的なことも多く、買い替える場合は屋外用（防錆仕様）を選ぶと固着の再発を減らせます。",
    link: { href: "/kagi-mawaranai/", label: "鍵が回らないときの解説はこちら" },
  },
  {
    q: "作業小屋や倉庫の鍵を家族で共有するときの注意点は？",
    a: "「どの錠にどの鍵か」をタグで区別しつつ、場所が特定できる書き方（「◯◯の小屋」など）を避けることと、本数と持ち主を記録しておくことです。手伝いの人に渡す場合は返却の約束まで決めておきましょう。錠の数が増えて管理しきれないときは、1本の鍵で複数の錠を開けられる同一キー化を業者に相談する方法もあります。",
    link: { href: "/aikagi/", label: "合鍵の基礎知識はこちら" },
  },
  {
    q: "小屋や物置の錠の交換・取り付けも鍵業者に頼めますか？",
    a: "多くの鍵業者が、住宅の玄関だけでなく物置・倉庫・シャッターの錠の交換や新規取り付けにも対応しています。農機具や工具は盗難の対象になり得るため、錠が古くなった小屋の交換や、施錠設備のない収納への後付けも相談できます。設置環境（屋外か・扉の材質）を伝えると見積もりがスムーズです。",
...
export default function AreaYamagataPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】山形の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "山形で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "山形の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#yamagata5",
      label: "1｜山形で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "山形の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#yamagata-tips", label: "6｜山形で鍵業者を呼ぶ前に確認したいこと（果樹園・作業小屋など屋外設備の鍵管理）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜山形の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">山形の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「収穫期になって作業小屋の南京錠を開けようとしたら、固くて回らない」「農機具をしまう倉庫の鍵を、家族の誰が持っているのかはっきりしない」——住まいの玄関に比べて、屋外の小屋や倉庫の鍵は管理が後回しになりがちです。果樹園や畑と暮らしの距離が近い山形では、母屋以外にも施錠したい場所が多く、雨風にさらされる錠の劣化と、複数人で使う鍵の共有管理が課題になります。
            </p>
            <p>
              本記事では、山形市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、作業小屋・倉庫など屋外設備の鍵の劣化対策と共有管理の考え方もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="yamagata5" num="1">山形で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            山形市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/sendai/" className="text-primary underline">仙台版</Link>・<Link href="/area/fukushima/" className="text-primary underline">福島版</Link>・<Link href="/area/niigata/" className="text-primary underline">新潟版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">山形の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            山形でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜山形特有 */}
          <H2 id="yamagata-tips" num="6">山形で鍵業者を呼ぶ前に確認したいこと（果樹園・作業小屋など屋外設備の鍵管理）</H2>
          <p className="leading-loose mb-4">
            屋外の錠は、雨風やほこりにさらされる分だけ<Hl>住まいの玄関より固着・さびが早く進みます</Hl>。久しぶりに使う南京錠が固いときは、鍵穴専用の潤滑剤だけを使い、油や一般的な潤滑スプレーは差さないのが鉄則です。無理にこじると鍵折れや錠の破損につながります。改善しない錠は破錠・交換が現実的で、買い替えの際は屋外用（防錆仕様）を選ぶと再発を減らせます。症状別の対処は<Link href="/kagi-mawaranai/" className="text-primary underline">鍵が回らないときの解説記事</Link>を、錠の種類は<Link href="/joumae-type-zukan/" className="text-primary underline">錠前の種類図鑑</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            複数人で使う小屋・倉庫では、<Hl>「どの錠にどの鍵か」「誰が何本持っているか」を記録する</Hl>ことが管理の柱になります。鍵のタグは色や番号で区別し、場所が特定できる書き方は避けましょう。収穫期だけ手伝いに来る人へ渡す鍵は、渡した日と返却の約束をセットにしておくと行き違いを防げます。共有の考え方は<Link href="/aikagi/" className="text-primary underline">合鍵の解説記事</Link>で整理しています。
          </p>
          <p className="leading-loose mb-4">
            錠の数が増えて鍵束が膨らんできたら、<Hl>1本の鍵で複数の錠を開けられる「同一キー化」を業者に相談する</Hl>のも一つの方法です。農機具や工具、収穫物を保管する小屋は盗難の対象になり得るため、施錠設備のない収納への錠の後付けや、古い錠の交換もあわせて検討する価値があります。日程の融通が利く依頼なので、費用は<Link href="/ryokin/" className="text-primary underline">料金相場ページ</Link>を目安に複数社を比較しましょう。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">山形の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">山形の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            山形で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            屋外設備の鍵管理は、固着させない（専用潤滑剤・防錆仕様の錠）・記録して共有する（タグと本数の管理）・まとめる（同一キー化や錠の後付けの相談）という3つの視点で整理できます。母屋の玄関ほど頻繁に使わないからこそ、農作業の節目に動作を確かめておくと、繁忙期のトラブルを防げます。交換や取り付けのように日程が読める依頼は、複数社から総額の見積もりを取って比較することが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["山形", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
