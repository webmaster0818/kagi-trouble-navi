...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】甲府の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "甲府で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、空き家・別荘の管理を頼むときの鍵の預け方と合鍵管理まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】甲府の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "甲府で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。空き家・別荘の管理を頼むときの鍵の預け方と、管理者交代時の交換判断も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、甲府市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」「空き家の鍵を交換したい」など状況と、鍵の種類をメモしておくと、電話での見積もり精度が上がります。無人の家の鍵開け・交換では、所有者確認の書類や現地への立ち会いを求められる場合があるため、依頼時に確認しておきましょう。所有者本人以外が依頼する場合は、所有者の同意を先に取っておくとスムーズです。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。空き家・別荘の鍵交換のように日程が読める依頼なら、じっくり比較する価値があります。現地立ち会いの日程もあわせて調整しましょう。今回の5社はいずれも全国対応型で甲府から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs = [
  {
    q: "甲府の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "空き家の管理をお願いする人に鍵を預けるときの注意点は？",
    a: "誰に・何本・いつ渡したかを記録し、渡す本数は必要最小限にすることです。管理業者なら鍵の保管方法と複製の可否を契約前に確認し、個人にお願いする場合も「合鍵は作らない」「又貸ししない」という取り決めを明確にしておきましょう。",
  },
  {
    q: "管理をやめるとき、鍵は返してもらえば十分ですか？",
    a: "返却を受けても、預けていた期間に合鍵が作られていた可能性を後から確かめる方法はありません。管理者が変わる・管理をやめるといった節目では、シリンダー交換で「開けられる人」をリセットすることを選択肢に入れると安心です。",
  },
  {
    q: "長く無人だった家の鍵が回らなくなりました。故障ですか？",
    a: "使われない錠は内部の潤滑が切れ、ほこりなどで固着することがあります。無理に回すと鍵折れにつながるため力任せは禁物です。鍵穴専用の潤滑剤で改善しない場合は業者に相談しましょう。劣化の程度によってはシリンダー交換の提案を受けることもあります。",
  },
  {
...
export default function AreaKofuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】甲府の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "甲府で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "甲府の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#kofu5",
      label: "1｜甲府で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "甲府の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#kofu-tips", label: "6｜甲府で鍵業者を呼ぶ前に確認したいこと（空き家・別荘の管理を頼むときの鍵の預け方）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜甲府の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">甲府の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「離れて暮らすうちに実家が空き家になり、近所の方に見回りをお願いしている」「別荘の管理を業者に任せたいが、鍵をどう渡せばよいか分からない」——自分が住んでいない家の鍵を誰にどう預けるかは、答えの出しにくい問題です。別荘地や親の家との関わりが生まれやすい甲府周辺でも、管理を第三者に頼む場面での鍵の受け渡しと合鍵の管理は、家を守るうえで重要なテーマです。
            </p>
            <p>
              本記事では、甲府市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、空き家・別荘の管理を人に頼むときの鍵の預け方と合鍵管理の考え方もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="kofu5" num="1">甲府で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            甲府市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/tokyo/" className="text-primary underline">東京版</Link>・<Link href="/area/nagano/" className="text-primary underline">長野版</Link>・<Link href="/area/shizuoka/" className="text-primary underline">静岡版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">甲府の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            甲府でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜甲府特有 */}
          <H2 id="kofu-tips" num="6">甲府で鍵業者を呼ぶ前に確認したいこと（空き家・別荘の管理を頼むときの鍵の預け方）</H2>
          <p className="leading-loose mb-4">
            鍵を預けるときの基本は、<Hl>「誰に・何本・いつ渡したか」を記録に残す</Hl>ことです。管理業者に依頼する場合は、鍵の保管方法と複製の可否を契約前に確認しましょう。個人にお願いする場合も、合鍵を作らないこと・又貸ししないことを、口頭ではなく書き付けで共有しておくと後々の行き違いを防げます。郵送で受け渡す場合は、住所の分かる書類と鍵を同封しないよう注意してください。合鍵の基礎は<Link href="/aikagi/" className="text-primary underline">合鍵の解説記事</Link>で説明しています。
          </p>
          <p className="leading-loose mb-4">
            現地にキーボックスを設置して受け渡しを省く方法もありますが、<Hl>無人の家は暗証番号を試される時間を与えやすい</Hl>という弱点を踏まえる必要があります。設置するなら、外から見えにくい位置に付ける・番号を定期的に変える・開閉のたびに連絡を入れてもらう、といった運用を決めてからにしましょう。詳しくは<Link href="/key-box-kanri/" className="text-primary underline">キーボックスでの鍵管理の解説記事</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            管理をやめるとき・頼む相手が変わるときは、<Hl>鍵の返却だけで安心せず、シリンダー交換を選択肢に入れる</Hl>のが堅実です。預けていた期間に合鍵が作られていないことを、後から確かめる方法はないためです。また、長期間無人の家は錠内部の固着も起きやすく、訪問時に抜き差しと施解錠の動作を確かめておくと、いざというときのトラブルを防げます。<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>と<Link href="/kagi-mawaranai/" className="text-primary underline">鍵が回らないときの解説記事</Link>も参考にしてください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">甲府の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">甲府の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            甲府で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            住んでいない家の鍵は、貸与の記録・複製の取り決め・管理者交代時の交換検討という基本を押さえることで、目の届かない期間の不安を減らせます。長期間無人の家は錠の固着も起きやすいため、訪問時の動作確認も習慣にしましょう。日程が読める依頼だからこそ、複数社から総額の見積もりを取って比較することが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["甲府", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
