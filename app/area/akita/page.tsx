...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】秋田の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "秋田で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、離れて暮らす親の見守りに備えた合鍵の預かり方と緊急時に家へ入る手段まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】秋田の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "秋田で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。実家の合鍵の預かり方・記録の残し方と、親と連絡が取れないときに家へ入る手段の考え方も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、秋田市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」「実家の鍵を交換したい」など状況と、鍵の種類をメモしておくと、電話での見積もり精度が上がります。親の家の鍵開け・交換を子ども世代が依頼する場合は、所有者確認の書類や本人・家族の立ち会いを求められることがあるため、依頼時に必要なものを確認しておきましょう。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。実家の鍵の交換や合鍵の追加のように日程が読める依頼なら、帰省の予定に合わせてじっくり比較する価値があります。今回の5社はいずれも全国対応型で秋田から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "秋田の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "離れて暮らす親の家の合鍵は、どのように預かっておくのがよいですか？",
    a: "子ども世代が1本を預かり、「誰が・何本持っているか」を家族全員で共有しておくのが基本です。遠方に住んでいる場合は、実家の近くに住む親族や信頼できる人にもう1本を託す組み合わせが現実的です。鍵には実家の住所が分かるタグを付けず、保管場所も家族で決めておきましょう。",
    link: { href: "/aikagi/", label: "合鍵の基礎知識はこちら" },
  },
  {
    q: "親と連絡が取れず、家にも入れません。鍵業者を呼べば開けてもらえますか？",
    a: "鍵業者による解錠は、その家に入る正当な権利の確認（身分証や住所の分かる書類、本人・家族の立ち会いなど）を前提とするのが一般的で、第三者の家をすぐに開けてもらえるとは限りません。倒れているかもしれないなど安否が急を要する状況なら、業者より先に119番・110番へ連絡してください。生命に関わる場面では救助の判断で開けられることがあります。",
  },
  {
    q: "親が鍵をなくすことが増えました。よい対策はありますか？",
    a: "持ち歩く鍵を1本に絞り、玄関近くの決まった場所に掛けるなど「定位置」をつくるのが基本です。屋外にキーボックスを設置して家族が番号を共有する方法もありますが、外から見えにくい位置に付ける・番号を定期的に変えるといった運用が前提になります。紛失が続く場合は、なくしても拾った人が家を特定できないよう、住所につながる物と鍵を分けて持つ工夫も大切です。",
    link: { href: "/key-box-kanri/", label: "キーボックスでの鍵管理の解説はこちら" },
  },
  {
...
export default function AreaAkitaPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】秋田の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "秋田で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "秋田の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#akita5",
      label: "1｜秋田で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "秋田の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#akita-tips", label: "6｜秋田で鍵業者を呼ぶ前に確認したいこと（離れて暮らす親の見守りと合鍵の備え）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜秋田の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">秋田の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「離れて暮らす親と連絡が取れなくなったとき、駆けつけても家に入れなかったらどうしよう」「実家の合鍵を預かっておくべきか、親に負担のない方法はないか」——親の見守りと鍵の備えは、多くの家族がいつか向き合うテーマです。秋田で暮らす親を市外・県外の子ども世代が気にかける形も含めて、「緊急時に家へ入れる手段」をあらかじめ決めておくことは、見守りの土台になります。
            </p>
            <p>
              本記事では、秋田市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、実家の合鍵の預かり方と、緊急時に家へ入るための備えもまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="akita5" num="1">秋田で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            秋田市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/sendai/" className="text-primary underline">仙台版</Link>・<Link href="/area/morioka/" className="text-primary underline">盛岡版</Link>・<Link href="/area/yamagata/" className="text-primary underline">山形版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">秋田の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            秋田でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜秋田特有 */}
          <H2 id="akita-tips" num="6">秋田で鍵業者を呼ぶ前に確認したいこと（離れて暮らす親の見守りと合鍵の備え）</H2>
          <p className="leading-loose mb-4">
            見守りの備えの第一歩は、<Hl>子ども世代が実家の合鍵を1本預かり、誰が何本持っているかを家族で共有する</Hl>ことです。遠方に住んでいるなら、実家の近くの親族や信頼できる人にもう1本を託す組み合わせが現実的です。預けた相手と本数は書き付けて残し、鍵に実家の住所が分かるタグを付けないことも徹底しましょう。合鍵づくりの基本は<Link href="/aikagi/" className="text-primary underline">合鍵の解説記事</Link>で説明しています。
          </p>
          <p className="leading-loose mb-4">
            親自身が鍵をなくしやすくなってきたと感じたら、<Hl>持ち歩く鍵を1本に絞り、家の中でも外でも「定位置」を決める</Hl>工夫が効きます。玄関のキーフックや、かばんの決まったポケットに固定するだけでも探し物は減ります。屋外のキーボックスに家族が番号を共有しておく方法もありますが、見えにくい位置への設置と番号の定期変更が前提です。運用の注意点は<Link href="/key-box-kanri/" className="text-primary underline">キーボックスでの鍵管理の解説記事</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            もっとも大切なのは、<Hl>緊急時の動き方を「鍵が開かない前提」で決めておかない</Hl>ことです。連絡が取れず安否が急を要する状況では、鍵業者の手配より先に119番・110番へ連絡してください。生命に関わる場面では救助の判断で開けられることがあります。一方、緊急性がない締め出しや紛失であれば、預けた合鍵で開ける・業者に依頼するという通常の手順で対応します。<Link href="/shimedashi/" className="text-primary underline">締め出されたときの対処</Link>と、実家の古い鍵を替えるかどうかの判断は<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>も参考にしてください。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">秋田の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">秋田の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            秋田で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            親の見守りと鍵の備えは、合鍵を誰が持つかの取り決め・持ち歩く鍵の定位置化・緊急時の連絡順という3点を決めておくだけで、いざというときの動きが大きく変わります。安否が急を要する場面では業者より先に119番・110番という原則も、家族で共有しておきましょう。交換や合鍵の追加のように日程が読める依頼は、複数社から総額の見積もりを取って比較することが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["秋田", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
