...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】福島の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "福島で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、転勤・単身赴任の引っ越しでの鍵の返却と入居時の交換の段取りまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】福島の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "福島で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。退去時の合鍵を含めた鍵の返却と、入居時に鍵交換を確認・依頼する段取りも解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、福島市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」「入居に合わせて交換したい」など状況と、鍵の種類をメモしておくと、電話での見積もり精度が上がります。賃貸の鍵の交換は管理会社・大家の承諾が前提のため、業者より先に管理会社へ連絡するのが基本です。引っ越しに合わせた依頼なら、入居日・退去日から逆算して日程を押さえましょう。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。入居に合わせた鍵交換のように日付が決まっている依頼なら、早めに動くほど比較の余地が生まれます。今回の5社はいずれも全国対応型で福島から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "福島の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "退去するとき、自分で作った合鍵も返す必要がありますか？",
    a: "原則として、入居時に受け取った鍵と自分で作った合鍵を含めた全数を返却します。合鍵の存在を黙って退去すると、後の入居者の防犯に関わるため、本数をそろえて返すのが誠実な対応です。鍵を1本でも紛失している場合は正直に申告し、シリンダー交換の費用負担について契約内容に沿って話し合いましょう。",
    link: { href: "/taikyo-kagi-trouble/", label: "退去時の鍵トラブルの解説はこちら" },
  },
  {
    q: "単身赴任先の賃貸の鍵を交換したい場合、勝手に交換してよいですか？",
    a: "いいえ。賃貸の鍵（シリンダー）は大家の所有物のため、無断交換は契約違反になり得ます。防犯上の不安があって交換したい場合は、まず管理会社・大家に相談し、承諾と費用負担の取り決めを得てから業者に依頼しましょう。退去時に元のシリンダーへ戻す条件が付くこともあります。",
    link: { href: "/chintai-kagi-koukan-hiyou/", label: "賃貸の鍵交換費用の解説はこちら" },
  },
  {
    q: "入居する部屋の鍵が、前の入居者のときから交換されているか確認できますか？",
    a: "契約前後に管理会社へ「前入居者退去後に鍵交換済みか」を確認しましょう。多くの賃貸では入居時に鍵交換費用を借主が負担する慣行がありますが、実施の有無や費用は物件により異なります。交換されていない場合は、防犯上の観点から交換の実施と費用負担を相談するのがおすすめです。",
  },
  {
...
export default function AreaFukushimaPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】福島の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "福島で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "福島の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#fukushima5",
      label: "1｜福島で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "福島の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#fukushima-tips", label: "6｜福島で鍵業者を呼ぶ前に確認したいこと（転勤・単身赴任の引っ越しと鍵の返却・交換）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜福島の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">福島の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「内示から引っ越しまで時間がなく、旧居の鍵の返却や新居の鍵の確認まで頭が回らない」「単身赴任先と自宅の二重生活で、鍵の管理が煩雑になってきた」——転勤にともなう引っ越しでは、荷造りや役所の手続きに追われて、鍵まわりの段取りが後回しになりがちです。転勤での出入りが一定数ある福島でも、退去時の鍵の返却と入居時の鍵の確認は、あとから揉めないために押さえておきたいポイントです。
            </p>
            <p>
              本記事では、福島市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、転勤・単身赴任の引っ越しにともなう鍵の返却と交換の段取りもまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="fukushima5" num="1">福島で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            福島市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/sendai/" className="text-primary underline">仙台版</Link>・<Link href="/area/yamagata/" className="text-primary underline">山形版</Link>・<Link href="/area/utsunomiya/" className="text-primary underline">宇都宮版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">福島の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            福島でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜福島特有 */}
          <H2 id="fukushima-tips" num="6">福島で鍵業者を呼ぶ前に確認したいこと（転勤・単身赴任の引っ越しと鍵の返却・交換）</H2>
          <p className="leading-loose mb-4">
            退去側の基本は、<Hl>入居時に受け取った鍵と、自分で作った合鍵を含めた全数の返却</Hl>です。合鍵を手元に残したまま退去すると、次の入居者の防犯に関わります。紛失している鍵があるなら黙らずに申告し、シリンダー交換の費用負担を契約内容に沿って確認しましょう。返却時は「何本返したか」を受け渡しの記録に残しておくと安心です。詳しくは<Link href="/taikyo-kagi-trouble/" className="text-primary underline">退去時の鍵トラブルの解説記事</Link>と<Link href="/hikkoshi-kagi-checklist/" className="text-primary underline">引っ越し時の鍵チェックリスト</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            入居側では、<Hl>前入居者の退去後に鍵が交換されているかを管理会社に確認する</Hl>ことが出発点です。交換されていなければ、前の入居者側に合鍵が残っている可能性を否定できません。交換の実施と費用負担（借主負担の慣行が多いものの物件により異なります）を契約時に確認し、自分で手配したい場合も必ず管理会社・大家の承諾を得てから業者に依頼しましょう。費用の考え方は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用の解説記事</Link>で整理しています。
          </p>
          <p className="leading-loose mb-4">
            単身赴任の二重生活では、<Hl>自宅の鍵と赴任先の鍵をキーホルダーごと分け、行き来のたびに持ち替えない</Hl>運用が紛失予防になります。赴任先で鍵をなくしたときは、管理会社への連絡・警察への遺失届・必要に応じた業者手配という順番で動きます。家族が自宅側の合鍵体制を把握しておくことも、帰省時の締め出し防止に役立ちます。なくしたときの手順は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>で説明しています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">福島の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">福島の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            福島で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            転勤・単身赴任の引っ越しでは、退去時は合鍵を含めた全数返却、入居時は交換済みかの確認と承諾を得たうえでの交換手配、二重生活では鍵の持ち分けという段取りを押さえておくと、鍵まわりのトラブルを避けられます。日付が決まっている依頼だからこそ、早めに複数社から総額の見積もりを取って比較することが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["福島", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
