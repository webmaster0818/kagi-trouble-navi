...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】杉並区の鍵開け・鍵交換業者おすすめ5選！古い建物の鍵の不調・交換判断も分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "杉並区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、古い木造アパート・戸建てで多い鍵の建て付け不調の切り分けと交換判断まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】杉並区の鍵開け・鍵交換業者おすすめ5選！古い建物の鍵の不調・交換判断も分かる鍵屋比較",
    description:
      "杉並区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。古い木造アパートや戸建てで多い、鍵と建て付けの不調の切り分け方と交換の判断基準も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、杉並区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。杉並区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「鍵が回らない」「引っかかる」などの症状に加えて、ドアを開けた状態でも症状が出るか（＝鍵側か建て付け側か）、建物の築年数や錠前のメーカー・型番が分かればメモしておくと、電話での見積もり精度が上がります。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、杉並区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "杉並区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜や早朝でも杉並区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、杉並区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "鍵が回りにくいのは、鍵とドアのどちらが原因ですか？",
    a: "ドアを開けた状態で施錠・解錠して切り分けます。開けた状態でスムーズに回るなら、ドアの傾きや受け金具のずれといった建て付け側の問題が疑われます。開けた状態でも回りにくいなら、シリンダー内部の摩耗や汚れが原因の可能性が高くなります。木造住宅は経年や湿気で建て付けが動きやすいため、この切り分けを最初に行うと無駄な交換を避けられます。",
    link: { href: "/kagi-mawaranai/", label: "鍵が回らないときの対処はこちら" },
  },
  {
    q: "鍵穴に潤滑油を差してもよいですか？",
    a: "食用油や一般的な潤滑油は差さないでください。油分がホコリを呼び込み、鍵穴の内部で固まって症状を悪化させることがあります。使うなら鍵穴専用の潤滑剤（パウダータイプ等）を少量にとどめ、それでも改善しない場合は無理に回さず業者に相談しましょう。",
  },
  {
    q: "古い鍵はどのタイミングで交換すべきですか？",
    a: "鍵の寿命は一般に10年程度が目安とされています。回りにくさなどの症状が「たまに」ではなく「毎回」出るようになったら交換を検討する段階です。古い錠前は部品の入手が難しい場合もあります。また、古い刻みキーはピッキング対策の面でも、防犯性の高いディンプルキーへの交換を検討する価値があります。賃貸は管理会社・大家に相談のうえで進めましょう。",
...
export default function AreaSuginamiKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】杉並区の鍵開け・鍵交換業者おすすめ5選！古い建物の鍵の不調・交換判断も分かる鍵屋比較",
    description:
      "杉並区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "杉並区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜杉並区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "杉並区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜杉並区で鍵業者を呼ぶ前に確認したいこと（古い建物の鍵の建て付けと交換判断）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜杉並区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">杉並区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「最近、鍵が引っかかって回しにくい」「築年数の経ったアパートで、鍵よりドアの建て付けが怪しい気がする」——昔ながらの住宅や木造アパートが残る杉並区では、経年による鍵の不調と建て付けの問題が絡み合ったトラブルが典型です。原因の切り分け方と、修理か交換かの判断基準を整理します。
            </p>
            <p>
              本記事では、杉並区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、古い建物で多い鍵の不調の切り分け方と交換判断もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">杉並区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            杉並区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、杉並区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">杉並区の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            杉並区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">杉並区で鍵業者を呼ぶ前に確認したいこと（古い木造住宅の鍵の不調の切り分けと交換判断）</H2>
          <p className="leading-loose mb-4">
            鍵が回りにくいとき、最初にやるべきは<Hl>「鍵の問題か、ドアの建て付けの問題か」の切り分け</Hl>です。ドアを開けた状態で施錠・解錠してみて、スムーズに回るならシリンダーではなく、ドアの傾きや受け金具（ストライク）のずれが原因の可能性が高くなります。木造住宅は経年や湿気でドアまわりが動きやすく、建て付け由来の不調は珍しくありません。逆にドアを開けた状態でも回りにくいなら、シリンダー内部の摩耗や汚れが疑われます。詳しくは<Link href="/kagi-mawaranai/" className="text-primary underline">鍵が回らないときの対処</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            応急手当てとして、鍵穴には<Hl>鍵穴専用の潤滑剤（パウダータイプ等）を使い、食用油や一般的な潤滑油は差さない</Hl>のが鉄則です。油分はホコリを呼び込み、内部で固まって症状を悪化させることがあります。鍵の寿命は一般に10年程度が目安とされ、古い錠前は部品の入手が難しくなっていることもあります。寿命のサインは<Link href="/kagi-jumyou/" className="text-primary underline">鍵の寿命の解説</Link>で、錠前の種類の見分け方は<Link href="/joumae-type-zukan/" className="text-primary underline">錠前タイプ図鑑</Link>でまとめています。
          </p>
          <p className="leading-loose mb-4">
            修理か交換かの判断は、<Hl>症状が「たまに」ではなく「毎回」出るようになったら交換を検討する段階</Hl>と考えると分かりやすいです。シリンダー交換は条件がそろえば自分で行える場合もありますが（<Link href="/cylinder-koukan-diy/" className="text-primary underline">シリンダー交換のDIY解説</Link>）、建て付け調整が絡む場合や古い錠前の型が分からない場合は業者への依頼が確実です。賃貸のアパートでは、交換・修理の前に必ず管理会社・大家へ相談しましょう。交換時期の考え方は<Link href="/kagi-koukan-timing/" className="text-primary underline">鍵交換のタイミング</Link>でも解説しています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">杉並区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">杉並区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            杉並区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            古い建物の鍵の不調は、「ドアを開けた状態で回るか」の切り分けを最初に行うと、建て付け調整で済むのか、シリンダー交換が必要なのかが見えてきます。鍵穴専用でない油を差さないことも忘れないでください。症状が毎回出るようになったら交換のサインです。交換のように日程が読める依頼は、複数社から総額の見積もりを取って比較するのが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["杉並区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-2">
              {[
                ...
                { href: "/kagi-jumyou/", label: "鍵の寿命と交換のサイン" },
                { href: "/joumae-type-zukan/", label: "錠前タイプ図鑑" },
                { href: "/cylinder-koukan-diy/", label: "シリンダー交換のDIY解説" },
