...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】広島の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "広島で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、賃貸マンションのオートロック締め出しで慌てないための知識まで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】広島の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "広島で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。賃貸マンションのオートロック締め出しで慌てないための知識や、業者を呼ぶ前の確認事項も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、広島市内をはじめ広島県内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」「折れた」など状況と、玄関・車・金庫といった対象、ディンプルキーなど鍵の種類をメモしておくと、電話での見積もり精度が上がります。オートロック付きマンションの場合は、締め出されたのがエントランスなのか自室の玄関なのかで対応が変わるため、そこも整理して伝えましょう。賃貸の場合は、業者より先に管理会社・大家への連絡が基本です。",
  },
  {
    t: "② 出張費・夜間料金を含めた総額の見積もりを取る",
    b: "広告の「◯◯円〜」は最低価格で、実際は鍵の種類・出張距離・時間帯で変わります。繁華街で夜遅くまで過ごした帰りに呼ぶ場合は、深夜割増の有無が総額を左右します。作業前に、出張費・部材費・深夜割増まで含めた総額を必ず確認しましょう。見積もり無料・キャンセル無料の業者なら、金額を見てから断ることもできます。",
  },
  {
    t: "③ 運営会社を確認する",
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で広島から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs = [
  {
    q: "広島の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "オートロックのマンションで締め出されました。鍵業者を呼べば開けてもらえますか？",
    a: "まず連絡すべきは管理会社・大家（または管理人）です。エントランスのオートロックは建物の共用部にあたるため、居住者の判断だけで業者に解錠させることはできず、業者側も本人確認や管理会社の許可を求めるのが通常です。管理会社が開けてくれれば費用もかかりません。深夜で連絡がつかない場合は、他の住人の出入りに合わせて共連れで入るのではなく、管理会社の緊急窓口の有無を確認しましょう。自室の玄関の鍵開けは、本人確認のうえで鍵業者に依頼できます。オートロック締め出しの予防策は別記事で解説しています。",
  },
  {
    q: "深夜や早朝でも広島に来てくれますか？",
...
export default function AreaHiroshimaPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】広島の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "広島で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "広島の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#hiroshima5",
      label: "1｜広島で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "広島の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#hiroshima-tips", label: "6｜広島で鍵業者を呼ぶ前に確認したいこと（オートロックの締め出し）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜広島の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">広島の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「ゴミ出しに出た数十秒でオートロックに締め出された」「流川で飲んだ帰りに鍵がないことに気づいた」——広島で鍵トラブルにあったとき、どの業者に頼めばよいか迷う方は多いはずです。市中心部を中心に賃貸マンション暮らしの方が多い広島では、部屋の鍵のトラブルに加えて、オートロックならではの締め出しも起こりがちです。オートロックのエントランスは共用部のため、実は「鍵業者を呼べば済む」とは限らず、連絡の順番を知っているかどうかで費用も解決までの時間も変わります。
            </p>
            <p>
              本記事では、広島市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。オートロックで締め出されたときの正しい動き方や、賃貸で業者を呼ぶ前に確認すべきこともまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="hiroshima5" num="1">広島で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            広島市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-2">
            ※他の都市の比較は、<Link href="/area/osaka/" className="text-primary underline">大阪版</Link>・<Link href="/area/kobe/" className="text-primary underline">神戸版</Link>・<Link href="/area/fukuoka/" className="text-primary underline">福岡版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">広島の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            流川・薬研堀など夜遅くまでにぎわう繁華街のある広島では、飲んだ帰りの深夜に紛失や締め出しに気づくケースも少なくありません。深夜・早朝の依頼では、24時間受付かどうかがまず重要です。一次確認値では、<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜広島特有：オートロック */}
          <H2 id="hiroshima-tips" num="6">広島で鍵業者を呼ぶ前に確認したいこと（オートロックの締め出し）</H2>
          <p className="leading-loose mb-4">
            賃貸マンション暮らしで特に注意したいのが、<Hl>ゴミ出しや郵便受けの確認などの「ちょっとそこまで」でオートロックに締め出されるトラブル</Hl>です。オートロックのエントランスは建物の共用部にあたるため、<Hl>まず連絡すべきは鍵業者ではなく管理会社・大家（または管理人）</Hl>です。業者もエントランスの解錠には管理会社の許可や本人確認を求めるのが通常で、管理会社が開けてくれれば費用はかかりません。自室の玄関の鍵開けは、本人確認のうえで業者に依頼できます。部屋着のまま締め出されないための予防策（携帯・キーケースの習慣化、家族との合鍵運用など）は<Link href="/autolock-shimedashi-yobou/" className="text-primary underline">オートロック締め出しの予防策</Link>で詳しく解説しています。
          </p>
          <p className="leading-loose mb-4">
            賃貸の鍵そのものについても、鍵は大家・管理会社の管理物にあたるため、<Hl>紛失・締め出しのときは鍵業者より先に管理会社・大家へ連絡するのが基本</Hl>です。合鍵やマスターキーで開けてもらえれば費用がかからずに済む場合があり、無断で鍵を開けたり交換したりすると契約上のトラブルになる恐れもあります。詳しい手順は<Link href="/shimedashi/" className="text-primary underline">鍵の閉じ込め・締め出しの対処法</Link>で、賃貸の防犯を高めたい場合は<Link href="/chintai-bouhan/" className="text-primary underline">賃貸でもできる防犯対策</Link>で解説しています。
          </p>
          <p className="leading-loose mb-4">
          ...
          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">広島の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">広島の鍵業者でよくある質問</H2>
                ...
                <p className="flex gap-3 px-5 py-4 text-sm leading-relaxed">
                  ...
                  <span>
                    ...
                    {f.q.includes("オートロック") && (
                      <>
                        {" "}
                        <Link href="/autolock-shimedashi-yobou/" className="text-primary underline">オートロック締め出しの予防策はこちら</Link>
                      </>
                    )}
                    {f.q.includes("賃貸") && (
                      <>
          ...
          <p className="leading-loose mb-4">
            広島で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝の繁華街帰りのトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            オートロック付きの賃貸マンションでは、締め出されたらまず管理会社・大家への連絡が先、が鉄則です。共用部のエントランスは居住者の判断だけでは開けられず、管理会社経由なら費用がかからないこともあります。そのうえで自室の鍵開けが必要なら、作業前に総額の見積もりを確認してから依頼することが失敗しないための共通ルールです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["広島", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
          ...
          {/* 市内の区 */}
          <div className="mt-10">
            <p className="text-sm font-bold text-text-muted mb-3">広島市内の区別ページ</p>
          ...
          <div className="mt-10">
            ...
            <ul className="grid gap-3 md:grid-cols-2">
              {[
                ...
                { href: "/autolock-shimedashi-yobou/", label: "オートロック締め出しの予防策" },
