...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】徳島の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較｜鍵トラブルナビ",
  },
  description:
    "徳島で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、戸建てで夜間に締め出されたときの動き方と家族の連絡体制づくりまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】徳島の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "徳島で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。深夜の締め出しで無理をしないための判断と、合鍵の分担・家族の連絡体制のつくり方も解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、徳島市内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「開かない」「なくした」「家族が中にいるが起きない」など状況と、鍵の種類をメモしておくと、電話での見積もり精度が上がります。深夜の締め出しでは、業者を呼ぶ前に合鍵を持つ家族・親族に連絡がつくかを先に確認しましょう。持ち家であれば所有の確認書類、家族名義であれば立ち会いについても依頼時に確認しておくとスムーズです。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。合鍵の作製や締め出し対策の補助錠追加のように日程が読める依頼なら、じっくり比較する価値があります。今回の5社はいずれも全国対応型で徳島から依頼できます。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "徳島の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜に締め出されました。施錠していない窓から入るのを試してもよいですか？",
    a: "おすすめできません。窓からの侵入はガラスの破損やけが、2階では転落の危険があるうえ、深夜に窓を乗り越える姿は近隣から不審者と誤解され通報につながることもあります。まず合鍵を持つ家族・親族への連絡を試し、朝まで待てるかどうかも含めて、無理のない選択肢から検討しましょう。",
    link: { href: "/shimedashi/", label: "締め出されたときの対処はこちら" },
  },
  {
    q: "合鍵を屋外に隠しておけば締め出し対策になりますか？",
    a: "植木鉢の下や郵便受けの裏といった定番の場所は、空き巣にもよく知られており危険です。屋外に置くならキーボックスを使い、外から見えにくい位置への設置と番号の定期変更を徹底してください。それよりも、家族それぞれが1本ずつ持つ・近くの親族に1本託すという「人に預ける」分担のほうが安全性は高くなります。",
    link: { href: "/key-box-kanri/", label: "キーボックスでの鍵管理の解説はこちら" },
  },
  {
    q: "家族で決めておくとよい締め出し対策はありますか？",
    a: "①家族の誰が合鍵を持つかの分担（全員1本＋近くの親族に1本が目安）、②帰りが遅くなる日の共有、③深夜でも起きられる連絡手段（電話の音量やインターホンの位置の確認）、の3つを決めておくと、締め出しの大半は業者を呼ばずに解決できます。あわせて外出時の施錠と鍵の持ち出し確認を習慣にしましょう。",
...
export default function AreaTokushimaPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】徳島の鍵開け・鍵交換業者おすすめ5選！24時間対応の鍵屋を徹底比較",
    description:
      "徳島で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "徳島の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#tokushima5",
      label: "1｜徳島で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "徳島の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tokushima-tips", label: "6｜徳島で鍵業者を呼ぶ前に確認したいこと（戸建ての夜間締め出しと家族の連絡体制）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜徳島の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">徳島の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「深夜に帰宅したら鍵がなく、家族は寝ていて呼び鈴にも電話にも気づかない」「家族そろって出かけたら、全員が鍵を持たずに出てしまっていた」——戸建ての夜間の締め出しは、オートロックのマンションとはまた違った形で起こります。戸建て住まいの多い徳島では、いざというとき誰に連絡し、誰なら開けられるのかという家族の体制をあらかじめ決めておくことが、深夜の業者依頼を減らすいちばんの近道になります。
            </p>
            <p>
              本記事では、徳島市内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、戸建てで夜間に締め出されたときの動き方と、家族の連絡体制のつくり方もまとめています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="tokushima5" num="1">徳島で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            徳島市内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          <p className="text-sm text-text-muted mb-8">
            ※他の都市の比較は、<Link href="/area/takamatsu/" className="text-primary underline">高松版</Link>・<Link href="/area/matsuyama/" className="text-primary underline">松山版</Link>・<Link href="/area/kobe/" className="text-primary underline">神戸版</Link>もご覧ください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">徳島の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            徳島でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜徳島特有 */}
          <H2 id="tokushima-tips" num="6">徳島で鍵業者を呼ぶ前に確認したいこと（戸建ての夜間締め出しと家族の連絡体制）</H2>
          <p className="leading-loose mb-4">
            夜間の締め出しでまず試すのは、<Hl>合鍵を持つ家族・親族への連絡</Hl>です。ここで効いてくるのが平時の体制づくりで、家族それぞれが1本ずつ持つ・近くの親族に1本託すという分担を決めておくと、選択肢が一気に増えます。家の中の家族が寝ていて気づかない事態に備え、深夜でも起きられる連絡手段（枕元の電話・インターホンの音量）を一度確認しておきましょう。動き方の全体像は<Link href="/shimedashi/" className="text-primary underline">締め出されたときの対処記事</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            やってはいけないのは、<Hl>施錠していない窓を探して乗り越えようとする</Hl>ことです。ガラスの破損やけが、2階からの転落の危険に加え、深夜に窓へ取り付く姿は近隣から不審者と誤解されかねません。また、合鍵を植木鉢の下などに隠す対策は、空き巣にも知られた定番の場所であり逆効果です。屋外に置くならキーボックスの運用ルール（見えにくい位置・番号の定期変更）を守りましょう。詳しくは<Link href="/key-box-kanri/" className="text-primary underline">キーボックスでの鍵管理の解説記事</Link>と<Link href="/aikagi/" className="text-primary underline">合鍵の解説記事</Link>をご覧ください。
          </p>
          <p className="leading-loose mb-4">
            家族に連絡がつかず業者を呼ぶ判断をするときは、<Hl>深夜割増の有無を含めた総額と到着目安を電話で確認してから</Hl>依頼します。朝には家族が起きる・帰ってくるという見通しがあるなら、近くで待てる場所を探して朝を待つほうが安く済む場合もあります。なくしたのか持ち忘れたのかで、その後にシリンダー交換まで検討すべきかも変わります。費用感は<Link href="/ryokin/" className="text-primary underline">料金相場ページ</Link>で確認しておきましょう。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">徳島の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">徳島の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            徳島で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            戸建ての夜間締め出しは、合鍵の分担と連絡体制という平時の備えで大半を防げます。深夜に起きてしまったときは、家族への連絡→待つ選択肢の検討→総額と到着目安を確認したうえでの業者依頼、という順で無理なく動きましょう。窓からの侵入や屋外への安易な鍵隠しは、危険や防犯上の逆効果につながるため避けるべきです。急がない依頼は複数社から総額の見積もりを取って比較することが失敗しないためのポイントです。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["徳島", "鍵開け", "鍵交換", "24時間対応", "業者比較", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
