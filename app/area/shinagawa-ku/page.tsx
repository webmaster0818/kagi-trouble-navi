...
export const metadata: Metadata = {
  title: {
    absolute:
      "【2026年最新】品川区の鍵開け・鍵交換業者おすすめ5選！単身赴任・転勤の入退去で慌てない鍵の段取りも分かる鍵屋比較｜鍵トラブルナビ",
  },
  description:
    "品川区で鍵開け・鍵交換を頼める鍵業者5社を一次確認値で比較。カギ110番（鍵開け6,600円〜）・鍵の生活救急車（最短15分）・JBR・鍵のレスキュー・カギの救急車の料金と受付時間、単身赴任・転勤の入居から退去までの鍵の段取りまで編集部が中立にまとめました。",
  alternates: { canonical: `${SITE}${PATH}` },
  openGraph: {
    title:
      "【2026年最新】品川区の鍵開け・鍵交換業者おすすめ5選！単身赴任・転勤の入退去で慌てない鍵の段取りも分かる鍵屋比較",
    description:
      "品川区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較。入居時の鍵交換の確認、入居中の合鍵管理、退去時の全本数返却まで、転勤族の鍵の段取りを時系列で解説します。",
    url: `${SITE}${PATH}`,
    type: "article",
...
const agents: AreaAgent[] = [
  {
    ...
    intro: (
      <>
        ...
        で、24時間365日受付・電話一本で最短5分の手配をうたっています。全国47都道府県の加盟店ネットワークが対象のため、品川区内の依頼でも近隣の加盟店が手配される仕組みです。見積もりは無料で、出張費・作業費・部材費を含めた金額を事前に提示する明朗会計を掲げています。
      </>
    ),
    recommend: [
    ...
    table: [
      ...
      { k: "受付時間", v: "24時間365日" },
      { k: "対応エリア", v: "全国47都道府県（東京都対応）", note: "東京都対応の一次確認値。品川区での詳細な対応可否・条件は依頼時に各社へ確認" },
...
const steps = [
  {
    t: "① 状況と鍵の種類を整理する",
    b: "「入居する部屋の鍵を交換したい」「退去までに合鍵を揃えたい」など目的と期日、賃貸なら管理会社の連絡先をメモしておくと段取りがスムーズです。賃貸の鍵まわりの交換・変更は、業者より先に管理会社・大家への確認が基本です。",
  },
  {
  ...
  {
    t: "④ 急ぎでなければ複数社を比較する",
    b: "締め出しなどの緊急時以外は、2〜3社から見積もりを取って総額・条件を比較するのがおすすめです。今回の5社はいずれも全国対応型で、品川区からも依頼できます（対応エリアは「東京都」としての一次確認値のため、詳細条件は各社へ確認を）。金額に納得できない場合は、その場で契約せず一度断る選択肢もあります。",
  },
];

const faqs: AreaFaq[] = [
  {
    q: "品川区の鍵開け料金の相場はいくらですか？",
  ...
  {
    q: "深夜や早朝でも品川区に来てくれますか？",
    a: "カギ110番・JBR・鍵のレスキューは24時間365日受付を掲げており、カギの救急車も多くの加盟店が24時間受付です。ただし深夜帯は割増料金や到着時間の変動があり得るため、電話の際に深夜料金の有無と到着目安を確認してから依頼するのがおすすめです。なお各社の対応エリアは「東京都」としての一次確認値のため、品川区での詳細は依頼時に各社へご確認ください。",
  },
  {
    q: "入居時の鍵交換は必ず必要ですか？費用は誰の負担ですか？",
    a: "法律で義務付けられているものではなく、実施の有無も費用負担も契約・物件によって異なります。前の入居者や出所の分からない合鍵のリスクを断つ意味で、交換されているかどうかを入居前に確認するのがおすすめです。交換費用を入居者負担とする契約も多いため、金額と内訳は契約書・重要事項説明で確認しましょう。",
    link: { href: "/chintai-kagi-koukan-hiyou/", label: "賃貸の鍵交換費用はこちら" },
  },
  {
    q: "転勤で退去しますが、合鍵を1本なくしたままです。どうすればいいですか？",
    a: "隠さずに管理会社・大家へ申告するのが最善です。鍵が全数返却されない場合、防犯上の理由からシリンダー交換費用を請求されるのが一般的で、金額や負担割合は契約によります。退去立ち会いの当日に発覚すると精算がこじれやすいため、退去日が決まった時点で本数を確認し、早めに相談しましょう。",
    link: { href: "/taikyo-kagi-trouble/", label: "退去時の鍵トラブルはこちら" },
  },
  {
    q: "単身赴任先の合鍵を家族に渡しておきたいのですが、問題ありませんか？",
    a: "賃貸では合鍵の作製に貸主・管理会社の承諾を求める契約が一般的なので、まず契約内容を確認しましょう。承諾を得て作る場合も、鍵番号が写った写真をSNSなどに載せない・渡した本数を把握しておく・退去時に全数回収する、の3点を守ると安心です。",
...
export default function AreaShinagawaKuPage() {
  const articleSchema = {
    ...
    headline:
      "【2026年最新】品川区の鍵開け・鍵交換業者おすすめ5選！単身赴任・転勤の入退去で慌てない鍵の段取りも分かる鍵屋比較",
    description:
      "品川区で鍵開け・鍵交換を頼める鍵業者5社を、料金・受付時間の一次確認値で比較した記事です。",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "品川区の鍵開け・鍵交換業者おすすめ5選", item: `${SITE}${PATH}` },
    ],
  };
  ...
  const toc: { href: string; label: string; sub?: { href: string; label: string }[] }[] = [
    {
      href: "#top5",
      label: "1｜品川区で人気のおすすめ・優良の鍵業者5選",
      sub: [
        { href: "#hikaku", label: "品川区の鍵業者を一覧表で徹底比較" },
    ...
    { href: "#tips", label: "6｜品川区で鍵業者を呼ぶ前に確認したいこと（単身赴任・転勤の入退去と鍵の段取り）" },
    { href: "#erabikata", label: "7｜失敗しない鍵業者の選び方" },
    { href: "#faq", label: "8｜品川区の鍵業者でよくある質問" },
        ...
        <div className="mx-auto max-w-4xl px-4 pt-4 pb-12 md:pb-16">
          ...
          <nav aria-label="パンくず" className="text-xs text-text-muted mb-4">
            <ol className="flex flex-wrap gap-1">
              ...
              <li>/</li>
              <li className="text-foreground">品川区の鍵開け・鍵交換業者おすすめ5選</li>
            </ol>
          </nav>
          ...
          <div className="mt-6 leading-loose text-foreground">
            <p className="mb-4">
              「辞令が出て、来月から単身赴任」「数年ごとの転勤で、入居と退去を繰り返している」——交通の便がよく、単身赴任や転勤にともなう住み替えの多い品川区では、入退去のたびに発生する鍵の段取りが悩みどころです。入居時の鍵交換、暮らしている間の合鍵、退去時の返却まで、節目ごとにやるべきことを押さえておくと、余計な出費とトラブルを防げます。
            </p>
            <p>
              本記事では、品川区内から依頼できる主要な鍵業者5社を、当サイト編集部が各社公式サイト等で一次確認した料金・受付時間・運営会社の情報だけで比較しました。あわせて、単身赴任・転勤の入居から退去までの鍵の段取りを時系列で整理しています。ぜひ鍵業者選びの参考にしてください。
            </p>
          </div>
          ...
          {/* 1｜5選 */}
          <H2 id="top5" num="1">品川区で人気のおすすめ・優良の鍵業者5選</H2>
          <p className="leading-loose mb-2">
            品川区内から依頼できる主要な鍵業者5社を紹介します。掲載している料金・受付時間・運営会社は、いずれも編集部が各社公式サイト等で一次確認した値です（確認日は各社の欄に記載）。各社の対応エリアは「東京都」としての一次確認値のため、品川区での詳細な対応可否・条件は依頼時に各社へご確認ください。実際の総額は鍵の種類・時間帯・出張距離で変わるため、必ず作業前に見積もりで確認してください。
          </p>
          ...
          {/* 2｜急ぎ */}
          <H2 id="isogi" num="2">品川区の鍵業者をとにかく急いで開けてほしいなら</H2>
          ...
          <p className="leading-loose mb-4">
            品川区でも、深夜の締め出しや帰宅後に紛失へ気づくトラブルは起こり得ます。この時間帯に頼れるのは24時間受付の業者で、一次確認値では<Hl>カギ110番・JBR・鍵のレスキューが24時間365日受付</Hl>を掲げており、カギの救急車も多くの加盟店が24時間受付です。特にJBRは賠償責任保険に加入した大手グループで、夜間の緊急依頼でも相談しやすい体制です。
          </p>
          <p className="leading-loose mb-4 text-sm text-text-muted">
          ...
          {/* 6｜区特有 */}
          <H2 id="tips" num="6">品川区で鍵業者を呼ぶ前に確認したいこと（単身赴任・転勤族の入退去と鍵の段取り）</H2>
          <p className="leading-loose mb-4">
            入居時の出発点は、<Hl>鍵交換の実施の有無と費用負担を契約書で確認する</Hl>ことです。入居時の鍵交換は、前の入居者や出所の分からない合鍵のリスクを断つためのもので、実施の有無・負担者は物件によって異なります。交換されていない部屋を自費で交換したい場合も、必ず貸主・管理会社の承諾を得てから進めましょう。費用感は<Link href="/chintai-kagi-koukan-hiyou/" className="text-primary underline">賃貸の鍵交換費用</Link>、入居前後のやることは<Link href="/hikkoshi-kagi-checklist/" className="text-primary underline">引っ越しの鍵チェックリスト</Link>にまとめています。
          </p>
          <p className="leading-loose mb-4">
            入居中は、<Hl>合鍵を勝手に増やさない</Hl>ことが大切です。賃貸の鍵は貸主からの貸与物で、複製に承諾を求める契約が一般的です。単身赴任で留守がちの住まいこそ、鍵をなくしたときは放置せず管理会社へ報告し、必要に応じて交換を相談しましょう。外出先で紛失したときの動き方は<Link href="/kagi-funshitsu/" className="text-primary underline">鍵をなくしたときの対処</Link>、合鍵の考え方は<Link href="/aikagi/" className="text-primary underline">合鍵の基礎知識</Link>を参考にしてください。
          </p>
          <p className="leading-loose mb-4">
            退去時は、<Hl>入居時に受け取った鍵と、承諾を得て作った合鍵を含む全本数の返却</Hl>が基本です。1本でも欠けていると、防犯上の理由からシリンダー交換費用を請求される場合があります。転勤の慌ただしさの中では、紛失に気づくのが退去直前になりがちです。退去日が決まったら、まず手元の鍵の本数を数えることから始めましょう。退去時に起きやすいトラブルと対処は<Link href="/taikyo-kagi-trouble/" className="text-primary underline">退去時の鍵トラブル</Link>で詳しく解説しています。
          </p>

          {/* 7｜選び方 */}
          <H2 id="erabikata" num="7">品川区の鍵業者の失敗しない選び方</H2>
          ...
          {/* 8｜FAQ */}
          <H2 id="faq" num="8">品川区の鍵業者でよくある質問</H2>
          ...
          <p className="leading-loose mb-4">
            品川区で鍵開け・鍵交換を頼める主要5社を、一次確認した料金・受付時間・運営会社の情報で比較しました。スピード重視なら鍵の生活救急車やカギ110番、深夜・早朝のトラブルならカギ110番・JBR・鍵のレスキュー、料金の明確さ重視なら鍵のレスキュー、というように、目的によって向いている業者は変わります。
          </p>
          <p className="leading-loose mb-8">
            単身赴任・転勤の住み替えは、入居時の「交換の有無と負担の確認」、入居中の「合鍵管理と紛失時の報告」、退去時の「全本数の返却」という3つの節目を押さえるだけで、鍵にまつわる出費とトラブルの多くを防げます。業者へ依頼する場面でも、賃貸では管理会社・大家への確認を先に。本記事の一次確認値を出発点に、状況に合った業者を選んでください。
          </p>
          ...
          <div className="mt-12">
            ...
            <ul className="flex flex-wrap gap-2">
              {["品川区", "東京23区", "鍵開け", "鍵交換", "24時間対応", "エリア別"].map((t) => (
                <li
                  key={t}
                  className="rounded-[2px] border border-border bg-white px-3 py-1.5 text-xs md:text-sm font-bold tracking-wide text-foreground shadow-[0px_2px_2px_0px_rgba(0,0,0,0.12)]"
                >
